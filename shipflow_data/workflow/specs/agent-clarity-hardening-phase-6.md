...
owner: "Diane"
user_story: "As the ShipFlow operator, I want tracker-maintenance and continuation skills to distinguish tracker bookkeeping vs next-step continuation in the first screen, so fresh agents do not confuse updating TASKS state with advancing the active work item."
confidence: high
risk_level: medium
security_impact: no
docs_impact: yes
linked_systems:
  - "skills/309-sf-tasks/SKILL.md"
...
supersedes:
  - "shipflow_data/workflow/specs/agent-clarity-hardening-phase-5.md"
evidence:
  ...
  - "Fresh agents can still confuse `309-sf-tasks` and `706-continue` because both can suggest a next step near `TASKS.md`, but one owns tracker bookkeeping while the other advances the currently resolved work item."
next_step: "/009-sf-skill-build continue agent-clarity hardening after the phase-6 pilot"
---
...
As the ShipFlow operator, I want tracker-maintenance and continuation skills to distinguish tracker bookkeeping vs next-step continuation in the first screen, so fresh agents do not confuse updating TASKS state with advancing the active work item.
...
- `309-sf-tasks` makes it obvious that it maintains task tracker state and suggests tracker-based next steps, but does not become the default continuation owner for active execution.
...
- `shipflow_data/technical/skill-runtime-and-lifecycle.md` captures the `track / continue` distinction so future edits keep these surfaces separate.

## Error Behavior

- If `309-sf-tasks` appears to own retrospective review, backlog grooming, or execution continuation by default, reject it.
...
- `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/agent-clarity-hardening-phase-6.md shipflow_data/technical/skill-runtime-and-lifecycle.md`
- `tools/shipflow_sync_skills.sh --check --skill 309-sf-tasks`
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-27 | 009-sf-skill-build | GPT-5 Codex | Create phase-6 spec and start tracker/continuation clarity hardening | implemented | Patch targeted skills, validate, and ship if green |

## Current Chantier Flow

- `100-sf-spec` ✅ phase-6 spec created
- `101-sf-ready` ✅ ready via bounded continuation of the active clarity hardening campaign
- `102-sf-start` 🔄 in progress on tracker/continuation boundary clarifications
