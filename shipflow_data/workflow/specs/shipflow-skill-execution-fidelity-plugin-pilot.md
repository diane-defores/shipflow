...
owner: "Diane"
user_story: "As the ShipFlow operator testing the first Codex plugin pilot, I want ShipFlow to measure and improve skill execution fidelity, so Codex follows the intended gates instead of treating long or ambiguous skill bodies as loose guidance."
confidence: medium
risk_level: high
security_impact: yes
docs_impact: yes
...
evidence:
  ...
  - "User concern 2026-05-25: Codex does not appear to follow ShipFlow skills to the letter and the skills are not yet excellent."
...
# Spec: ShipFlow Skill Execution Fidelity Plugin Pilot

## Title

ShipFlow Skill Execution Fidelity Plugin Pilot
...
As the ShipFlow operator testing the first Codex plugin pilot, I want ShipFlow to measure and improve skill execution fidelity, so Codex follows the intended gates instead of treating long or ambiguous skill bodies as loose guidance.

## Minimal Behavior Contract

When the ShipFlow plugin pilot surfaces skill-quality findings, ShipFlow must distinguish true execution-fidelity risks from heuristic false positives, define a mechanical audit that checks whether skills expose the gates Codex must obey, and apply only a bounded first remediation batch. The system must not rewrite all skills, rename invocations, or treat budget compliance as proof of execution quality. The easiest edge case to miss is turning every missing `Mission` heading into churn while ignoring the real problem: whether a fresh Codex run can find the required trigger, owner boundary, stop conditions, validation, reporting gate, and agent-run next proof step quickly enough to follow them without operator micromanagement.

## Success Behavior

- Preconditions: `shipflow-core@personal` is installed locally, the ShipFlow repo is clean, and the official `tools/skill_budget_audit.py` already passes except for known body-size risks.
...
- System effect: the plugin audit becomes more precise, a durable ShipFlow execution-fidelity policy or check exists, and the first selected skills or plugin surfaces are updated without weakening lifecycle gates.
...
Create a bounded execution-fidelity pass. First, improve the plugin audit so it classifies findings as hard risks, review-needed risks, and style-only signals. Second, define a ShipFlow-local fidelity contract for skill activation bodies: trigger, mission/owner boundary, required references, stop conditions, validation/proof, and reporting must be visible or explicitly delegated to a loaded shared reference. Third, remediate only a small first batch selected by evidence, not the whole corpus.
...
- Add or update a ShipFlow-owned reference or tool only if the policy needs to be durable outside the plugin pilot.
...
- Converting the plugin into the canonical ShipFlow installer.
...
- ShipFlow repo changes must remain scoped and reviewable.
- Plugin changes live under `/home/claude/plugins/shipflow-core/` and remain separate from the ShipFlow repo unless explicitly promoted.
...
- Runtime skill sync check when ShipFlow skills change: `tools/shipflow_sync_skills.sh --check --all`
...
- `shipflow-core` becomes the first test surface for packaging and quality-checking ShipFlow outside the monorepo.
...
- `sf-skill-build` owns any material skill-contract edit.
- `sf-verify` must be able to judge whether the first remediation batch improved obedience without weakening existing gates.

## Documentation Coherence

- Update the plugin README when plugin testing or reinstall steps change.
- Update a ShipFlow reference only if the fidelity policy becomes durable for future skill work.
- Update public docs only if the plugin or skills make a public promise, which is out of scope for this first pilot.
- Changelog/closure updates are deferred to the normal `sf-end`/`sf-ship` path if the implementation proceeds beyond this spec.
...
- [x] Task 3: Decide whether to promote the fidelity policy into ShipFlow
  ...
  - User story link: Prevents the pilot from remaining a one-off script with no ShipFlow governance path.
  - Depends on: Task 1
...
- [x] Task 4: Select and remediate one first batch only if evidence supports it
  ...
  - Notes: No ShipFlow skills were edited because no hard skill-contract findings remained after audit improvements.

- [x] Task 5: Verify runtime and report remaining follow-ups
  - File: `shipflow_data/workflow/specs/shipflow-skill-execution-fidelity-plugin-pilot.md`
  - Action: Append run history, verify plugin and ShipFlow checks, and list follow-up candidates.
...
- Run `tools/shipflow_sync_skills.sh --check --all` only when ShipFlow skills are edited.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-25 12:54:14 UTC | sf-spec | GPT-5 Codex | Created ready spec from the shipflow-core plugin pilot and skill-obedience concern. | ready | /sf-skill-build shipflow-skill-execution-fidelity-plugin-pilot |
| 2026-05-25 12:54:14 UTC | sf-ready | GPT-5 Codex | Self-reviewed readiness against required sections, scope, risk, proof path, and open questions. | ready | /sf-skill-build shipflow-skill-execution-fidelity-plugin-pilot |
| 2026-05-25 12:57:21 UTC | sf-skill-build | GPT-5 Codex | Improved shipflow-core audit classification, validated and reinstalled the plugin, and stopped without skill edits because no hard skill-contract findings remained. | implemented | /sf-verify shipflow-skill-execution-fidelity-plugin-pilot |
| 2026-05-25 13:03:00 UTC | sf-skill-build | GPT-5 Codex | Added durable skill execution-fidelity reference for future skill update specs. | implemented | /sf-verify shipflow-skill-execution-fidelity-plugin-pilot |
| 2026-05-26 00:00:00 UTC | sf-skill-build | GPT-5 Codex | Added operator-last-resort proof rule from conversation evidence about agents asking the user to continue/retest. | implemented | /sf-verify shipflow-skill-execution-fidelity-plugin-pilot |
...
- sf-end: pending
- sf-ship: pending
- Next step: /sf-verify shipflow-skill-execution-fidelity-plugin-pilot
