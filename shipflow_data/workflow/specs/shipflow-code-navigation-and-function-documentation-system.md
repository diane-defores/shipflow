...
linked_systems:
  ...
  - "skills/301-sf-context/SKILL.md"
...
evidence:
  ...
  - "Existing ShipFlow technical layer already provides path -> doc mapping, but it does not define a portfolio-wide term -> behavior -> function navigation contract."
...
next_step: "/102-sf-start ShipFlow Code Navigation And Function Documentation System"
---

# Spec: ShipFlow Code Navigation And Function Documentation System

## Title

ShipFlow Code Navigation And Function Documentation System

## Status

Ready.

This chantier extends the existing ShipFlow technical documentation layer. It does not replace `shipflow_data/technical/code-docs-map.md`; it adds the missing navigation contract that starts from operator language or product behavior and lands on the right code, comments, decisions, tests, and docs.

## User Story

En tant qu'operatrice ShipFlow qui pilote un portefeuille d'au moins 20 sites et apps avec des agents, je veux un systeme global de navigation et de documentation du code, afin qu'un agent puisse partir d'un terme fonctionnel comme swipe, sync, checkout ou auth et retrouver rapidement les comportements, fonctions, decisions, tests et docs associes sans refaire une enquete a chaque fois.

## Minimal Behavior Contract

ShipFlow must provide a portfolio-scale code navigation standard where every governed code project can map operator terms and product behaviors to canonical technical docs, behavior references, code entrypoints, key functions/classes, tests, specs, bugs, and decisions. When an agent receives a term such as `swipe`, it first checks the project's technical navigation artifacts, resolves ambiguous meanings into named behaviors, opens the mapped code and docs, and reports any missing coverage as a governance gap. If a term is unmapped, the system must guide the agent to create or route a bounded documentation update instead of silently rediscovering the same code later; the easy-to-miss edge case is that one word can represent several different behaviors across one project or across the portfolio.
...
- Given a codebase contains a high-complexity module, when `300-sf-docs technical audit` runs, then it verifies that the module has path mapping, behavior mapping, source-comment coverage for high-cognitive-load functions, linked tests, and decision records for durable design choices.
- Given a fresh agent starts a code task, when it opens `shipflow_data/technical/code-docs-map.md`, then the map points not only to the primary subsystem doc but also to behavior indexes or domain-model docs when the subsystem needs concept-level recovery.
...
- If a mapped behavior points to missing files, renamed functions, deleted tests, stale specs, or obsolete decisions, `300-sf-docs technical audit` reports drift and blocks a clean documentation verdict.
...
ShipFlow already has a strong technical documentation layer: `code-docs-map.md` maps paths to primary docs, subsystem docs describe durable behavior, and documentation governance rules define placement and update discipline. That layer is necessary but not sufficient for portfolio-scale agent work.
...
Create a ShipFlow-wide code navigation standard with four layers:

1. `code-docs-map.md` remains the path-to-doc map.
2. A new behavior/concept navigation layer maps operator terms and product behaviors to code, docs, tests, and decisions.
3. Stack-specific source comments document high-cognitive-load symbols only, using KDoc/JSDoc/TSDoc/Python docstrings or the local equivalent.
4. ADR/decision records capture durable rationale that should not live in comments or chantier specs.

Implement this as a reusable ShipFlow governance capability through references, templates, `300-sf-docs` audit/bootstrap behavior, and one concrete WinFlowz pilot for IME gestures.
...
- Define a ShipFlow code navigation doctrine in a new shared reference, for example `skills/references/code-navigation-and-function-docs.md`.
...
- Update `300-sf-docs` technical mode so it can bootstrap and audit behavior indexes, not only path maps.
- Update `301-sf-context` or its references if needed so context priming reads behavior navigation artifacts for term-based requests.
...
- Pilot the system on WinFlowz IME gestures by creating or specifying `shipflow_data/technical/winflowz_app/ime-gesture-model.md` and connecting it from the WinFlowz `code-docs-map.md`.
...
- In monorepos, behavior docs live under the root `shipflow_data/technical/`, scoped by surface when needed.
- Shared ShipFlow references and templates are sequential integration files.
...
- Stack/surface: `ShipFlow governance`, `documentation`, `skills`, `templates`, `WinFlowz pilot`
...
- Needed: yes
- Checklist path: `shipflow_data/workflow/test-checklists/code-navigation-and-function-documentation-system.md`
...
- Exception with proof: device/runtime behavior is not required for the ShipFlow standard itself; WinFlowz IME runtime proof remains owned by its IME chantiers.
...
- Contract/integration proof: run `300-sf-docs technical audit` or targeted equivalent on ShipFlow and WinFlowz after implementation.
...
- WinFlowz pilot contracts:
  - `/home/claude/winflowz/shipflow_data/technical/winflowz_app/code-docs-map.md`
...
- `shipflow_data/technical/code-docs-map.md` remains the first path-based map.
...
- Upstream systems: `000-shipflow`, `100-sf-spec`, `300-sf-docs`, `301-sf-context`, `102-sf-start`, `103-sf-verify`, project governance references.
- Downstream systems: every ShipFlow-governed code project, beginning with WinFlowz as the pilot.
...
- Agent workflow consequence: context priming can load small, relevant navigation artifacts before opening broad code.
...
- Updated `300-sf-docs` technical mode and possibly mode playbooks.
...
- [ ] Task 1: Define the ShipFlow code navigation doctrine.
  - File: `skills/references/code-navigation-and-function-docs.md`
  - Action: Create the shared reference that defines the layer model, term resolution flow, behavior index rules, comment rules, ADR boundaries, stale-map handling, and security constraints.
...
- [ ] Task 4: Wire the standard into technical corpus loading.
  ...
  - Action: Add term-based recovery: load `code-docs-map.md`, then behavior index/domain model when operator wording names a behavior rather than a path.
...
- [ ] Task 6: Update `300-sf-docs` technical mode.
  - File: `skills/300-sf-docs/SKILL.md`, `skills/300-sf-docs/references/mode-playbooks.md`
  ...
  - Validate with: `rg -n "behavior index|function documentation|source comment|technical_behavior_index|code navigation" skills/300-sf-docs`
  - Notes: Route non-trivial skill contract changes through `009-sf-skill-build` if readiness identifies that as required.

- [ ] Task 7: Update context priming if needed.
  - File: `skills/301-sf-context/SKILL.md` or its references
  ...
  - Depends on: Task 4.
  - Validate with: `rg -n "behavior index|operator term|code navigation|technical-docs-corpus" skills/301-sf-context`
  - Notes: Keep context loading bounded.

- [ ] Task 8: Pilot the system on WinFlowz IME gestures.
  - File: `/home/claude/winflowz/shipflow_data/technical/winflowz_app/ime-gesture-model.md`
  ...
  - Depends on: Tasks 1-2.
  - Validate with: `python3 /home/claude/shipflow/tools/shipflow_metadata_lint.py /home/claude/winflowz/shipflow_data/technical/winflowz_app/ime-gesture-model.md`
  - Notes: Do not change IME behavior in this task.

- [ ] Task 9: Connect the WinFlowz pilot to its code-docs map.
  - File: `/home/claude/winflowz/shipflow_data/technical/winflowz_app/code-docs-map.md`
  ...
  - Depends on: Task 8.
  - Validate with: `rg -n "ime-gesture-model|long-press swipe|WinFlowzKeyboardView|KeyboardLongPressSwipePolicy" /home/claude/winflowz/shipflow_data/technical/winflowz_app/code-docs-map.md`
...
- [ ] Task 11: Add a manual proof checklist for the standard.
  - File: `shipflow_data/workflow/test-checklists/code-navigation-and-function-documentation-system.md`
  ...
  - Validate with: `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/test-checklists/code-navigation-and-function-documentation-system.md`
  - Notes: Checklist proves documentation navigation, not runtime IME behavior.

- [ ] Task 12: Validate the ShipFlow and WinFlowz standard.
...
- [ ] AC 6: Given `300-sf-docs technical audit` runs on a project with behavior indexes, when references are stale or missing, then the audit reports the exact missing or stale coverage.
...
Validation commands for ShipFlow implementation:

```bash
python3 tools/shipflow_metadata_lint.py skills/references/code-navigation-and-function-docs.md templates/artifacts/technical_behavior_index.md shipflow_data/workflow/specs/shipflow-code-navigation-and-function-documentation-system.md
python3 tools/skill_budget_audit.py --skills-root skills --format markdown
tools/shipflow_sync_skills.sh --check --all
rg -n "behavior index|operator term|function documentation|source comment|code navigation" skills/references skills/300-sf-docs templates/artifacts shipflow_data/technical
```

Validation commands for WinFlowz pilot:

```bash
python3 /home/claude/shipflow/tools/shipflow_metadata_lint.py /home/claude/winflowz/shipflow_data/technical/winflowz_app/ime-gesture-model.md /home/claude/winflowz/shipflow_data/technical/winflowz_app/code-docs-map.md
rg -n "ime-gesture-model|long-press swipe|WinFlowzKeyboardView|KeyboardLongPressSwipePolicy|tryDispatchAfterLongPressSwipe|tryActivateLongPressSwipeFromExit" /home/claude/winflowz/shipflow_data/technical /home/claude/winflowz/winflowz_app/android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime
```
...
- Risk: portfolio rollout becomes too broad. Mitigation: pilot on WinFlowz, then roll out through `300-sf-docs technical audit` project by project.
...
- `skills/references/project-governance-rules.md`
- `skills/300-sf-docs/SKILL.md`
- `skills/300-sf-docs/references/mode-playbooks.md`
- `shipflow_data/technical/code-docs-map.md`
- `templates/artifacts/technical_module_context.md`
- `/home/claude/winflowz/shipflow_data/technical/winflowz_app/code-docs-map.md`
- `/home/claude/winflowz/winflowz_app/docs/technical/android-native.md`
...
2. Integrate the doctrine into `300-sf-docs` and technical corpus loading.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-30 13:47:10 UTC | 100-sf-spec | GPT-5 Codex | Created spec for portfolio-scale code navigation and function documentation system with WinFlowz IME pilot | draft | /101-sf-ready ShipFlow Code Navigation And Function Documentation System |
| 2026-06-30 14:02:30 UTC | 101-sf-ready | GPT-5 Codex | Reviewed readiness, scope integrity, proof contract, documentation coherence, and security posture | ready | /102-sf-start ShipFlow Code Navigation And Function Documentation System |
...
Next step: `/102-sf-start ShipFlow Code Navigation And Function Documentation System`
