...
owner: "Diane"
user_story: "En tant qu'utilisatrice ShipFlow, je veux que les skills de cadrage, pilotage et exploration chargent explicitement la doctrine opérateur/questions et qu'un garde-fou détecte les oublis, afin que ShipFlow pose les bonnes questions métier au lieu de se déclarer bloqué trop tôt."
confidence: high
risk_level: medium
...
linked_systems:
  ...
  - "skills/702-sf-priorities/SKILL.md"
...
next_step: "/101-sf-ready operator-partnership-doctrine-propagation-and-coverage-guard"
---

# Spec: Operator Partnership Doctrine Propagation And Coverage Guard

🟠 [ShipFlow] spec: Operator Partnership Doctrine Propagation And Coverage Guard | status: draft | path: shipflow_data/workflow/specs/operator-partnership-doctrine-propagation-and-coverage-guard.md | next: /101-sf-ready operator-partnership-doctrine-propagation-and-coverage-guard
...
The direction is clear enough to specify. The expected implementation path is bounded skill-maintenance work inside ShipFlow, with no external-doc dependency. Readiness should focus on target-skill list, acceptance criteria, and the automation boundary.

## User Story

En tant qu'utilisatrice ShipFlow, je veux que les skills de cadrage, pilotage et exploration chargent explicitement la doctrine opérateur/questions et qu'un garde-fou détecte les oublis, afin que ShipFlow pose les bonnes questions métier au lieu de se déclarer bloqué trop tôt.

## Minimal Behavior Contract

The selected ShipFlow skills that frequently frame work, ask questions, or decide blocked states must explicitly load `skills/references/question-contract.md` and/or `skills/references/operator-partnership-contract.md` when their behavior depends on user-facing questions or operator-owned business truth. A lightweight coverage guard must make missing links visible during ShipFlow maintenance, without forcing every skill in the repo to load both references indiscriminately.

## Success Behavior

- Preconditions: shared doctrine references already exist and are active.
- Trigger: ShipFlow skill-maintenance work targets the next propagation wave after the initial doctrine rollout.
...
Run a second bounded propagation wave across the highest-value remaining skills, then add a minimal doctrine-coverage check or documented validation pattern that makes missing explicit links visible in future ShipFlow maintenance. Keep the guard narrow, pragmatic, and category-aware rather than universal.

## Scope In

- Update the next propagation-wave skills with explicit doctrine links where behavior justifies them.
- Define the target skill set for this wave, starting from high-risk framing/pilotage/content/bug-maintenance owners such as `002-sf-maintain`, `003-sf-bug`, `007-sf-content`, `702-sf-priorities`, and `705-sf-conversation-audit`.
...
- The coverage guard must be easy to run during ShipFlow maintenance and should not depend on fragile heuristics that classify all skills identically.
- Respect existing unrelated dirty files in the ShipFlow worktree.
...
Fresh external docs: `fresh-docs not needed` — this chantier is governed by local ShipFlow skill contracts and technical docs, not unstable external framework behavior.
...
- `009-sf-skill-build` and future refresh/maintenance work gain a clear validation target for operator-collaboration behavior.
...
- [ ] Task 4: Update mapped technical documentation.
  - Files: `shipflow_data/technical/skill-runtime-and-lifecycle.md`, `shipflow_data/technical/code-docs-map.md`, and any directly impacted governance doc.
...
- [ ] Task 5: Run ShipFlow validation for the chantier.
...
python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/operator-partnership-doctrine-propagation-and-coverage-guard.md shipflow_data/technical/skill-runtime-and-lifecycle.md shipflow_data/technical/code-docs-map.md
rg -n "question-contract|operator-partnership-contract|operator-owned|business|framing|blocked" skills/002-sf-maintain/SKILL.md skills/003-sf-bug/SKILL.md skills/007-sf-content/SKILL.md skills/702-sf-priorities/SKILL.md skills/705-sf-conversation-audit/SKILL.md
```
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-29 00:00:00 UTC | 100-sf-spec | Codex GPT-5 | Created the chantier spec for the second propagation wave of operator/question doctrine links and a lightweight coverage guard. | implemented | /101-sf-ready operator-partnership-doctrine-propagation-and-coverage-guard |

## Current Chantier Flow

- `100-sf-spec`: implemented; durable spec created for the next doctrine propagation wave.
