...
owner: "Diane"
user_story: "En tant qu'operatrice ShipFlow, je veux qu'un verdict partial pour preuve hebergee manquante soit toujours converti en route proprietaire concrete, afin de savoir quel skill lancer, sur quel environnement, et pour quel scenario sans devoir inferer le prochain geste."
confidence: high
risk_level: medium
security_impact: "yes"
docs_impact: "yes"
...
depends_on:
  ...
  - artifact: "skills/references/spec-driven-development-discipline.md"
    artifact_version: "1.3.0"
    required_status: "active"
  - artifact: "shipflow_data/workflow/specs/skill-reporting-modes-and-compact-reports.md"
    artifact_version: "1.1.0"
    required_status: "ready"
  - artifact: "shipflow_data/workflow/specs/spec-driven-tdd-evidence-gates.md"
    artifact_version: "1.0.0"
    required_status: "ready"
  - artifact: "shipflow_data/workflow/conversation-audits/conversation-shipflow-doctrine-de-langue-20260530-203004-audit.md"
    artifact_version: "1.0.0"
    required_status: "draft"
supersedes: []
evidence:
  - "sf-conversation-audit 2026-06-10: sf-verify partial named missing hosted proof but did not route it to a concrete owner such as sf-prod, sf-test, sf-browser, or sf-auth-debug."
...
ShipFlow Hosted Proof Follow-Through Routing
...
En tant qu'opératrice ShipFlow, je veux qu'un verdict partial pour preuve hébergée manquante soit toujours converti en route propriétaire concrète, afin de savoir quel skill lancer, sur quel environnement, et pour quel scénario sans devoir inférer le prochain geste.

Acteur principal: opératrice ShipFlow qui lit un verdict de skill.
...
When a required hosted, preview, production, auth, browser, manual, webhook, provider, or deployment proof is missing, the responsible ShipFlow skill must translate that proof gap into a specific owner route such as `sf-ship -> sf-prod`, `sf-prod -> sf-browser`, `sf-prod -> sf-auth-debug`, or `sf-test --preview`, including the scenario to run and the target/environment prerequisite. The easy edge case is a technically accurate partial report that lists missing evidence nouns but leaves the operator to infer which skill owns the next proof.

## Success Behavior

- Given a partial `sf-verify` caused by missing hosted proof, when the report names the gap, then it also names the owner route and scenario, for example `sf-ship web-ui -> sf-prod winflowz-preview -> sf-test --preview login-smoke`.
- Given a hosted proof requires a push/deploy first, when the report names the next proof, then it routes through `sf-ship -> sf-prod` before browser/auth/manual proof.
- Given the deployment URL is unknown, when proof is missing, then the next route starts with `sf-prod` or says the target URL must be supplied.
- Given local implementation is complete but production proof is pending, when `sf-start`, `sf-build`, or `sf-verify` reports the result, then it avoids closure/ship-ready language and routes to the next proof owner.
...
- If a report claims ship-ready or closed while required hosted proof remains missing, `sf-verify` must reject it or mark it partial.
...
- partial `sf-verify` with a concrete hosted-proof route;
...
- `skills/sf-build/SKILL.md`: ensure post-implementation hosted-proof gaps route to the next owner instead of stopping at a descriptive next step.
- `skills/sf-verify/SKILL.md`: require partial proof gaps to name concrete owner routes and scenario targets.
- `skills/sf-start/SKILL.md` only if implementation-complete wording needs a local rule to avoid closure language while proof remains pending.
- `skills/sf-prod/SKILL.md`, `skills/sf-test/SKILL.md`, `skills/sf-browser/SKILL.md`, and `skills/sf-auth-debug/SKILL.md` only if references need a clearer routing vocabulary.
...
- No broad rewrite of every ShipFlow skill report template.
...
- Maintain existing proof-owner boundaries:
  - `sf-prod`: deployment/runtime truth, logs, health, preview/prod readiness.
  - `sf-browser`: non-auth page/browser evidence after a URL exists.
  - `sf-auth-debug`: auth/session/callback/protected-route proof.
  - `sf-test`: durable manual QA, retests, checklist rows, and user-confirmed scenarios.
  - `sf-ship`: commit/push/staging scope before preview-push proof.
- Preserve security redaction rules for hosted logs, tokens, secrets, cookies, private provider payloads, and customer data.

## Test Contract

- `surface`: ShipFlow skill contracts and shared references.
...
- `required_scenario_ids`:
  - `VERIFY-PARTIAL-HOSTED`: `sf-verify partial` from missing hosted proof includes owner route and scenario.
  - `UNKNOWN-DEPLOYMENT-TARGET`: hosted proof cannot proceed until `sf-prod` discovers or confirms the target URL.
  - `LOCAL-COMPLETE-PROD-PENDING`: `sf-start`/`sf-build` avoids closure-sounding wording when production proof is pending.
  - `PREVIEW-PUSH-LADDER`: preview-push validation routes `sf-ship -> sf-prod` before browser/auth/manual proof.
...
- `required_results`:
  ...
  - `sf-verify` contract explicitly distinguishes missing evidence type, proof owner, scenario, and target.
  - `sf-build` contract routes post-implementation hosted-proof gaps through the right proof owner.
...
- Fresh external docs: not needed; this is local ShipFlow workflow/reporting behavior, not external API behavior.
...
- `skills/sf-build/SKILL.md`: master orchestration and downstream reporting behavior.
- `skills/sf-verify/SKILL.md`: partial verification and proof-gap semantics.
- `skills/sf-prod/SKILL.md`: deployment/runtime proof owner.
- `skills/sf-test/SKILL.md`: durable manual QA and checklist proof owner.
- `skills/sf-browser/SKILL.md`: browser proof owner.
- `skills/sf-auth-debug/SKILL.md`: auth/session proof owner.
- `shipflow_data/workflow/conversation-audits/conversation-shipflow-doctrine-de-langue-20260530-203004-audit.md`: source finding set.
...
- Improves `sf-build` and `sf-verify` handoff quality after local implementation but before hosted proof.
- Reduces user friction from ambiguous partial-proof next steps.
- Makes `sf-prod`, `sf-browser`, `sf-auth-debug`, and `sf-test` routing more predictable.
...
- Update `CHANGELOG.md` during `sf-end`/`sf-ship` if skill behavior changes.
- Update `shipflow_data/technical/skill-runtime-and-lifecycle.md` only if it describes proof-owner routing or user report modes affected by this change.
...
- A hosted proof requires a push first; route `sf-ship -> sf-prod` before browser/manual proof.
- A deployment URL is already known; route directly to `sf-prod` or downstream proof owner as appropriate.
...
- [ ] Task 6: Run skill quality checks
  ...
  - User story link: Preserves ShipFlow skill quality and discoverability.
  - Depends on: Tasks 1-5.
  - Validate with:
    - `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/shipflow-hosted-proof-follow-through-and-user-report-discipline.md`
...
- Scenario-first validation:
  ...
  - Re-run or manually map the audited SocialGlowz transcript outcome to the new contract: `sf-verify partial` would route to specific proof owner(s), target environment, and scenario.
- Static validation:
...
- Proof routing can become wrong if it ignores project development mode; preserve `project-development-mode` and `sf-ship -> sf-prod` preview gates.
...
- Source intake from `sf-conversation-audit`:
  ...
  - Severite: P2
  - Scope: `sf-build`, `sf-verify`, reporting contract pressure scenarios, proof-owner routing language.
  - Spec recommandee: `/sf-spec shipflow hosted-proof follow-through and user-report discipline`
- Fresh external docs: not needed; this is local ShipFlow skill/reporting behavior.
- First files to inspect during implementation: `skills/references/reporting-contract.md`, `skills/references/spec-driven-development-discipline.md`, `skills/sf-verify/SKILL.md`, `skills/sf-build/SKILL.md`, `skills/sf-start/SKILL.md`.
- Preserve unrelated dirty changes in the ShipFlow repo.
