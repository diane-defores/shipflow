...
owner: "unknown"
user_story: "As the ShipFlow operator, I want the maintenance and release master skills semantically compacted through the canonical artifact taxonomy, so agents stop confusing maintenance orchestration, bug-loop execution, and release-confidence orchestration."
confidence: "high"
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
...
- If compacting `002-sf-maintain` weakens the distinction between maintenance orchestration and direct owner-lane execution, verification must fail.
- If compacting `003-sf-bug` weakens the distinction between one bug work item and generic maintenance or release orchestration, verification must fail.
- If compacting `004-sf-deploy` weakens the distinction between push success and release proof, verification must fail.
...
The risk is not merely length. It is role collapse. `002-sf-maintain` can start sounding like a global project supervisor for any technical work. `003-sf-bug` can start sounding like a generic defect router instead of a one-bug lifecycle executor. `004-sf-deploy` can start sounding like a simple ship alias or a production-health skill. This batch should compact semantics while preserving the boundaries that protect correct routing, proof quality, and release truth.

# Solution

Run one bounded master-family batch on `002-sf-maintain`, `003-sf-bug`, and `004-sf-deploy`.
...
| --- | --- | --- | --- | --- | --- |
| `skills/002-sf-maintain/SKILL.md` | `master-workflow` | `Quel travail de maintenance doit etre traite pour ce projet, et jusqu'ou peut-on le mener proprement dans cette run ?` | Orchestrate maintenance lanes across bugs, deps, docs, audits, checks, repair, verification, and ship/deploy routing | Becoming a generic build/deploy executor or silently bypassing owner skills | `002-sf-maintain` decides maintenance scope and carries it through the right owner lanes to verified, shippable completion. |
| `skills/003-sf-bug/SKILL.md` | `master-workflow` | `Quel est l'etat reel de ce bug work item, et quelle est la prochaine etape de lifecycle la plus sure ?` | Execute one bug lifecycle through evidence, fix, retest, verify, and ship-risk gates | Becoming generic maintenance triage, direct code-fix ownership, or release orchestration | `003-sf-bug` owns bug-loop state interpretation and continuation, not generic project maintenance or release proof. |
| `skills/004-sf-deploy/SKILL.md` | `master-workflow` | `Quel niveau de confiance release faut-il obtenir pour ce scope, de checks locaux jusqu'a la preuve post-deploy ?` | Orchestrate release checks, ship, deployment truth, post-deploy proof, verification, and changelog routing | Becoming a push-only shortcut, hosted-debug catch-all, or generic prod-health reader | `004-sf-deploy` treats push and deployment as gates toward proof, not as proof themselves. |

# Scope In

- Semantically compact the activation contracts of `002-sf-maintain`, `003-sf-bug`, and `004-sf-deploy`.
...
- checklist_path: `shipflow_data/workflow/test-checklists/semantic-compaction-of-maintenance-and-release-master-skills.md` if implementation requires more than straightforward skill-local edits; otherwise `not required`
...
- required_results:
  ...
  - `002-sf-maintain` keeps owner-lane orchestration explicit and does not collapse into a generic executor.
  - `003-sf-bug` keeps one-bug lifecycle interpretation explicit and does not absorb generic maintenance or deploy proof.
  - `004-sf-deploy` still keeps check -> ship -> hosted truth -> proof -> verify order explicit.
...
- Automated proof available:
  - `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/semantic-compaction-of-maintenance-and-release-master-skills.md`
...
- `002-sf-maintain` remains the project maintenance master, not a direct replacement for `001-sf-build`, `003-sf-bug`, or `004-sf-deploy`.
- `003-sf-bug` remains the bug-loop lifecycle executor for one bug work item and must keep bug-file truth, retest, and ship-risk semantics explicit.
- `004-sf-deploy` remains the release-confidence orchestrator and must keep hosted truth and post-deploy proof distinct from a push.
...
- Update `shipflow_data/technical/skill-runtime-and-lifecycle.md` only if this batch changes the documented distinction between maintenance, bug-loop, and release-confidence masters.
...
- `002-sf-maintain` starts sounding like the default wrapper for any non-trivial technical request.
- `003-sf-bug` starts sounding like a thin command router instead of a lifecycle executor.
- `004-sf-deploy` starts sounding like push/deploy success alone is enough to claim release confidence.
...
- [x] Task 1: Establish the family matrix in the spec.
  - File: `shipflow_data/workflow/specs/semantic-compaction-of-maintenance-and-release-master-skills.md`
  ...
  - Depends on: None.
  - Validate with: `rg -n "Ideal Master Matrix|002-sf-maintain|003-sf-bug|004-sf-deploy" shipflow_data/workflow/specs/semantic-compaction-of-maintenance-and-release-master-skills.md`
  - Notes: This task completes with the spec itself.

- [x] Task 2: Compact the master activation contracts.
  - Files: `skills/002-sf-maintain/SKILL.md`, `skills/003-sf-bug/SKILL.md`, `skills/004-sf-deploy/SKILL.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "Primary artifact type|Mission|Trace category|Process role|Master Delegation|Master Workflow Lifecycle" skills/002-sf-maintain/SKILL.md skills/003-sf-bug/SKILL.md skills/004-sf-deploy/SKILL.md`
...
- [x] Task 4: Decide whether shared-contract or doc follow-up is needed.
  ...
  - Depends on: Task 3.
  - Validate with: `rg -n "shared-contract follow-up|no shared-reference drift|documentation follow-up" shipflow_data/workflow/specs/semantic-compaction-of-maintenance-and-release-master-skills.md`
...
- [x] AC 2: Given `002-sf-maintain`, when an agent routes work, then maintenance orchestration remains distinct from generic build/deploy or direct implementation ownership.
- [x] AC 3: Given `003-sf-bug`, when an agent handles a defect, then one-bug lifecycle state interpretation remains explicit and distinct from generic maintenance.
- [x] AC 4: Given `004-sf-deploy`, when an agent reads the release path, then checks, ship, hosted truth, post-deploy proof, and verify remain visibly ordered.
...
- Read first: `skills/002-sf-maintain/SKILL.md`, `skills/003-sf-bug/SKILL.md`, `skills/004-sf-deploy/SKILL.md`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-12 23:40:00 UTC | 100-sf-spec | GPT-5 Codex | Created a new bounded master-family semantic compaction spec for the D1 batch covering maintenance, bug-loop, and release-confidence skills | drafted | /101-sf-ready shipflow_data/workflow/specs/semantic-compaction-of-maintenance-and-release-master-skills.md |
| 2026-06-12 23:45:00 UTC | 101-sf-ready | GPT-5 Codex | Validated readiness for the D1 master-family batch and approved the bounded skill-local execution scope | ready | /102-sf-start shipflow_data/workflow/specs/semantic-compaction-of-maintenance-and-release-master-skills.md |
