...
scope: feature
owner: Diane
user_story: "En tant qu'operatrice ShipFlow qui cherche souvent une skill precise dans Claude Code ou Codex, je veux un code numerique comprehensible devant chaque skill sans renommer la skill canonique, afin de retrouver rapidement le bon workflow par memoire et par frequence d'usage."
confidence: high
risk_level: medium
...
next_step: "/sf-ship Numeric Skill Code Index"
---
...
En tant qu'operatrice ShipFlow qui cherche souvent une skill precise dans Claude Code ou Codex, je veux un code numerique comprehensible devant chaque skill sans renommer la skill canonique, afin de retrouver rapidement le bon workflow par memoire et par frequence d'usage.

## Minimal Behavior Contract

ShipFlow doit exposer un index canonique `code -> skill` ou les codes sont faciles a memoriser: les codes courts `00-09` couvrent les skills master et les plus frequentes, puis les familles `10`, `20`, `30`, etc. groupent execution, contenu, docs, audit, design, data, pilotage et helpers. Les dossiers de skills, les champs `name:` et les commandes canoniques ne changent pas. Le routeur `shipflow` et l'aide doivent pouvoir expliquer qu'un utilisateur peut demander `01`, `01-sf-build` ou `01sfbuild` comme raccourci de routage vers `sf-build`. Le cas facile a rater est de creer de vrais doublons runtime ou de renommer les skills, ce qui casserait la compatibilite.

## Success Behavior

- Preconditions: les skills ShipFlow existent sous `skills/<name>/SKILL.md`.
- Trigger: l'operatrice consulte l'aide ou donne un code numerique au routeur ShipFlow.
...
La liste de skills est longue et certains noms sont proches. Chercher `sf-audit-copywriting`, `sf-local-cloud-sync` ou `sf-conversation-audit` demande trop de frappe et de memoire. Les alias natifs de runtime ne sont pas garantis de la meme maniere entre Claude Code et Codex, donc un renommage ou des wrappers dupliques creeraient de la confusion.

## Solution

Ajouter une couche canonique de decouverte numerique dans `skills/references/skill-code-index.md`, garder tous les noms de skills inchanges, puis documenter la resolution dans `shipflow`, `sf-help`, la cheatsheet et la doc technique. Ajouter un linter standard-library Python pour valider que l'index reste synchronise avec `skills/`.
...
- Downstream systems: `shipflow` et `sf-help` consomment l'index pour orienter l'operatrice.
...
- `skills/sf-help/SKILL.md` et `skills/sf-help/references/help-catalog.md` doivent pointer vers l'index.
- `docs/skill-launch-cheatsheet.md` doit resumer les familles numeriques.
- `shipflow_data/technical/skill-runtime-and-lifecycle.md` et `code-docs-map.md` doivent documenter la surface.
- `CHANGELOG.md` doit mentionner la nouvelle couche de decouverte.

## Edge Cases

- `01` seul doit etre resolvable par contexte vers `sf-build`.
- `01-sf-build` et `01sfbuild` doivent etre compris comme le meme code.
- Un code avec un nom incoherent, par exemple `01-sf-docs`, doit privilegier le code et signaler la mismatch si une implementation stricte est ajoutee plus tard.
...
- [x] Task 1: Creer la spec prete
  - File: `shipflow_data/workflow/specs/numeric-skill-code-index.md`
  ...
  - Depends on: None
  - Validate with: `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/numeric-skill-code-index.md`
  - Notes: None
...
- [x] Task 4: Mettre a jour le routeur et l'aide
  - File: `skills/shipflow/SKILL.md`, `skills/sf-help/SKILL.md`, `skills/sf-help/references/help-catalog.md`
  ...
  - Validate with: `rg -n "skill-code-index|numeric skill code|01-sf-build|00-shipflow" skills/shipflow/SKILL.md skills/sf-help/SKILL.md skills/sf-help/references/help-catalog.md`
  - Notes: Ne pas dupliquer toute la table dans `SKILL.md`.

- [x] Task 5: Mettre a jour les docs techniques et la cheatsheet
  - File: `docs/skill-launch-cheatsheet.md`, `shipflow_data/technical/skill-runtime-and-lifecycle.md`, `shipflow_data/technical/code-docs-map.md`, `CHANGELOG.md`
  ...
  - Validate with: `rg -n "skill-code-index|skill_code_index_lint|00-shipflow|01-sf-build" docs/skill-launch-cheatsheet.md shipflow_data/technical/skill-runtime-and-lifecycle.md shipflow_data/technical/code-docs-map.md CHANGELOG.md`
...
- [x] AC 2: Given le code `01`, `01-sf-build`, ou `01sfbuild`, when le routeur ShipFlow interprete la demande, then la skill canonique cible est `sf-build`.
...
- Integration: `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/numeric-skill-code-index.md skills/references/skill-code-index.md shipflow_data/technical/skill-runtime-and-lifecycle.md shipflow_data/technical/code-docs-map.md`.
...
- Needed: no
- Checklist path: `shipflow_data/workflow/test-checklists/numeric-skill-code-index.md`
...
- Read first: `skills/references/skill-code-index.md` once created, `skills/shipflow/SKILL.md`, `skills/sf-help/SKILL.md`, `docs/skill-launch-cheatsheet.md`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-10 08:09:02 UTC | sf-build | GPT-5 Codex | Created ready spec for numeric skill code index | implemented | /sf-start Numeric Skill Code Index |
| 2026-06-10 08:09:02 UTC | sf-build | GPT-5 Codex | Implemented numeric skill-code index, router/help/docs wiring, and linter validation | implemented | /sf-end Numeric Skill Code Index |
| 2026-06-10 11:04:06 UTC | sf-end | GPT-5 Codex | Closed the chantier after verified implementation, changelog entry, and task tracking alignment | closed | /sf-ship Numeric Skill Code Index |

## Current Chantier Flow

- `sf-spec`: done, ready spec created from user decision.
- `sf-ready`: accepted by direct user approval and bounded ready contract.
- `sf-start`: done, index, linter, router/help/docs updates implemented.
- `sf-verify`: done, focused checks passed.
- `sf-end`: done, chantier closed.
- `sf-ship`: next.

Next step: `/sf-ship Numeric Skill Code Index`
