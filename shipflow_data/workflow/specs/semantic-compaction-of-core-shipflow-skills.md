...
owner: "unknown"
user_story: "As the ShipFlow operator, I want the core ShipFlow skills semantically compacted through the canonical artifact taxonomy, so agents keep the right responsibilities in memory without confusing routers, masters, specialists, contracts, references, templates, and records."
confidence: "high"
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
...
Semantic Compaction Of Core ShipFlow Skills
...
As the ShipFlow operator, I want the core ShipFlow skills semantically compacted through the canonical artifact taxonomy, so agents keep the right responsibilities in memory without confusing routers, masters, specialists, contracts, references, templates, and records.

# Minimal Behavior Contract

When this chantier runs, ShipFlow must reduce semantic confusion in core skill instructions by making every touched `SKILL.md` state one primary artifact role, one dominant responsibility, one owner boundary, and one proof/reporting contract, while moving misplaced reusable doctrine into contracts or references. The work must start with a sequential pilot before any broad rewrite. Parallel execution is allowed only after the pilot proves the method and a ready spec defines non-overlapping `Execution Batches`. If a change makes a skill shorter but less precise about ownership, lifecycle state, proof, security, or delegation, verification must fail. The easiest edge case to miss is compacting several lifecycle skills at once and accidentally making `000-shipflow`, `001-sf-build`, `102-sf-start`, and `103-sf-verify` sound interchangeable.
...
- User/operator result: core ShipFlow skills become easier to reason about: router skills route, master skills orchestrate, specialist skills execute, contracts define rules, references support application, templates shape artifacts, and records trace cases.
...
ShipFlow has already reduced several forms of instruction bloat: long bodies were layered, descriptions were compacted, and a canonical artifact taxonomy now exists. The remaining problem is semantic density and role confusion inside the most important skill surfaces.
...
1. Run a **sequential pilot** on a representative vertical slice: `000-shipflow`, `001-sf-build`, and `102-sf-start`.
...
| `skills/001-sf-build/SKILL.md` | `master-workflow` | Orchestrate the full lifecycle from readiness to closure handoff | Router-only behavior, specialist execution detail, standalone closure claims | `001-sf-build` remains the lifecycle orchestrator through closure handoff. |
| `skills/102-sf-start/SKILL.md` | `specialist-workflow` | Implement ready specs and run local, non-destructive verification when eligible | Master-level orchestration, trace semantics, shipping and lifecycle closure sequencing | `102-sf-start` signals `implemented` when implementation is complete and routes missing proof to the right owner. |

# Scope In

- Semantically compact the core ShipFlow skill activation contracts using the canonical artifact taxonomy.
...
- Parallel execution is allowed only after `/101-sf-ready` approves explicit `Execution Batches`.
...
- Surface profile: ShipFlow governance, skill activation contracts, shared references, and lifecycle docs.
...
- Manual checklist expected if more than the pilot is implemented: `shipflow_data/workflow/test-checklists/semantic-compaction-of-core-shipflow-skills.md`
- Fresh external docs verdict: `fresh-docs not needed`, because this chantier changes local ShipFlow instruction architecture, not provider behavior.

# Dependencies

- `shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`: canonical artifact role taxonomy and anti-pattern matrix.
...
- `shipflow_data/technical/skill-runtime-and-lifecycle.md`: docs surface for runtime skill role alignment.
...
- `001-sf-build` remains a `master-workflow`, not a specialist executor or pure router.
- `100-sf-spec`, `101-sf-ready`, `102-sf-start`, `103-sf-verify`, `104-sf-end`, and `005-sf-ship` keep their lifecycle source-of-truth roles.
- `102-sf-start` implementation-complete semantics remain distinct from `103-sf-verify` proof completeness.
- `104-sf-end` closure remains distinct from `005-sf-ship` commit/push.
...
- Update `shipflow_data/technical/skill-runtime-and-lifecycle.md` if core role boundaries change materially.
...
- Changelog and task tracker updates belong to `104-sf-end` / `005-sf-ship`, not this spec.

# Edge Cases

- A shorter `000-shipflow` starts sounding like it may execute lifecycle work itself.
- A shorter `001-sf-build` loses its obligation to continue through `104-sf-end` and `005-sf-ship`.
- A shorter `102-sf-start` collapses `implemented` into `verified`.
- A shorter `103-sf-verify` stops naming the owner route for missing proof.
...
- [x] Task 1: Build the pilot role matrix.
  - File: `shipflow_data/workflow/specs/semantic-compaction-of-core-shipflow-skills.md`
  - Action: Add a pilot analysis table for `000-shipflow`, `001-sf-build`, and `102-sf-start`: primary artifact type, dominant question, allowed content, forbidden content, must-preserve sentences, candidate extraction targets.
  - User story link: Establishes the method before editing skill text.
  - Depends on: None.
  - Validate with: `rg -n "Pilot Role Matrix|000-shipflow|001-sf-build|102-sf-start|must-preserve" shipflow_data/workflow/specs/semantic-compaction-of-core-shipflow-skills.md`
  - Notes: This task must complete before any skill body edit.

- [x] Task 2: Run the sequential pilot edits.
  - Files: `skills/000-shipflow/SKILL.md`, `skills/001-sf-build/SKILL.md`, `skills/102-sf-start/SKILL.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "entrypoint-router|master-workflow|specialist-workflow|Trace category|Process role|Result semantics|Auto-verify semantics" skills/000-shipflow/SKILL.md skills/001-sf-build/SKILL.md skills/102-sf-start/SKILL.md`
  - Notes: Execute sequentially, not in parallel.

- [x] Task 3: Verify the pilot before expanding.
  - Files: `skills/000-shipflow/SKILL.md`, `skills/001-sf-build/SKILL.md`, `skills/102-sf-start/SKILL.md`, changed references/docs if any.
...
- [ ] Task 7: Final integration and proof.
  ...
  - Notes: Do not close or ship until `103-sf-verify` passes.
...
- [ ] AC 2: Given `001-sf-build` is read after compaction, when an agent evaluates it, then it can tell it orchestrates the full lifecycle through closure and ship routing.
- [ ] AC 3: Given `102-sf-start` is read after compaction, when local implementation is complete but hosted/manual proof is missing, then its result semantics still produce `implemented` and route proof to the right owner rather than `partial`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-12 12:51:11 UTC | 100-sf-spec | GPT-5 Codex | Created draft spec for semantic compaction of core ShipFlow skills with mandatory sequential pilot and gated parallel batch strategy | draft | /101-sf-ready shipflow_data/workflow/specs/semantic-compaction-of-core-shipflow-skills.md |
| 2026-06-12 13:30:39 UTC | 101-sf-ready | gpt-5.5 | Validated readiness for pilot-first semantic compaction and recorded mandatory Pilot Role Matrix requirements | ready | /102-sf-start shipflow_data/workflow/specs/semantic-compaction-of-core-shipflow-skills.md |
| 2026-06-12 13:40:02 UTC | 102-sf-start | gpt-5 Codex | Executed sequential pilot compaction across core entrypoint/master/specialist workflow skills | implemented | /103-sf-verify shipflow_data/workflow/specs/semantic-compaction-of-core-shipflow-skills.md |
