...
owner: "Diane"
confidence: high
user_story: "En tant qu'opératrice ShipFlow, je veux auditer chaque projet produit du serveur contre la doctrine design-system authority, afin d'empêcher les agents de produire des UI incohérentes ou des tokens locaux hors source canonique."
risk_level: high
...
supersedes: []
evidence:
  - "2026-06-11 server scan found UI/app signals in contentglowz, gocharbon, replayglowz, shipflow_app, socialglowz, temu, and winflowz."
  - "2026-06-11 server scan found design-system authority already present in shipflow and temu."
  - "2026-06-11 server scan found missing design-system authority in contentglowz, gocharbon, replayglowz, shipflow_app, socialglowz, and winflowz."
...
next_step: "/300-sf-docs technical design-system-authority contentglowz"
---
...
En tant qu'opératrice ShipFlow, je veux auditer chaque projet produit du serveur contre la doctrine design-system authority, afin d'empêcher les agents de produire des UI incohérentes ou des tokens locaux hors source canonique.
...
Given a project is missing `shipflow_data/technical/design-system-authority.md`, when it has UI surfaces, then the audit classifies it as a governance gap and routes remediation to `300-sf-docs` and `006-sf-design` before visual implementation work.
...
ShipFlow now has a central design-system token doctrine, but the product repos on the server were created before this doctrine was formalized. Agents can still enter those repos and produce local UI changes unless each project declares a canonical source for colors, typography, spacing, shadows, motion, component variants, layout constants, and mobile/app behavior.

## Solution

Run a staged cross-project audit. First produce a read-only inventory and compliance matrix. Then, project by project, create or update the project-local `shipflow_data/technical/design-system-authority.md` and route deeper token/component audits only where the authority exists or is intentionally created.

## Scope In

- Audit these detected product/project repos: `contentglowz`, `gocharbon`, `replayglowz`, `shipflow_app`, `socialglowz`, `temu`, and `winflowz`.
...
- Check for canonical authority at `shipflow_data/technical/design-system-authority.md`, with monorepo root handling.
- Identify likely token/theme/component authority candidates.
- Run broad drift scans only in warn-only/read-only mode.
- Produce a central audit report under `shipflow_data/workflow/audits/`.
...
- Project authority defaults to `shipflow_data/technical/design-system-authority.md`.
...
- `shipflow_data/technical/design-system-authority.md` as the ShipFlow reference authority model.
...
- Project-local `shipflow_data/`, `CLAUDE.md`, package/config files, theme/token files, and existing specs.
...
Upstream:
- ShipFlow design-system token doctrine and project governance layout.
- Existing product app tokens/themes/components.

Downstream:
- `300-sf-docs` creates or updates project authority docs.
- `006-sf-design` or `500-sf-design-from-scratch` handles projects without a coherent token system.
- `503-sf-audit-design-tokens` and `504-sf-audit-components` handle deeper health checks.
- `103-sf-verify` checks whether the inventory and remediation routes satisfy this spec.
- `005-sf-ship` ships only bounded changes in the active repo.

## Documentation Coherence

The first audit creates or updates a central ShipFlow workflow audit report. It does not alter project docs unless a project-specific remediation task is started. Each project that lacks authority should later receive a local `shipflow_data/technical/design-system-authority.md` or an explicit documented exception.

## Edge Cases

- `shipflow_app` stores governance under `app/shipflow_data` signals, but the repo root has the git root; the audit must identify whether this is a standalone exception or migration debt.
...
- [x] Task 1: Create the ready spec.
  - File: `shipflow_data/workflow/specs/audit-design-system-authority-all-projects.md`
  - Action: Capture scope, project set, gates, proof path, and remediation route.
  - Validate with: `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/audit-design-system-authority-all-projects.md`

- [x] Task 2: Run read-only project inventory.
  - File: `shipflow_data/workflow/audits/2026-06-11-design-system-authority-all-projects.md`
  - Action: Record each project, UI evidence, authority status, token/theme candidates, dirty-state note, and remediation priority.
  - Validate with: `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/audits/2026-06-11-design-system-authority-all-projects.md`
...
- [x] Task 5: Verify ShipFlow audit artifacts.
  - Files: this spec and the central audit report.
  - Action: Run metadata lint, drift-check script smoke, focused report coverage checks, and `103-sf-verify`.
...
- `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/audit-design-system-authority-all-projects.md`
...
- `git status --short` before and after inventory to prove only ShipFlow audit artifacts changed.
- Focused project discovery command listing `.git`, `shipflow_data`, UI markers, authority files, and token/theme candidates.
- `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/audits/2026-06-11-design-system-authority-all-projects.md`
...
- Monorepo governance ambiguity: mitigated by explicitly classifying nested `shipflow_data` as migration debt unless documented.
...
- each project `shipflow_data/technical/code-docs-map.md` when present

Recommended route:
1. `101-sf-ready` on this spec.
2. `102-sf-start` for the read-only inventory report in ShipFlow.
3. `103-sf-verify` to check the inventory against acceptance criteria.
4. Project-specific `300-sf-docs`/`006-sf-design` chantiers for remediation.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-11 14:12:23 UTC | 100-sf-spec | GPT-5 Codex | Created cross-project design-system authority audit spec from the server scan and operator approval to proceed. | draft spec created | `/101-sf-ready audit-design-system-authority-all-projects` |
| 2026-06-11 14:14:20 UTC | 101-sf-ready | GPT-5 Codex | Validated structure, user story fit, design-system authority contract, read-only proof path, documentation coherence, and security scope. | ready | `/102-sf-start audit-design-system-authority-all-projects` |
| 2026-06-11 14:16:25 UTC | 102-sf-start | GPT-5 Codex | Created the read-only cross-project authority inventory report and ran warn-only drift scans for the detected UI/app projects. | implemented | `/103-sf-verify audit-design-system-authority-all-projects` |
| 2026-06-11 14:17:50 UTC | 103-sf-verify | GPT-5 Codex | Verified metadata, report coverage, changed-file drift check, diff check, evidence-first proof path, and project-by-project remediation routing. | verified | `/300-sf-docs technical design-system-authority contentglowz` |

## Current Chantier Flow

```text
100-sf-spec: done (draft created)
101-sf-ready: done (ready)
102-sf-start: done (implemented; auto-verify skipped because 001-sf-build owns lifecycle verification)
103-sf-verify: done (verified for inventory scope)
104-sf-end: not needed for this continuing audit sequence
005-sf-ship: pending if the operator wants to ship the audit artifacts before project remediation
```
