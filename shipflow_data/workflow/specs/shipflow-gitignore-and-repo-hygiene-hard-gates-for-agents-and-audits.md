...
owner: "Diane"
user_story: "En tant qu'operatrice ShipFlow, je veux que les agents et audits bloquent mecaniquement les artefacts locaux, secrets et sorties transitoires mal ignores avant implementation, checks ou ship, afin d'eviter les commits sales, les faux signaux de verification, et la fuite de fichiers locaux."
confidence: high
risk_level: high
security_impact: "yes"
docs_impact: "yes"
linked_systems:
  ...
  - "skills/401-sf-audit-code/references/audit-workflow.md"
...
supersedes: []
evidence:
  - "Current ShipFlow worktree on 2026-06-18 shows `.gitignore` modified while tracked transient artifacts under `site/.playwright-mcp/` are deleted, proving repo-hygiene drift can coexist with ordinary work."
  - "401-sf-audit-code workflow currently checks only `.env` presence in `.gitignore`, which is too narrow for generated local-tool outputs and tracked transient artifacts."
  - "005-sf-ship currently runs a secret pre-stage check but has no dedicated hard mechanical gate for broad `.gitignore`/transient-artifact hygiene."
  - "102-sf-start and 105-sf-check currently rely on general guardrails and check interpretation, not a shared machine-readable repo-hygiene verdict."
next_step: "/101-sf-ready shipflow_data/workflow/specs/shipflow-gitignore-and-repo-hygiene-hard-gates-for-agents-and-audits.md"
---

## Title

ShipFlow Gitignore and Repo Hygiene Hard Gates for Agents and Audits

## Status

Draft. Intended to be implementation-ready after `/101-sf-ready` without widening scope beyond the named skills, one mechanical tool, and scenario-first audit coverage.

## User Story

En tant qu'opératrice ShipFlow, je veux que les skills d'exécution, de checks, de ship et d'audit détectent puis bloquent mécaniquement les artefacts locaux, secrets et sorties transitoires mal ignorées avant qu'un agent implémente, valide ou pousse, afin d'éviter les commits pollués, les faux verdicts "green", et les fuites de fichiers locaux.

Acteur principal: opératrice ShipFlow qui délègue à `102-sf-start`, `105-sf-check`, `005-sf-ship`, `900-shipflow-core`, `400-sf-audit`, ou `401-sf-audit-code`.
...
Avant toute implémentation, check, audit de conformité, ou ship, ShipFlow doit exécuter un contrôle mécanique de repo hygiene qui détecte les artefacts locaux/secrets/transitoires mal ignorés ou déjà suivis, puis bloque la progression dans `102-sf-start`, `105-sf-check`, et `005-sf-ship` tant que l'état n'est pas assaini ou explicitement inclus dans le scope de la spec. L'edge case facile à rater est un repo techniquement "fonctionnel" dont les checks passent alors que des sorties locales comme `.playwright-mcp`, `.codebase-mcp`, `.env`, caches Python ou autres artefacts de machine polluent l'index, masquent un vrai problème `.gitignore`, ou risquent une fuite au push.

## Success Behavior

- Given un repo avec artefacts transitoires suivis ou non ignorés, when `102-sf-start` initialise son exécution, then il stoppe avant toute modification non triviale et route vers une remédiation d'hygiène ou vers une spec existante qui possède explicitement ce nettoyage.
- Given un repo avec règles `.gitignore` manquantes pour des sorties locales connues, when `105-sf-check` lance son pass de confiance, then il renvoie un verdict bloquant avec la liste des patterns/path concernés avant d'interpréter le repo comme "green".
- Given un repo sale avant commit/push, when `005-sf-ship` prépare le stage, then il bloque le ship même si `skip-check` est demandé, car la gate d'hygiène fait partie des préconditions de sécurité et de confiance, pas des checks optionnels.
- Given un audit interne ShipFlow, when `900-shipflow-core` ou `401-sf-audit-code` inspecte les skills/workflows, then l'audit produit un finding scenario-first si un skill lifecycle manque la gate mécanique ou si le workflow d'audit reste limité au seul check `.env`.
...
La preuve locale actuelle confirme le problème: la worktree contient une modification de `.gitignore` et des suppressions de fichiers `site/.playwright-mcp/*.yml`, signe qu'un outillage local a déjà produit des artefacts suivis ou mal nettoyés. En parallèle, `401-sf-audit-code` ne vérifie mécaniquement que `.env` dans `.gitignore`, ce qui est trop étroit.

## Solution

Introduire une gate mécanique unique `tools/audit_gitignore_hygiene.py`, puis l'imposer explicitement dans `102-sf-start`, `105-sf-check`, et `005-sf-ship`. Le tool doit produire un verdict structuré et conservateur (`clean`, `hard`, `review`, `style`) à partir de l'état Git, de `.gitignore`, et d'un jeu minimal de règles ShipFlow pour artefacts locaux/transitoires/sensibles. Les skills lifecycle traitent `hard` comme bloquant, `review` comme finding à reporter ou à auditer selon le contexte, et `style` comme non bloquant.

En parallèle, étendre la couverture scenario-first dans `900-shipflow-core`, `400-sf-audit`, `401-sf-audit-code`, et le workflow de `401-sf-audit-code` pour que l'absence de gate, le scope trop étroit des checks `.gitignore`, ou le passage abusif d'un repo sale soient auditables et vérifiables.
...
- Ajouter une gate bloquante dans `skills/102-sf-start/SKILL.md` avant implémentation significative.
- Ajouter une gate bloquante dans `skills/105-sf-check/SKILL.md` avant de déclarer un pass de checks techniquement fiable.
- Ajouter une gate bloquante dans `skills/005-sf-ship/SKILL.md` avant stage/commit/push, non contournable via `skip-check`.
...
- Étendre `skills/400-sf-audit/SKILL.md`, `skills/401-sf-audit-code/SKILL.md`, et `skills/401-sf-audit-code/references/audit-workflow.md` pour traiter repo hygiene / `.gitignore` comme axe d'audit explicite, pas comme sous-ligne `.env` isolée.
...
- Aucun nettoyage automatique global de tous les repos ShipFlow.
- Aucune modification automatique de `.gitignore` par le tool lui-même.
- Aucun élargissement de scope vers `310-sf-github-hygiene`; cette spec traite l'hygiène repo locale et la gate agentique, pas la dérive GitHub/branches.
...
- Aucun changement aux projets applicatifs externes; la première implémentation vise la discipline ShipFlow et ses audits.
...
- La gate de `005-sf-ship` doit rester active même en mode `skip-check`, car elle protège le stage/push lui-même.
- Fresh external docs: not needed; le comportement visé est local à ShipFlow, Git, et nos skills.
...
- Lifecycle contract:
  - `102-sf-start`, `105-sf-check`, `005-sf-ship` doivent arrêter ou rerouter sur exit `2` ou `3`.
  - `skip-check` ne contourne pas cette gate dans `005-sf-ship`.
...
- `surface`: ShipFlow skills, audit workflows, and one local Python helper.
...
- `checklist_path`: `shipflow_data/workflow/test-checklists/gitignore-repo-hygiene.md`
...
- `required_results`:
  - `102-sf-start` refuses non-owned implementation when `hard` hygiene findings exist.
  - `105-sf-check` cannot report green technical confidence with `hard` hygiene findings.
  - `005-sf-ship` cannot stage/push with `hard` hygiene findings, even with `skip-check`.
...
- ShipFlow ne doit jamais présenter un repo sale/transitoire comme "green" sans au minimum un finding explicite.
...
- Étend `401-sf-audit-code` au-delà de la ligne `.env` dans `.gitignore`.
...
- Mettre à jour les skills ciblés et, si nécessaire, `shipflow_data/technical/skill-runtime-and-lifecycle.md` si ce document décrit les gates lifecycle/check/ship sans mention d'hygiène repo mécanique.
- Mettre à jour `CHANGELOG.md` pendant `104-sf-end`/`005-sf-ship` si le comportement des skills change matériellement.
- Aucun doc externe/public requis tant que le changement reste interne au fonctionnement ShipFlow.
...
- [ ] Task 2: Add the start hard gate
  - File: `skills/102-sf-start/SKILL.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "audit_gitignore_hygiene|repo hygiene|gitignore|hard gate|scoped exception" skills/102-sf-start/SKILL.md`

- [ ] Task 3: Add the check hard gate
  - File: `skills/105-sf-check/SKILL.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "audit_gitignore_hygiene|repo hygiene|gitignore|green" skills/105-sf-check/SKILL.md`

- [ ] Task 4: Add the ship hard gate
  - File: `skills/005-sf-ship/SKILL.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "audit_gitignore_hygiene|skip-check|git add|hard gate|repo hygiene" skills/005-sf-ship/SKILL.md`

- [ ] Task 5: Add scenario-first audit coverage to ShipFlow core
...
- [ ] Task 6: Extend audit entrypoints and code-audit workflow
  - File: `skills/400-sf-audit/SKILL.md`
  ...
  - Validate with: `rg -n "gitignore|repo hygiene|hygiene gate" skills/400-sf-audit/SKILL.md`

- [ ] Task 7: Extend code-audit skill and checklist
  - File: `skills/401-sf-audit-code/SKILL.md`
  ...
  - Depends on: Tasks 2-5.
  - Validate with: `rg -n "gitignore|repo hygiene|transient|secret" skills/401-sf-audit-code/SKILL.md`

- [ ] Task 8: Replace env-only checklist coverage with scenario-first repo-hygiene coverage
  - File: `skills/401-sf-audit-code/references/audit-workflow.md`
  ...
  - Depends on: Task 7.
  - Validate with: `rg -n "\\.gitignore|repo hygiene|tracked transient|GIH-" skills/401-sf-audit-code/references/audit-workflow.md`

- [ ] Task 9: Integrate the mechanical checker into ShipFlow skill auditing
...
- [ ] Task 10: Add the manual scenario checklist artifact
  - File: `shipflow_data/workflow/test-checklists/gitignore-repo-hygiene.md`
  - Action: Record scenario IDs, setup, expected verdicts, and proof capture order for `103-sf-verify`.
  - User story link: Makes verification deterministic for a fresh agent.
  - Depends on: Tasks 1-9.
  - Validate with: `test -f shipflow_data/workflow/test-checklists/gitignore-repo-hygiene.md`

## Acceptance Criteria

- [ ] AC 1: Given a repo with tracked `site/.playwright-mcp/*.yml` or equivalent transient outputs, when `005-sf-ship` runs, then it stops before `git add` and reports a `hard` hygiene failure with the offending paths.
- [ ] AC 2: Given a repo with `.env` or other sensitive local credential files not ignored, when `102-sf-start`, `105-sf-check`, or `005-sf-ship` runs, then the skill blocks and does not claim success/green/ship.
- [ ] AC 3: Given a repo with missing ignore patterns for known local-tool outputs, when `105-sf-check` runs, then it cannot finish with a clean technical-confidence verdict until the hygiene issue is resolved or explicitly owned by the chantier.
- [ ] AC 4: Given a spec whose scope is specifically repo-hygiene cleanup, when `102-sf-start` runs, then it may continue despite the dirty hygiene surface, but only after explicitly naming that owned exception in the execution/report contract.
- [ ] AC 5: Given `skip-check` in `005-sf-ship`, when `hard` hygiene findings exist, then ship is still blocked because hygiene gating is not an optional check lane.
...
- [ ] AC 7: Given `401-sf-audit-code` runs on a project with unsafe transient or secret local outputs, then the audit can report a finding even when `.env` is not the only issue.
...
- Manual proof through `shipflow_data/workflow/test-checklists/gitignore-repo-hygiene.md` for the blocker paths and scoped exception path.
- Regression check that `005-sf-ship skip-check` still enforces hygiene.
...
2. Wire `102-sf-start`, `105-sf-check`, and `005-sf-ship` to the tool and ship the blocker semantics first.
3. Extend `900-shipflow-core`, `400-sf-audit`, `401-sf-audit-code`, and the audit workflow/checklist with `GIH-*` scenarios.
4. Verify with `103-sf-verify` using the dedicated checklist and meta-audit.
5. Close with `104-sf-end` only after runtime docs/changelog coherence is either updated or explicitly marked not needed.
...
rg -n "audit_gitignore_hygiene|repo hygiene|gitignore" skills/102-sf-start/SKILL.md skills/105-sf-check/SKILL.md skills/005-sf-ship/SKILL.md skills/900-shipflow-core/SKILL.md skills/400-sf-audit/SKILL.md skills/401-sf-audit-code/SKILL.md skills/401-sf-audit-code/references/audit-workflow.md
```

## Execution Notes

- Read first: `skills/102-sf-start/SKILL.md`, `skills/105-sf-check/SKILL.md`, `skills/005-sf-ship/SKILL.md`, `skills/900-shipflow-core/SKILL.md`, `skills/401-sf-audit-code/references/audit-workflow.md`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-18 21:13:04 UTC | 100-sf-spec | GPT-5 Codex | Created a ready-to-implement spec for hard `.gitignore` and repo-hygiene gates across lifecycle skills, a mechanical checker, and scenario-first audit coverage | drafted | /101-sf-ready shipflow_data/workflow/specs/shipflow-gitignore-and-repo-hygiene-hard-gates-for-agents-and-audits.md |
...
- 104-sf-end: pending
- 005-sf-ship: pending
- Next step: `/101-sf-ready shipflow_data/workflow/specs/shipflow-gitignore-and-repo-hygiene-hard-gates-for-agents-and-audits.md`
