...
owner: "unknown"
user_story: "As the ShipFlow operator, I want a complete but simple taxonomy for skills, references, templates, and records, so we can reduce confusion first at the architecture level and only then compact wording safely."
confidence: "high"
risk_level: "high"
...
linked_systems:
  ...
  - "shipflow_data/workflow/specs/audit-and-compact-skill-taxonomy-descriptions.md"
  - "shipflow_data/workflow/specs/skill-taxonomy-and-chantier-sources.md"
depends_on:
...
next_step: "/005-sf-ship shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md"
---

# Title

ShipFlow Artifact Taxonomy For Skills And References
...
As the ShipFlow operator, I want a complete but simple taxonomy for skills, references, templates, and records, so we can reduce confusion first at the architecture level and only then compact wording safely.

# Minimal Behavior Contract

When this chantier runs, ShipFlow must define one canonical taxonomy for the artifacts that govern skill behavior and project guidance: `entrypoint-router`, `master-workflow`, `specialist-workflow`, `contract`, `reference`, `template`, and `record`. For each type, the system must state its question centrale, unit of value, allowed content, forbidden content, and expected output, then map existing ShipFlow artifacts to one primary type only. If a file materially performs two jobs, the taxonomy must require extraction or split instead of accepting ambiguity. The easiest edge case to miss is treating every router-like skill the same, which collapses the distinction between choosing a workflow and orchestrating a chantier.
...
- User/operator result: the operator gets a canonical artifact taxonomy, an ideal responsibility matrix, a short classification rubric, and a concrete mapping of current ShipFlow files to their primary type.
...
- If a proposed type is too abstract to classify existing ShipFlow files quickly, the run must reject it or tighten the matrix.
...
- If the resulting taxonomy cannot distinguish `000-shipflow` from `001-sf-build`, or a shared rule from a support map, verification fails.
...
For each type, document its dominant question, responsibility boundary, legal/illegal content, and expected output. Then create a matrix that classifies representative ShipFlow artifacts and exposes where current files are structurally mixed. Use that matrix to drive future extraction, compaction, and documentation updates.

# Canonical Artifact Taxonomy

ShipFlow documents that govern behavior should resolve to exactly one primary artifact type below.
...
| --- | --- | --- | --- |
| Router vs orchestrator confusion | A router file contains workflow commands and trace updates | `entrypoint-router` starts running `103-sf-verify` logic itself | Extract orchestration into `master-workflow` or specialist links; keep router focused to handoff |
...
| `skills/001-sf-build/SKILL.md` | `master-workflow` | Owns delegated execution with readiness + verification + ship handoff | 
| `skills/100-sf-spec/SKILL.md` | `master-workflow` | Owns the spec intake and reading gate for durable work |
| `skills/references/canonical-paths.md` | `contract` | Defines mandatory path-resolution rule and exception policy |
| `shipflow_data/technical/context.md` | `reference` | Context map used to orient local decisions and routing |
| `templates/artifacts/spec.md` | `template` | Artifact structure for durable spec creation |
| `shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md` | `record` | Durable result and mapping for this chantier |

## Structural Debt and Extraction Targets

- `skills/102-sf-start/SKILL.md`: mixed `SKILL.md` contract and multi-page execution playbook; low-risk target remains `execution-workflow.md`, but the local contract still embeds long model/topology prose. Recommendation: keep as is for this chantier and plan a future `102-sf-start` cleanup follow-up only if execution semantics change.
...
- Define the canonical seven-type artifact taxonomy for ShipFlow skill and reference architecture.
...
- Update the architecture and lifecycle docs that explain ShipFlow artifact structure if implementation changes are accepted.
...
- The taxonomy must preserve existing distinctions already surfaced in ShipFlow, especially router versus orchestrator versus specialist, and contract versus support reference.
...
- Manual checklist expected if the chantier updates many files or the wording remains debatable: `shipflow_data/workflow/test-checklists/shipflow-artifact-taxonomy-for-skills-and-references.md`
- Exception with proof: no browser, auth, provider, or device proof is required because the change is local to ShipFlow governance artifacts.
...
- Related prior chantiers:
  ...
  - `shipflow_data/workflow/specs/audit-and-compact-skill-taxonomy-descriptions.md`
  - `shipflow_data/workflow/specs/skill-taxonomy-and-chantier-sources.md`
- Fresh external docs verdict: `fresh-docs not needed`, because this chantier defines local ShipFlow artifact architecture rather than framework or provider behavior.
...
- Update `shipflow_data/technical/skill-runtime-and-lifecycle.md` with the canonical taxonomy matrix.
...
- [x] Task 1: Write the canonical taxonomy definition.
  - File: `shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
  ...
  - Depends on: None
  - Validate with: `rg -n "entrypoint-router|master-workflow|specialist-workflow|contract|reference|template|record" shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
...
- [x] Task 2: Add the ideal artifact matrix.
  ...
  - Depends on: Task 1
  - Validate with: `rg -n "Question centrale|Unite de valeur|Horizon|Contenu legitime|Contenu illegitime|Verbe dominant|Sortie attendue" shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
...
- [x] Task 3: Add the boundary and anti-pattern matrix.
  ...
  - Depends on: Task 2
  - Validate with: `rg -n "anti-pattern|contamination|router|orchestrator|drift|split|extraction" shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
  - Notes: The output should prefer split/extraction recommendations over tolerant ambiguity.

- [x] Task 4: Map representative ShipFlow artifacts to primary types.
  - File: `shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
  - Action: Classify representative files such as `AGENT.md`, `skills/000-shipflow/SKILL.md`, `skills/001-sf-build/SKILL.md`, `skills/100-sf-spec/SKILL.md`, `skills/references/canonical-paths.md`, `shipflow_data/technical/context.md`, `templates/artifacts/*`, and durable specs.
  - User story link: Proves the taxonomy works on real files and not only on theory.
  - Depends on: Task 3
  - Validate with: `rg -n "AGENT.md|000-shipflow|001-sf-build|100-sf-spec|canonical-paths|context.md|templates/artifacts|shipflow_data/workflow/specs" shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
...
- [x] Task 5: Identify structural debt and extraction targets.
  ...
  - Depends on: Task 4
  - Validate with: `rg -n "split|extract|no-change|mixed|primary type|debt" shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
...
- [x] Task 6: Align the core architecture docs.
  - Files: `shipflow_data/technical/skill-runtime-and-lifecycle.md`, `shipflow-spec-driven-workflow.md`, `README.md`, `AGENT.md`
  ...
  - Validate with: `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md shipflow_data/technical/skill-runtime-and-lifecycle.md shipflow-spec-driven-workflow.md README.md AGENT.md`
...
- [x] A representative mapping of current ShipFlow artifacts to primary types exists.
...
- If the taxonomy implies major structural changes to many skills, stop after the architecture pass and route through `/101-sf-ready` again before implementation.
- Validation commands:
  - `rg -n "entrypoint-router|master-workflow|specialist-workflow|contract|reference|template|record" shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md`
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-12 08:52:46 UTC | 100-sf-spec | GPT-5 Codex | Created a draft spec for canonical ShipFlow artifact taxonomy across skills, references, templates, and records | draft | /101-sf-ready shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md |
| 2026-06-12 09:25:52 UTC | 101-sf-ready | GPT-5 Codex | Reviewed readiness of `ShipFlow Artifact Taxonomy For Skills And References`; closed Open Questions and approved for implementation | ready | /102-sf-start shipflow_data/workflow/specs/shipflow-artifact-taxonomy-for-skills-and-references.md |
