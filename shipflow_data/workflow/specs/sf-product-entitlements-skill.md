...
sf-product-entitlements skill

## Status

Ready. The placement decision is made: create `sf-product-entitlements` as a new domain skill, and update `sf-local-cloud-sync` only with a clear entitlement handoff. The spec has enough user-story fit, behavior contract, security scope, implementation tasks, and proof definition for `/sf-skill-build`.

## User Story

As a ShipFlow operator building suite products, I want a dedicated product entitlements skill, so that identity, provider events, product access, product-local mirrors, support actions, and protected backend gates are designed and verified without duplicating auth-debug or local-cloud sync workflows.

Primary actor: ShipFlow operator or implementation agent introducing, changing, auditing, or repairing product access.
...
- `sf-local-cloud-sync`, which owns data promotion and merge policy once access preconditions are explicit;
- `sf-auth-debug`, which owns auth/session/OAuth diagnosis.

Trigger: the operator invokes `sf-product-entitlements` for a project, product, provider, grant/revoke/refund flow, product-access mirror, backend guard, paywall, support runbook, or entitlement compliance question.
...
- Given data promotion, hydration, merge, offline queue, conflict policy, tombstones, or reinstall recovery is in scope, when entitlement preconditions are defined, then the skill hands off the data-trust contract to `sf-local-cloud-sync`.
- Given auth callback, OAuth, cookies, session restore, provider identity, or token acceptance is failing, when the problem is session-level rather than entitlement-lifecycle design, then the skill routes diagnosis to `sf-auth-debug`.
- Given skill implementation completes, when the operator uses `$sf-product-entitlements`, then the skill is discoverable in current-user Claude/Codex runtime links, help catalog, public skill content, and technical docs.
...
- If implementation scope includes local/cloud data sync without merge, tombstone, account-boundary, or offline semantics, the skill must hand off to `sf-local-cloud-sync` instead of inventing sync logic.
- If the request is only an auth/session/OAuth debugging problem, the skill must route to `sf-auth-debug` rather than broadening the chantier.

## Problem

ShipFlow already has a strong product entitlements playbook, but no dedicated skill entrypoint. That creates a repeated routing gap: agents discover the doctrine only if they already know to load the reference. In suite products, that gap can become a security problem because product teams may accidentally grant access from identity alone, create duplicate product-local ledgers, trust provider events directly at runtime, or sync product data before entitlement and tenant boundaries are verified.

The exploration on 2026-06-10 found that `sf-local-cloud-sync` already owns synchronization well. Creating a generic `sf-data-sync` would duplicate it. The missing entrypoint is specifically product access lifecycle ownership.

## Solution

Create `skills/sf-product-entitlements/SKILL.md` as a domain skill backed by `skills/references/product-entitlements-playbook.md`. Update `sf-local-cloud-sync` with a concise entitlement handoff so sync contracts name access preconditions and route entitlement-ledger work back to `sf-product-entitlements`. Add discoverability and public/docs coherence through help catalog, technical maps, README/workflow surfaces, runtime skill sync, and public skill content if the current site content model supports it.

## Scope In

- Create `skills/sf-product-entitlements/SKILL.md`.
...
- Define clear handoffs to `sf-auth-debug`, `sf-local-cloud-sync`, `sf-spec`, `sf-ready`, `sf-start`/`sf-build`, `sf-verify`, `sf-docs`, `sf-prod`, and `sf-test`.
- Update `skills/sf-local-cloud-sync/SKILL.md` with entitlement-gated sync handoff language only; do not move sync doctrine into the new skill.
- Add or update skill discoverability surfaces: `skills/sf-help/references/help-catalog.md`, `README.md`, `shipflow-spec-driven-workflow.md`, `shipflow_data/technical/skill-runtime-and-lifecycle.md`, and `shipflow_data/technical/code-docs-map.md`.
...
- Repair/check current-user runtime links for `$HOME/.claude/skills/sf-product-entitlements` and `$HOME/.codex/skills/sf-product-entitlements`.
...
- Creating a standalone `sf-data-sync` skill.
- Replacing `sf-local-cloud-sync` or moving its merge/offline/tombstone doctrine into `sf-product-entitlements`.
- Replacing `sf-auth-debug` for session/OAuth/cookie/callback diagnosis.
...
- The skill name is fixed by operator decision: `sf-product-entitlements`.
...
- The implementation must not overwrite unrelated dirty files already present in the ShipFlow repo.
- Runtime skill sync must use ShipFlow helper tools and block on non-symlink conflicts.

## Test Contract

- surface: ShipFlow skill contract, adjacent skill handoff, help/docs/public skill surfaces, runtime skill links.
- stack profile: Markdown skill contracts, ShipFlow governance docs, static site content, shell validation tools.
...
- non-automated proof required: review of pressure scenarios in the spec and `sf-verify` lifecycle gate.
- proof order: spec readiness -> skill creation/update -> runtime link repair/check -> scenario `rg` checks -> skill budget audit -> metadata lint -> public/docs build checks -> `sf-verify` -> ship.
- checklist path: no product manual QA checklist is required; this is a ShipFlow skill-maintenance chantier.
- required scenario ids:
  ...
  - SPE-008 entitlement-gated sync: `sf-product-entitlements` defines access preconditions and hands merge/hydration/tombstone/offline contract to `sf-local-cloud-sync`.
  - SPE-009 auth/session failure: skill routes OAuth/session/cookie/callback debugging to `sf-auth-debug`.
...
- `skills/sf-local-cloud-sync/SKILL.md` and skill-local sync references: existing sync owner to preserve and link.
- `skills/sf-auth-debug/SKILL.md`: auth/session/OAuth diagnosis owner to preserve and link.
- `skills/sf-skill-build/SKILL.md`: implementation owner for skill creation and validation.
...
Fresh external docs verdict: `fresh-docs not needed` for this spec because it defines a ShipFlow skill contract and does not implement provider/API behavior. Future product entitlement specs that touch providers, marketplace APIs, app-store validation, OAuth, webhook signatures, or current SDK behavior must pass the documentation freshness gate with official/current sources.
...
- `sf-build` and `sf-start` can route entitlement implementation through this skill for contracts before coding.
- `sf-local-cloud-sync` remains the data-trust owner and gains an entitlement handoff, reducing ambiguous sync/data-access routing.
- `sf-auth-debug` remains the owner for session/OAuth/callback/provider-auth failures.
- `sf-prod` and `sf-test` remain responsible for hosted/live behavior proof after entitlement implementation ships.
- `sf-docs` remains responsible for docs corpus edits, but this skill must identify docs/support/public-copy impact.
...
- Add the new skill to `skills/sf-help/references/help-catalog.md`.
...
- Update `shipflow_data/technical/skill-runtime-and-lifecycle.md` and `shipflow_data/technical/code-docs-map.md`.
- Add `site/src/content/skills/sf-product-entitlements.md` if current public skill-page patterns support it.
- Avoid copying the full product entitlements playbook into every doc; link to the canonical reference.
- Mention the `sf-local-cloud-sync` handoff in both skills only enough to prevent duplicate sync ownership.
...
1. Create `skills/sf-product-entitlements/SKILL.md` with frontmatter, canonical path instructions, chantier tracking, reporting, required references, mode detection, mission, contract shape, handoffs, stop conditions, validation, and final report format.
...
4. Add explicit handoffs to `sf-auth-debug`, `sf-local-cloud-sync`, `sf-spec`, `sf-ready`, `sf-start`/`sf-build`, `sf-verify`, `sf-prod`, `sf-test`, and `sf-docs`.
5. Update `skills/sf-local-cloud-sync/SKILL.md` with a short entitlement-gated sync integration note and stop/handoff language.
6. Update help/discoverability surfaces: `skills/sf-help/references/help-catalog.md`, `README.md`, `shipflow-spec-driven-workflow.md`, `shipflow_data/technical/skill-runtime-and-lifecycle.md`, and `shipflow_data/technical/code-docs-map.md`.
7. Add public skill page `site/src/content/skills/sf-product-entitlements.md` if public skill pages are currently part of the repo and buildable.
8. Run runtime repair/check with `tools/shipflow_sync_skills.sh --repair --skill sf-product-entitlements` and `tools/shipflow_sync_skills.sh --check --skill sf-product-entitlements`.
...
- `skills/sf-product-entitlements/SKILL.md` exists and is discoverable as `sf-product-entitlements`.
...
- The skill routes data promotion, hydration, merge, conflict, tombstone, offline queue, and reinstall-recovery concerns to `sf-local-cloud-sync`.
- `sf-local-cloud-sync` names entitlement preconditions and hands entitlement-ledger ownership to `sf-product-entitlements`.
- The skill routes session/OAuth/cookie/callback failures to `sf-auth-debug`.
...
- Validation commands pass or any blocker is recorded before `/sf-verify` can pass.

## Test Strategy

Use `scenario-first` because this is a skill contract and routing change. Validate each required `SPE-*` scenario with targeted `rg` checks across `skills/sf-product-entitlements/SKILL.md`, `skills/sf-local-cloud-sync/SKILL.md`, and docs/help surfaces when relevant.

Mechanical validation commands expected during implementation:

```bash
rg -n "name: sf-product-entitlements|product-entitlements-playbook|suite ledger|duplicate|provider event|activation code|snapshot|mirror|backend authorization|support runbook|sf-local-cloud-sync|sf-auth-debug" skills/sf-product-entitlements/SKILL.md
rg -n "entitlement|product access|sf-product-entitlements|fail closed|access preconditions" skills/sf-local-cloud-sync/SKILL.md
python3 tools/skill_budget_audit.py --skills-root skills --format markdown
tools/shipflow_sync_skills.sh --check --skill sf-product-entitlements
tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/sf-product-entitlements-skill.md shipflow_data/technical
git diff --check
```
...
- Overlap risk: the skill could duplicate `sf-local-cloud-sync` or `sf-auth-debug`. Mitigation: handoff rules are part of scope and acceptance criteria.
...
- Dirty-worktree risk: ShipFlow has unrelated dirty files. Mitigation: implementation must avoid reverting or co-mingling unrelated edits.

## Execution Notes

- This spec originated from `/sf-explore` report `/home/claude/replayglowz/explorations/2026-06-10-suite-entitlements-sync-skill-placement.md`.
- The user selected the skill name `sf-product-entitlements` on 2026-06-10.
- Preferred implementation owner: `/sf-skill-build shipflow_data/workflow/specs/sf-product-entitlements-skill.md` after `/sf-ready`.
- Preferred proof path: `scenario-first`.
- Do not create `sf-data-sync` in this chantier.
- Do not edit product projects such as ReplayGlowz as part of this skill-maintenance spec.
- Stop if another active spec already owns the exact `sf-product-entitlements` skill creation.

## Open Questions

None blocking. The public skill page should be included if the current ShipFlow site content surface is buildable during implementation; if the site is unavailable, record a docs/public-content blocker rather than weakening the skill contract.
