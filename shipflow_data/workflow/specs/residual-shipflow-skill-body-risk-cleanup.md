...
updated: "2026-06-10"
updated_at: "2026-06-10 19:04:20 UTC"
status: ready
source_skill: sf-spec
source_model: "GPT-5 Codex"
scope: "skill-governance-refactor"
owner: "Diane"
user_story: "As a ShipFlow operator maintaining local skills, I want the remaining body-size review findings resolved through the established layering contract, so plugin audits finish cleanly without weakening workflow guardrails."
confidence: high
risk_level: high
security_impact: yes
docs_impact: yes
linked_systems:
  ...
  - "skills/sf-audit-components/references/"
  - "skills/sf-build/references/"
  - "skills/sf-fix/references/"
  - "skills/sf-test/references/"
...
depends_on:
  ...
  - artifact: "skills/references/decision-quality-contract.md"
    ...
    required_status: "active"
  - artifact: "shipflow_data/workflow/specs/compact-shipflow-skill-instructions-phase-4.md"
    artifact_version: "1.0.0"
    required_status: "ready"
supersedes: []
evidence:
  - "ShipFlow Core plugin audit on 2026-06-10 after direct cleanup: 66 skills audited, 4 files with review findings, 0 hard findings, 0 style findings."
  - "Remaining plugin audit review findings: sf-audit-components body-size risk about 5111 tokens, sf-build about 5031 tokens, sf-fix about 5134 tokens, and sf-test about 5360 tokens."
...
next_step: "/sf-start Residual ShipFlow Skill Body Risk Cleanup"
---
...
- Trigger: a maintainer runs `/sf-start Residual ShipFlow Skill Body Risk Cleanup` after `/sf-ready` marks this spec ready.
- User/operator result: the operator can rerun the local ShipFlow Core audit and see no findings, or only documented body-size exceptions that are intentionally accepted by verification.
- System effect: `sf-audit-components`, `sf-build`, `sf-fix`, and `sf-test` either fall below the plugin audit body-risk threshold or carry explicit, narrow exceptions justified by safety and accepted by `sf-verify`.
...
- If `sf-test` loses the rule that no test result may be invented or that failing manual tests create/update durable bug records, verification must fail.
- If `sf-fix` loses bug-file intake, direct-versus-spec-first routing, proof-path selection, or security/data stop conditions, verification must fail.
- If `sf-build` loses lifecycle orchestration, model/topology routing, governance corpus gates, or concrete proof-owner routing, verification must fail.
- If `sf-audit-components` loses source-de-chantier reporting, component-system audit phases, or pre-check behavior, verification must fail.
...
The direct local cleanup resolved all hard, style, portability, and runtime-wording findings from the ShipFlow Core plugin audit. The remaining findings are review-level body-size risks on four skills:
...
- `skills/sf-audit-components/references/component-audit-workflow.md`
- `skills/sf-build/references/build-lifecycle-workflow.md`
- `skills/sf-fix/references/bug-fix-workflow.md`
- `skills/sf-test/references/manual-qa-workflow.md`
...
- Compact `skills/sf-audit-components/SKILL.md`.
- Compact `skills/sf-build/SKILL.md`.
- Compact `skills/sf-fix/SKILL.md`.
- Compact `skills/sf-test/SKILL.md`.
...
- Rewriting the broader ShipFlow taxonomy.
- Editing public site skill pages unless the visible public promise changes.
- Changing the ShipFlow Core plugin audit script.
...
- Existing skill-local README files for `sf-audit-components`, `sf-fix`, and `sf-test` may be read for context but must not become implicit hidden instruction sources unless the top-level skill explicitly routes to them.
- Fresh external docs: not needed; this change is local ShipFlow instruction architecture and does not depend on current framework, SDK, provider, auth, build, migration, cache, routing, or integration behavior.
...
- `sf-build` compaction affects the highest-level user-facing orchestrator. Any loss of lifecycle routing can make future implementation work close too early.
- `sf-fix` compaction affects bug traceability, security triage, and proof path selection.
- `sf-test` compaction affects manual QA integrity and durable bug record creation.
- `sf-audit-components` compaction affects audit output quality and source-de-chantier escalation.
...
- Internal technical docs: update `shipflow_data/technical/skill-runtime-and-lifecycle.md` or `shipflow_data/technical/code-docs-map.md` only if new reference-file conventions or skill architecture expectations change.
...
- `sf-build` may be under the local `skill_budget_audit.py` token threshold but over the plugin audit threshold; treat the stricter plugin result as in scope.
...
- [ ] Task 1: Baseline the four residual findings.
  - File: `skills/sf-audit-components/SKILL.md`, `skills/sf-build/SKILL.md`, `skills/sf-fix/SKILL.md`, `skills/sf-test/SKILL.md`
  ...
  - Notes: Treat `sf-build` as in scope even if only the plugin audit flags it.

- [ ] Task 2: Extract `sf-audit-components` detailed audit phases.
  - File: `skills/sf-audit-components/SKILL.md`, `skills/sf-audit-components/references/component-audit-workflow.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "Trace category|Process role|Chantier Potential|Report Modes|Mode detection|Pre-check|component-audit-workflow|Validation" skills/sf-audit-components/SKILL.md`.
  - Notes: Preserve the runtime-neutral wording for structured questions and parallel tooling.

- [ ] Task 3: Extract `sf-build` lifecycle detail without weakening orchestration.
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "Trace category|Process role|Master Delegation|Master Workflow Lifecycle|Existing Chantier Check|Question Gate|Stop Conditions|Final Report|build-lifecycle-workflow" skills/sf-build/SKILL.md`.
  - Notes: Do not change subagent consent semantics or spec-gated parallel rules.

- [ ] Task 4: Extract `sf-fix` bug workflow detail.
  - File: `skills/sf-fix/SKILL.md`, `skills/sf-fix/references/bug-fix-workflow.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "Trace category|Process role|Chantier Potential|spec-driven-development-discipline|decision-quality-contract|Direct fix|Spec-first|BUG-ID|Stop Conditions|bug-fix-workflow" skills/sf-fix/SKILL.md`.
  - Notes: No bug trace exception expansion is allowed.

- [ ] Task 5: Extract `sf-test` manual QA workflow detail.
  - File: `skills/sf-test/SKILL.md`, `skills/sf-test/references/manual-qa-workflow.md`
  ...
  - Depends on: Task 1.
  - Validate with: `rg -n "Trace category|Process role|Never invent test results|TEST_LOG|BUG-|project-development-mode|manual-qa-workflow|Final Report" skills/sf-test/SKILL.md`.
...
- [ ] Task 6: Validate references, budgets, and behavior-preserving labels.
  ...
  - Validate with: `python3 ~/plugins/shipflow-core/scripts/audit_shipflow_skills.py`; `python3 tools/skill_budget_audit.py --skills-root skills --format markdown`; `python3 tools/shipflow_metadata_lint.py <new-reference-files> shipflow_data/workflow/specs/residual-shipflow-skill-body-risk-cleanup.md`; `git diff --check`.
...
- [ ] Task 7: Update docs/changelog only if affected.
  - File: `shipflow_data/technical/skill-runtime-and-lifecycle.md`, `shipflow_data/technical/code-docs-map.md`, `CHANGELOG.md`
  ...
  - Depends on: Task 6.
  - Validate with: `rg -n "sf-audit-components|sf-build|sf-fix|sf-test|skill-local reference|instruction layering" shipflow_data/technical CHANGELOG.md`.
  - Notes: Public docs are out of scope unless behavior claims change.

## Acceptance Criteria

- [ ] AC 1: Given the current ShipFlow skill tree, when `python3 ~/plugins/shipflow-core/scripts/audit_shipflow_skills.py` runs, then it reports 0 unresolved hard, style, and review findings, or any remaining review finding is explicitly documented as an accepted exception in this spec and final report.
- [ ] AC 2: Given the current ShipFlow skill tree, when `python3 tools/skill_budget_audit.py --skills-root skills --format markdown` runs, then it reports 0 hard violations, 0 warnings, and no unresolved body-token risks for the four target skills.
...
- [ ] AC 4: Given `sf-test` is compacted, when manual QA is requested, then the top-level body still prevents invented test results and routes durable logs/bug files correctly.
- [ ] AC 5: Given `sf-fix` is compacted, when a bug is reported, then the top-level body still distinguishes direct fixes from spec-first work and preserves bug-file/proof-path/security gates.
- [ ] AC 6: Given `sf-build` is compacted, when a lifecycle implementation is requested, then the top-level body still preserves existing chantier checks, readiness routing, topology semantics, and proof-owner routing.
- [ ] AC 7: Given `sf-audit-components` is compacted, when project/file/global audit modes are requested, then the top-level body still routes to the detailed component audit workflow and preserves source-de-chantier output.
- [ ] AC 8: Given new reference files are created, when metadata lint runs, then their frontmatter is valid and they resolve from `$SHIPFLOW_ROOT`.
...
- Security impact: yes, because `sf-fix`, `sf-test`, and `sf-build` include security/data/proof gates; mitigation is to keep non-negotiable guards local and verify labels with `rg`.
...
- Next step: `/sf-start Residual ShipFlow Skill Body Risk Cleanup`.
