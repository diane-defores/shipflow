...
owner: "Diane"
user_story: "En tant qu'operatrice ShipFlow, je veux que les skills de domaine ne portent plus un mode comme audit ou debug dans leur identite principale mais exposent ces intentions comme des modes, afin que le routing soit plus clair et que les playbooks, checklists et references portent la doctrine metier."
risk_level: high
...
supersedes: []
evidence:
  - "User decision 2026-06-29: audit-seo should become sf-seo with audit as a mode."
  - "User decision 2026-06-29: auth-debug should become sf-auth with debug as a mode."
  - "First tranche already migrated 406-sf-audit-seo to 406-sf-seo and public page sf-audit-seo to sf-seo."
  ...
  - "ShipFlow governance now separates skills as activation routers from references, playbooks, checklists, and test-checklists as doctrine/control/proof artifacts."
next_step: "/101-sf-ready audit-skill-domain-mode-taxonomy-migration"
---
...
En tant qu'operatrice ShipFlow, je veux que les skills de domaine ne portent plus un mode comme `audit` ou `debug` dans leur identite principale mais exposent ces intentions comme des modes, afin que le routing soit plus clair et que les playbooks, checklists et references portent la doctrine metier.

## Minimal Behavior Contract

ShipFlow must rename domain skills whose runtime identity contains a mode name, such as `audit` or `debug`, to domain-first runtime names while preserving their numeric code, converting the mode word from identity to mode, and moving any remaining long doctrine out of `SKILL.md` into references, playbooks, or checklists. Existing live routing, public skill pages, plugin pack catalogs, help docs, code index, runtime symlinks, and content/governance references must point to the new names. Historical specs, changelogs, archives, and audit reports may keep old names as historical evidence.

## Success Behavior

- A fresh agent can invoke the domain skill name, for example `406-sf-seo` or `109-sf-auth`, then choose or infer modes such as `audit`, `debug`, `setup`, `verify`, `fix`, `launch`, or `monitoring`.
...
Several skills still encode a mode directly in their runtime identity. That made sense when those skills were mostly one-off helpers, but ShipFlow is moving toward domain routers backed by playbooks, checklists, and references. Names such as `406-sf-audit-seo` make SEO look like an audit-only function even though it now owns launch readiness, monitoring, fix routing, and domain doctrine loading. `109-sf-auth-debug` has the same structural problem: auth is the domain, while debug is only one operating mode.
...
- Rename domain-oriented audit skills so `audit` becomes a mode:
  - `406-sf-audit-seo` -> `406-sf-seo` (already started)
  - `407-sf-audit-translate` -> `407-sf-translate`
  - `408-sf-audit-gtm` -> `408-sf-gtm`
  - `409-sf-audit-a11y` -> `409-sf-a11y`
  - `503-sf-audit-design-tokens` -> `503-sf-design-tokens`
  - `504-sf-audit-components` -> `504-sf-components`
- Decide and implement the correct design lane treatment:
  - preferred target: `006-sf-design` owns mode `audit`
  - `502-sf-audit-design` must either become `502-sf-design-review` as a specialist mode helper or be deprecated/absorbed through a documented route
- Decide and implement the correct code lane treatment:
  - target candidate: `401-sf-code`, with mode `audit`
  - keep `400-sf-audit` as the cross-domain audit master only if the word `audit` is still semantically the product identity; otherwise rename to `400-sf-review` or `400-sf-quality`
...
- Do not rename `705-sf-conversation-audit` in this tranche unless the operator explicitly decides conversation audit should also become a broader domain skill.
- Do not use the `109-sf-auth` rename to broaden auth behavior into destructive account, provider, or production-data mutation without explicit approval.
...
- Fresh external docs are not needed because this is an internal ShipFlow taxonomy and documentation migration.
...
- `SEO-DOMAIN-ROUTER`: `$406-sf-seo audit <page>` routes to SEO audit detail, while `$406-sf-seo launch` loads the launch playbook/checklist.
- `PUBLIC-SKILL-PAGE`: public site builds and exposes `/skills/sf-seo/` without the old `/skills/sf-audit-seo/` content source.
...
- `AUTH-DOMAIN-ROUTER`: `$109-sf-auth debug <flow>` preserves current auth-debug behavior, while `$109-sf-auth verify <flow>` can route auth smoke proof without implying a bug investigation.

## Dependencies

- `skills/references/skill-code-index.md` defines code/name policy and must be updated for each rename.
- `skills/109-sf-auth-debug/SKILL.md` currently owns auth debug behavior and must become the source for the new `109-sf-auth` router contract.
...
- `000-shipflow` and `302-sf-help` discoverability may need updates so users see domain-first commands.
- `007-sf-content`, `202-sf-repurpose`, `200-sf-redact`, and `201-sf-enrich` routing must use `406-sf-seo` for SEO review.
- Design routes must avoid duplicate ownership between `006-sf-design` and any renamed `502` helper.
- Public skill category labels may need a later review because `Audit & Improve` will contain fewer `sf-audit-*` names.
...
- historical `shipflow_data/workflow/specs/**`
- historical `shipflow_data/workflow/archives/**`
...
2. Complete and verify SEO migration.
   - Files: `skills/406-sf-seo/**`, `shipflow-site/src/content/skills/sf-seo.md`, active docs and pack catalogs.
   - Action: ensure all active references use `406-sf-seo` / `sf-seo`; remove stale runtime symlinks; validate site build.

3. Migrate GTM.
   - Files: `skills/408-sf-audit-gtm/**`, `shipflow-site/src/content/skills/sf-audit-gtm.md`, active references.
   - Action: rename to `skills/408-sf-gtm`, update `name:`, public slug `sf-gtm`, references, pack catalogs, help docs, runtime symlinks; keep mode `audit` in the skill.

4. Migrate i18n/translation.
   - Files: `skills/407-sf-audit-translate/**`, public skill page, active references.
   - Action: rename to `407-sf-translate`, expose modes `audit`, `sync`, `apply` as appropriate, and move long workflow detail into references if still top-heavy.

5. Migrate accessibility.
   - Files: `skills/409-sf-audit-a11y/**`, public skill page, active references.
   - Action: rename to `409-sf-a11y`, expose mode `audit`, preserve accessibility proof and WCAG references.

6. Migrate design-token and component specialists.
   - Files: `skills/503-sf-audit-design-tokens/**`, `skills/504-sf-audit-components/**`, public pages, active references.
   - Action: rename to `503-sf-design-tokens` and `504-sf-components`; keep audit/deep-audit as modes or references.

7. Decide and implement the design audit lane.
   - Files: `skills/006-sf-design/**`, `skills/502-sf-audit-design/**`, public pages, help docs.
   - Action: either absorb `502` into `006-sf-design audit` or rename `502` to a non-audit specialist helper such as `502-sf-design-review`; document the final route.

8. Decide and implement code/copy lanes.
   - Files: `skills/401-sf-audit-code/**`, `skills/206-sf-audit-copy/**`, `skills/207-sf-audit-copywriting/**`, public pages, routing docs.
...
9. Migrate auth debug to auth domain router.
   - Files: `skills/109-sf-auth-debug/**`, public skill page if present, help docs, routing docs, skill code index, runtime symlinks.
   - Action: rename to `109-sf-auth`, keep current auth debug behavior as mode `debug`, add mode routing for `audit`, `setup`, `verify`, and `fix` only where existing doctrine or owner routing supports it, and move long auth debug detail to references if needed.
...
11. Validate and close the tranche.
    ...
    - Action: run validation commands listed below, fix failures, then route to `/103-sf-verify`.
...
- [ ] `109-sf-auth` exposes `debug` as a mode and preserves the current auth-debug behavior contract.
...
rg -n "<old-name>|<old-public-slug>" . \
  ...
  --glob '!shipflow_data/workflow/specs/**' \
  --glob '!shipflow_data/workflow/archives/**' \
  --glob '!shipflow_data/workflow/reviews/**' \
  --glob '!shipflow_data/workflow/audits/**' \
...
100-sf-spec ✅ -> 101-sf-ready ⏳ -> 102-sf-start ⏳ -> 103-sf-verify ⏳ -> 104-sf-end ⏳ -> 005-sf-ship ⏳

## Skill Run History

| Date UTC | Skill | Model | Action | Result | Next step |
|----------|-------|-------|--------|--------|-----------|
| 2026-06-29 09:42:00 UTC | 100-sf-spec | GPT-5 Codex | Created migration spec for renaming audit-named domain skills and extracting audit doctrine into references/playbooks/checklists. | draft | `/101-sf-ready audit-skill-domain-mode-taxonomy-migration` |
