...
owner: "Diane"
confidence: high
user_story: "En tant qu'opératrice ShipFlow dans un terminal, je veux ouvrir une TUI locale depuis /home/claude/shipflow/tui pour naviguer projets, chantiers, audits, tâches et logs sans lancer l'app Flutter, afin de piloter ShipFlow plus vite tout en gardant les fichiers Markdown et ledgers comme source de vérité."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
supersedes: []
evidence:
  - "User decision 2026-05-17: put the TUI inside shipflow_app, not in a separate GitHub repository."
  - "User decision 2026-05-23: move the TUI into /home/claude/shipflow/tui next to the terminal skills, not in ShipFlow App."
  - "User decision 2026-05-17: formalize the TUI application with a ShipFlow spec."
  - "Prior exploration 2026-05-17: Gum fits immediate shell prompts; OpenTUI fits a future persistent terminal dashboard."
  - "README.md and tui/README.md define the Terminal TUI as a local-first read-only dashboard over ShipFlow Markdown and ledgers."
  - "CLAUDE.md requires active ShipFlow work to preserve Markdown/repository files as source of truth and avoid client-side privileged secrets."
...
next_step: "/sf-start ShipFlow Terminal TUI V1"
---
...
Ready for implementation. This spec formalizes a new optional terminal UI surface inside `/home/claude/shipflow/tui` and has passed `/sf-ready` for scope, dependencies, security boundaries, and validation gates.

# User Story

En tant qu'opératrice ShipFlow dans un terminal, je veux ouvrir une TUI locale dans `/home/claude/shipflow/tui` pour naviguer projets, chantiers, audits, tâches et logs sans lancer l'app Flutter, afin de piloter ShipFlow plus vite tout en gardant les fichiers Markdown et ledgers comme source de vérité.

# Minimal Behavior Contract

The TUI accepts a local terminal session on the operator machine, reads only allowlisted ShipFlow Markdown and ledger sources, renders a persistent dashboard for projects, specs/chantiers, tasks/audits, checks/logs, and skills, and exits cleanly without requiring Flutter, web deployment, cloud auth, database projection, or a new repository. If OpenTUI/Bun is unavailable, the existing ShipFlow CLI and Flutter app remain functional, and command-line flows can fall back to plain shell or Gum-enhanced prompts where Gum is installed. The easy edge case to miss is accidentally turning the TUI into an execution or write-back surface before read-only navigation and source safety are proven.
...
- Given ShipFlow source files exist, the TUI shows a read-only overview of projects, chantiers/specs, tasks/audits, ledgers, and recent operational signals.
...
- Given a spec exists under `shipflow_data/workflow/specs/`, the TUI displays its title, status, user story, next step, run history summary, and current chantier flow.
...
- Given Bun or OpenTUI is not installed, ShipFlow core CLI, Flutter app, and existing validation flows remain unaffected.
...
`shipflow_app` pursues a Flutter dashboard for graphical operational visibility. That remains useful for a product surface, but the immediate ShipFlow operator workflow lives in the terminal. Keeping the TUI beside the ShipFlow skills is more coherent than hosting it in the Flutter app repository.

The product need is a fast local cockpit that lives next to the existing ShipFlow skills, reuses the same governance corpus, avoids a new GitHub repository, and does not disturb the Flutter runtime or future graphical product path.

# Solution

Add an optional terminal UI package under `/home/claude/shipflow/tui` as a separate subproject. V1 uses OpenTUI for a persistent interactive dashboard if Bun/OpenTUI are available, while keeping Gum as a separate shell-enhancement tool for one-shot prompts in `shipflow.sh` or helper scripts. The TUI reads ShipFlow Markdown and ledgers through a local allowlisted source layer and exposes read-only navigation first: projects, specs/chantiers, tasks/audits, checks/logs, and skills.
...
- Define a read-only source layer for ShipFlow Markdown and ledgers using allowlisted roots and size limits.
...
- Document the role split between ShipFlow CLI, Gum-enhanced shell prompts, TUI, and Flutter app.
...
- No terminal-based write-back to ShipFlow trackers, specs, repositories, Git, PM2, Caddy, Flox, or deployment systems.
...
- ShipFlow Markdown and repository files remain canonical.
...
- Local project context:
  - `README.md` defines the current local-first, read-only ShipFlow dashboard direction.
  - `CLAUDE.md` defines the active ShipFlow runtime, Flutter boundaries, local source-of-truth rule, and security constraints.
  - `AGENT.md` requires active work to target ShipFlow modules and classify legacy reuse before changes.
  - `shipflow_data/workflow/specs/shipflow-dashboard-readonly-projection.md` defines the existing dashboard read-only projection posture.
  - `shipflow_data/technical/markdown-source-of-truth.md` and `runtime-boundary.md` govern source authority and active/legacy boundaries.
...
- ShipFlow CLI: remains the stable core entrypoint. Future CLI integration may expose `shipflow tui`, but the first implementation can run from `/home/claude/shipflow/tui`.
...
- ShipFlow data model: TUI reads local Markdown and ledgers directly; it does not depend on Firestore projection.
...
- Packaging: Bun/OpenTUI dependency must be isolated to avoid making ShipFlow installation heavier by default.
...
- Add `shipflow_data/technical/terminal-tui.md` during implementation to document architecture, source policy, views, commands, and validation.
- Update `shipflow_data/technical/code-docs-map.md` to include `tui/**` once files exist.
...
- `/home/claude/shipflow_data` exists but expected files are missing.
- Project-local `shipflow_data/` exists for some projects and not others.
...
- [ ] Task 3: Define TUI source policy.
  ...
  - User story link: Lets the TUI read local ShipFlow sources safely.
  - Depends on: Task 1.
...
- [ ] Task 4: Add source readers and parsers.
  ...
  - Action: Read master ShipFlow files, project-local governance docs, specs, tasks/audits, operations log, dependency log, and skill metadata through the source policy.
...
- [ ] Task 7: Add command wrapper documentation.
  ...
  - Validate with: `rg -n "Bun|OpenTUI|read-only|Flutter|Gum|shipflow_data" tui/README.md`
...
- [ ] Task 8: Add technical governance doc.
  - File: `shipflow_data/technical/terminal-tui.md`
  ...
  - User story link: Keeps the TUI maintainable inside the ShipFlow governance corpus.
  - Depends on: Tasks 1-7.
  - Validate with: `rg -n "source policy|read-only|OpenTUI|Gum|Flutter|write action" shipflow_data/technical/terminal-tui.md`
...
- [ ] Task 9: Update contributor and project docs after implementation exists.
  - File: `README.md`, `CLAUDE.md`, `AGENT.md`, `shipflow_data/technical/code-docs-map.md`
  ...
  - Validate with: `rg -n "tui|OpenTUI|Bun|read-only|Flutter" README.md CLAUDE.md AGENT.md shipflow_data/technical/code-docs-map.md`
  - Notes: Do not update public/user-facing claims before the TUI works.

# Acceptance Criteria

- The TUI spec is stored under `/home/claude/shipflow/shipflow_data/workflow/specs/` and does not create a new repository.
...
- Static validation:
  ...
  - `rg -n "OpenTUI|Bun|read-only|source policy|Flutter|Gum" README.md CLAUDE.md AGENT.md shipflow_data/technical/terminal-tui.md`
- Unit tests:
...
- Regression checks:
  - `cd /home/claude/shipflow_app/app && flutter analyze` remains valid for Flutter work when touched.
  - `cd /home/claude/shipflow_app/app && flutter test` remains the Flutter validation loop when Flutter files are touched.
...
- Maintenance risk: adding TypeScript/Bun introduces a second stack in `shipflow_app`. Mitigation: keep package small, with explicit validation commands.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-17 21:45:03 UTC | sf-spec | GPT-5 Codex | Created the ShipFlow Terminal TUI V1 spec from user decision to place the TUI inside shipflow_app instead of a separate repo. | Draft spec created. | /sf-ready ShipFlow Terminal TUI V1 |
| 2026-05-17 21:51:33 UTC | sf-ready | GPT-5 Codex | Validated structure, metadata, user-story fit, source safety, external docs, language doctrine, adversarial review, and security scope. | ready | /sf-start ShipFlow Terminal TUI V1 |
| 2026-05-17 22:08:35 UTC | sf-build | GPT-5.3 Codex | Implemented optional `/home/claude/shipflow/tui` Bun/TypeScript OpenTUI V1 with read-only source policy, readers, view model, dashboard view, entrypoint, deterministic tests, documentation updates, and integration corrections for project-root allowlisting and current OpenTUI package version. | partial: Bun validation unavailable locally. | Install Bun, run `bun install`, `bun run typecheck`, `bun test`, then /sf-verify ShipFlow Terminal TUI V1 |
| 2026-05-17 22:44:21 UTC | sf-verify | GPT-5 Codex | Verified the TUI implementation against the spec, corrected the spec-title parser smoke failure, checked source-policy smoke behavior, static TypeScript syntax, docs coherence, dependency freshness, and Bun availability. | not verified: Bun/OpenTUI runtime checks cannot run locally and interactive navigation/filtering/selected-detail acceptance remains incomplete. | /sf-start complete TUI interaction gaps, install Bun, run `bun install`, `bun run typecheck`, `bun test`, and manual OpenTUI smoke. |
| 2026-05-18 05:21:58 UTC | sf-build | GPT-5 Codex | Completed the TUI interaction gaps: added dashboard navigation state, project/spec filters, selected detail rendering, run-history and chantier-flow spec summaries, Bun installation, current OpenTUI dependency, Bun lockfile, tests, docs updates, and OpenTUI TTY smoke. | implemented | /sf-verify ShipFlow Terminal TUI V1 |
| 2026-05-18 15:13:22 UTC | sf-verify | GPT-5 Codex | Verified user-story outcome, completeness, correctness, documentation coherence, dependency freshness, read-only/security boundary, bug gate, Bun/OpenTUI runtime smoke, typecheck, tests, source policy, and dependency audit. | verified | /sf-end ShipFlow Terminal TUI V1 |
| 2026-05-19 15:40:31 UTC | sf-design | GPT-5 Codex | Refined the TUI for phone-sized terminals: active-panel-only rendering, shorter header/status line, tighter project grid, shorter spec/activity lists, and compact docs/tests. | implemented | /sf-verify compact mobile terminal rendering before closure. |
| 2026-05-19 15:47:58 UTC | sf-fix | GPT-5 Codex | Fixed BUG-2026-05-19-001: project filtering now scopes specs by parsed spec `project:` metadata, selected project aliases, or the typed project scope; restored compact multi-section rendering with projects, specs, and recent tasks/audits. | fixed-pending-verify | /sf-test --retest BUG-2026-05-19-001 |
| 2026-05-19 16:03:54 UTC | sf-fix | GPT-5 Codex | Repaired BUG-2026-05-19-001 false-positive scope leak from generic path aliases (`home`, `claude`) using user evidence `p:"gocharb"` with `gocharbon_quiz` selected. | fixed-pending-verify | /sf-test --retest BUG-2026-05-19-001 |
| 2026-05-19 17:07:27 UTC | sf-design | GPT-5 Codex | Reduced phone-screen density: default view now renders only Projects and Specs; Recent tasks/audits render only from the activity panel. | implemented | /sf-test --retest BUG-2026-05-19-001 on phone. |
| 2026-05-19 17:33:05 UTC | sf-design | GPT-5 Codex | Moved Diagnostics out of the regular Tab cycle; Tab now cycles Projects, Specs, and Activity, while `d` opens Diagnostics as a rare action. | superseded | Use `!!!` instead of `d` to avoid filter collisions. |
| 2026-05-19 17:43:53 UTC | sf-design | GPT-5 Codex | Replaced the diagnostics shortcut `d` with the reserved `!!!` chord so normal letters remain available for project/spec filtering. | implemented | /sf-test TUI keyboard smoke on phone. |
| 2026-05-20 09:16:36 UTC | sf-fix | GPT-5 Codex | Fixed BUG-2026-05-20-001: Activity now parses task/audit Markdown tables instead of rendering file headings; local project rows are prioritized and legacy task sections are ignored. | fixed-pending-verify | /sf-test --retest BUG-2026-05-20-001 |
| 2026-05-23 10:35:35 UTC | sf-design | GPT-5 Codex | Added a global left margin and hanging continuation indentation across Projects, Specs, Activity, Audits, and Diagnostics; tightened the 3-column project grid to avoid terminal hard-wrap on phone width. | implemented | /sf-test TUI phone wrap smoke. |
| 2026-05-23 10:37:03 UTC | sf-design | GPT-5 Codex | Removed duplicated section labels from Activity/Audits rendering: Activity panel title is now `Tasks`, and Audits no longer repeats `Audits:` inside the list. | implemented | /sf-test TUI activity/audits label smoke. |
| 2026-05-23 11:08:19 UTC | sf-design | GPT-5 Codex | Increased the global TUI left gutter from two to four spaces so status traffic lights no longer sit against the screen edge; updated wrap regression coverage. | implemented | /sf-test TUI phone gutter smoke. |
| 2026-05-23 11:12:08 UTC | sf-design | GPT-5 Codex | Homogenized arrow navigation across panels: `Up`/`Down` now selects specs, tasks, and audits in their active panels, with the same `>` marker semantics. | implemented | /sf-test TUI active-panel navigation smoke. |
| 2026-05-23 11:21:45 UTC | sf-design | GPT-5 Codex | Added auto-scrolling visible windows for projects, specs, tasks, and audits so arrow navigation reveals results beyond the initially displayed rows without separate pagination keys. | implemented | /sf-test TUI list scrolling smoke. |
| 2026-05-23 11:25:57 UTC | sf-design | GPT-5 Codex | Added blank spacing between visible result rows across project rows, specs, tasks, and audits to improve phone readability without changing wrapped continuation indentation. | implemented | /sf-test TUI row spacing smoke. |
| 2026-05-23 16:48:22 UTC | sf-build | GPT-5 Codex | Moved the Terminal TUI ownership from `shipflow_app` into `/home/claude/shipflow/tui`, moved TUI spec/bug/technical docs into ShipFlow workflow data, updated installer aliases, and repointed app docs to the ShipFlow-owned location. | implemented | /sf-test TUI relocated install smoke. |
| 2026-05-23 19:44:11 UTC | continue | GPT-5 Codex | Integrated the ShipFlow root installer with the TUI installer, added the short `tui` command, ensured `~/.local/bin` is exported by ShipFlow user setup, and documented the new install path. | implemented | /sf-verify ShipFlow install TUI bootstrap. |
| 2026-05-23 19:49:02 UTC | sf-content | GPT-5 Codex | Documented the Terminal TUI internally and publicly across README, TUI operator guide, technical contract, editorial maps, public surface map, and the public docs page. | implemented | /sf-verify ShipFlow TUI documentation surfaces. |

# Current Chantier Flow

| Phase | Status | Evidence | Next step |
|-------|--------|----------|-----------|
| sf-spec | done | `shipflow_data/workflow/specs/shipflow-terminal-tui-v1.md` | /sf-ready ShipFlow Terminal TUI V1 |
| sf-ready | ready | Readiness gate passed on 2026-05-17 21:51:33 UTC. | /sf-start ShipFlow Terminal TUI V1 |
| sf-start | done | `/home/claude/shipflow/tui` implemented with read-only V1 scope, keyboard navigation, filtering, selected detail rendering, docs, tests, and Bun/OpenTUI smoke. | /sf-verify ShipFlow Terminal TUI V1 |
| sf-verify | verified | Bun 1.3.14, OpenTUI 0.2.14, `bun run typecheck`, `bun test`, `bun audit`, source-policy smoke, reader/view-model smoke, docs checks, and OpenTUI TTY smoke passed. | /sf-end ShipFlow Terminal TUI V1 |
| sf-design | implemented | Phone-sized terminal refinement: active-panel-only rendering, compact header, reduced list heights, docs, and regression tests. | /sf-verify compact mobile terminal rendering before closure. |
| sf-fix | fixed-pending-verify | BUG-2026-05-19-001 fixed with project-scoped specs, no generic path alias leaks, compact multi-section layout, typecheck, tests, diff check, and real-data smoke for `contentflow_app` and `gocharb`. | /sf-test --retest BUG-2026-05-19-001 |
| sf-design | implemented | Default phone view reduced to Projects + Specs; Recent tasks/audits moved behind the activity panel. | /sf-test --retest BUG-2026-05-19-001 on phone. |
| sf-design | implemented | Diagnostics removed from regular Tab cycle and moved to dedicated `!!!` chord, leaving normal letters available for filtering. | /sf-test TUI keyboard smoke on phone. |
| sf-fix | fixed-pending-verify | BUG-2026-05-20-001 fixed with table-aware activity summaries, local-first task/audit rows, legacy task exclusion, tests, typecheck, diff check, and real-data Activity smoke. | /sf-test --retest BUG-2026-05-20-001 |
| sf-design | implemented | Global TUI left margin and hanging wrap applied to all panels; 54-column smoke covers Projects, Specs, Activity, Audits, and Diagnostics. | /sf-test TUI phone wrap smoke. |
| sf-design | implemented | Activity/Audits title duplication removed; smoke shows `* Tasks` and `* Audits` without repeated inner labels. | /sf-test TUI activity/audits label smoke. |
| sf-design | implemented | Left gutter increased to four spaces before panel titles, project rows, and status traffic lights; 54-column smoke confirms indentation and hanging wraps. | /sf-test TUI phone gutter smoke. |
| sf-design | implemented | Active-panel navigation homogenized: task and audit rows now keep their own selection state and render `>` like specs. | /sf-test TUI active-panel navigation smoke. |
| sf-design | implemented | Visible lists now auto-scroll when the active selection moves past the displayed rows; no separate next-page command is required. | /sf-test TUI list scrolling smoke. |
| sf-design | implemented | Blank spacing inserted between displayed result rows while preserving hanging wrap alignment inside each result. | /sf-test TUI row spacing smoke. |
| sf-build | implemented | Terminal TUI moved to `/home/claude/shipflow/tui`; installer, aliases, spec, bugs, and technical docs now resolve from ShipFlow instead of ShipFlow App. | /sf-test TUI relocated install smoke. |
| continue | implemented | Root ShipFlow `install.sh` now provisions the TUI for configured users, exports `~/.local/bin`, and the TUI installer creates `tui`, `sftui`, `sf-tui`, and `shipflow-tui`. | /sf-verify ShipFlow install TUI bootstrap. |
| sf-content | implemented | Internal and public documentation now cover TUI purpose, install, aliases, read-only boundary, source-policy positioning, and Gum/Flutter relationship. | /sf-verify ShipFlow TUI documentation surfaces. |
| sf-end | pending | Not started. | Run closure. |
| sf-ship | pending | Not started. | Run after closure. |
