...
owner: "Diane"
user_story: "As the ShipFlow operator, I want ShipFlow Core to become an internal ShipFlow skill with a versioned audit tool, so I can use it on future servers without publishing or maintaining a separate public Codex plugin."
confidence: high
risk_level: high
security_impact: yes
docs_impact: yes
...
depends_on:
  ...
  - artifact: "skills/references/skill-execution-fidelity.md"
    ...
    required_status: "active"
  - artifact: "shipflow_data/technical/codex-plugin-packaging.md"
    artifact_version: "1.0.0"
    required_status: "reviewed"
supersedes: []
evidence:
  - "The operator decided that ShipFlow Core is useful internally but should not be packaged as the public user plugin."
...
next_step: "/104-sf-end shipflow-core internal skill port"
---

# Title

ShipFlow Core Internal Skill Port
...
As the ShipFlow operator, I want ShipFlow Core to become an internal ShipFlow skill with a versioned audit tool, so I can use it on future servers without publishing or maintaining a separate public Codex plugin.

## Minimal Behavior Contract

ShipFlow must expose an internal operator skill named `900-shipflow-core` from the ShipFlow repo itself. The skill must audit and inspect local ShipFlow skills through a versioned `tools/audit_shipflow_skills.py` helper, preserve read-only behavior by default, and clearly separate internal operator usage from the public `shipflow` plugin. If the ShipFlow source tree or audit helper is unavailable, the skill reports a ShipFlow installation gap instead of relying on the deprecated plugin path. The easy edge case is leaving stale `~/plugins/shipflow-core/...` references that work only on Diane's current machine and break on a new server.

## Success Behavior

Given a machine with the ShipFlow repo installed and current-user skill links synced, when Diane invokes `$900-shipflow-core audit local ShipFlow skills`, Codex loads `skills/900-shipflow-core/SKILL.md`, resolves ShipFlow-owned paths from `${SHIPFLOW_ROOT:-$HOME/shipflow}`, runs or points to `tools/audit_shipflow_skills.py`, reports audit findings in French for the operator, and does not require `shipflow-core@personal` to be installed from a marketplace.

## Error Behavior

If `${SHIPFLOW_ROOT:-$HOME/shipflow}` is missing, `skills/` is missing, the audit helper is missing, or runtime skill links cannot be repaired because a non-symlink target blocks them, the run must stop with a concrete installation/runtime-link gap. It must not silently fall back to `/home/claude/plugins/shipflow-core`, scrape public docs for execution rules, or ask the operator to test before local validation paths have been attempted.

## Problem

`shipflow-core` started as a Codex plugin pilot. That proved local plugin packaging works, but it also created confusion: the operator thought ShipFlow Core might be a multi-skill bundle, while the actual pilot is one plugin-native skill plus one audit script. Keeping it as a public or marketplace plugin would make an internal maintenance surface look like a user-facing product and would be harder to recover on a new server.

## Solution

Promote the useful part of the pilot into the ShipFlow repo: create `skills/900-shipflow-core/SKILL.md`, move the audit helper into `tools/audit_shipflow_skills.py`, update references and docs away from `~/plugins/shipflow-core/...`, sync the new skill into current runtimes, and leave the old plugin source as deprecated local history until a later cleanup safely removes it.
...
- Update ShipFlow references that instruct agents to run the old plugin script path.
...
- ShipFlow-owned paths must resolve from `${SHIPFLOW_ROOT:-$HOME/shipflow}`.
...
- `surface`: ShipFlow skill/runtime portability and local audit tooling.
...
- Local ShipFlow repo at `${SHIPFLOW_ROOT:-$HOME/shipflow}`.
...
- The audit script uses `SHIPFLOW_ROOT` or `$HOME/shipflow`, not the old plugin directory, to locate skills.
- The old plugin can remain installed locally during transition, but it must not be required for the new skill path.
- Runtime links must be symlinks managed by ShipFlow tooling, not manual copies.

## Links & Consequences

- `skills/705-sf-conversation-audit/SKILL.md` currently points at the old plugin audit path and must route to the versioned tool instead.
...
- `shipflow_data/technical/codex-plugin-packaging.md` and `code-docs-map.md` must reflect that the public plugin is `shipflow`, while `shipflow-core` is internal.
...
- A future server has ShipFlow cloned but no `~/plugins/shipflow-core`; `$900-shipflow-core` must still work.
...
2. File: `tools/audit_shipflow_skills.py`
   - Action: add the pilot audit helper to the versioned ShipFlow toolset with neutral wording and canonical `SHIPFLOW_ROOT` behavior.
   - Validate with: `python3 tools/audit_shipflow_skills.py`.
3. Files: `skills/705-sf-conversation-audit/SKILL.md`, `skills/references/skill-execution-fidelity.md`
   - Action: replace active old plugin script paths with `python3 ${SHIPFLOW_ROOT:-$HOME/shipflow}/tools/audit_shipflow_skills.py` or equivalent canonical wording.
   - Validate with: `rg -n "plugins/shipflow-core|~/plugins/shipflow-core|audit_shipflow_skills" skills/705-sf-conversation-audit/SKILL.md skills/references/skill-execution-fidelity.md`.
4. Files: `shipflow_data/technical/codex-plugin-packaging.md`, `shipflow_data/technical/code-docs-map.md`
...
- [x] AC 1: Given ShipFlow is cloned on a machine, when `$900-shipflow-core` is invoked, then Codex can load the internal skill from `skills/900-shipflow-core/SKILL.md` after runtime sync.
- [x] AC 2: Given local skills exist, when `python3 tools/audit_shipflow_skills.py` runs, then it audits `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills` without depending on the old plugin path.
- [x] AC 3: Given the personal marketplace is listed, then `shipflow-core` is absent and `shipflow` remains present.
- [x] AC 4: Given docs and references mention active audit commands, then they point to the versioned ShipFlow tool path, not `~/plugins/shipflow-core/...`.
...
- Read first: old plugin skill, old audit script, `skills/references/skill-execution-fidelity.md`, and `shipflow_data/technical/codex-plugin-packaging.md`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-11 10:05:00 UTC | 100-sf-spec | GPT-5 Codex | Created spec for porting ShipFlow Core from plugin pilot into an internal ShipFlow skill and versioned audit tool. | draft | /101-sf-ready shipflow-core internal skill port |
| 2026-06-11 10:06:29 UTC | 101-sf-ready | GPT-5 Codex | Checked required sections, user-story fit, scope, proof path, security posture, docs coherence, and metadata lint. | ready | /009-sf-skill-build shipflow-core internal skill port |
| 2026-06-11 10:15:19 UTC | 009-sf-skill-build | GPT-5 Codex | Added internal 900-shipflow-core skill, versioned audit helper, routing/help/docs updates, runtime sync, and public-plugin separation docs. | implemented | /103-sf-verify shipflow-core internal skill port |
| 2026-06-11 10:15:19 UTC | 103-sf-verify | GPT-5 Codex | Verified scenario-first contract with audit script, skill-code index lint, metadata lint, skill budget audit, runtime sync checks, marketplace JSON, stale-path scan, and diff check. | verified | /104-sf-end shipflow-core internal skill port |
| 2026-06-11 10:15:19 UTC | 001-sf-build | GPT-5 Codex | Orchestrated the spec-first build from operator request through local implementation and verification. | implemented locally; ship pending | /104-sf-end shipflow-core internal skill port |

## Current Chantier Flow

100-sf-spec ✅ -> 101-sf-ready ✅ -> 009-sf-skill-build ✅ -> 103-sf-verify ✅ -> 104-sf-end next -> 005-sf-ship pending
