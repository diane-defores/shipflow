...
user_story: "En tant qu'opératrice ShipFlow, je veux une cohérence visuelle stricte sur le site public sans valeur visuelle arbitraire, pour une identité stable, pro et cohérente."
owner: "Diane"
confidence: medium
risk_level: medium
...
linked_systems:
  ...
  - "skills/500-sf-design-from-scratch/SKILL.md"
depends_on:
...
supersedes: []
next_step: "/102-sf-start ShipFlow site token hardening and visual standardization"
---

# Title

ShipFlow Site Token Hardening and Visual Standardization

## Status

Ready.

This chantier enforces visual consistency for the ShipFlow public site with no quick-fix styling shortcuts. The canonical source remains `site/src/styles/global.css`, and all production UI decisions in `site/src` must consume shared tokens and component abstractions instead of ad-hoc literals.

## User Story

En tant qu'opératrice ShipFlow, je veux une règle stricte d'application du design-system sur le site public, sans valeurs visuelles arbitraires dans les pages et composants, pour garantir une identité cohérente, un rendu stable et une qualité visuelle pro sur desktop et mobile.
...
Le site ShipFlow a encore une dette visuelle active: beaucoup de valeurs de couleurs, dimensions, shadows et timings sont exprimées directement dans les fichiers UI. Cela introduit des divergences entre pages, complique les évolutions design, et rend le comportement mobile non homogène.
...
- Design-system governance:
  - Validation and enforcement in `shipflow_data/technical/design-system-authority.md`.
- Evidence and proof:
...
- Le site ShipFlow conserve une source de vérité visuelle unique pour les décisions de design.
...
Upstream:
- Design doctrine via `shipflow_data/technical/design-system-authority.md` and `skills/references/design-system-token-contract.md`.
...
- `103-sf-verify` and `005-sf-ship` use `design_system_drift_check.py` output as a hard gate for this chantier.

## Documentation Coherence

- Mettre à jour `shipflow_data/technical/design-system-authority.md` si la source canonique ou les exceptions légitimes changent.
- Mettre à jour les docs de surface seulement si des pages changent de rôle, d'intention ou de promesse visuelle.
- Conserver le découpage page/surface existant dans `shipflow_data/technical/public-site-and-content-runtime.md`.
...
- [ ] Tâche 7: Exécuter la preuve de conformité chantier
  - Fichiers: `shipflow_data/workflow/test-checklists/shipflow-site-token-hardening-and-visual-standardization.md`
  - Action: renseigner les scénarios requis et passer en `PASS`/`BLOCKED` selon la preuve réelle.
  - Validate with: `103-sf-verify`.
...
- [ ] AC-6: `shipflow_data/technical/design-system-authority.md` reste aligné avec le périmètre site de ce chantier.
