...
owner: "Diane"
user_story: "As the ShipFlow operator, I want helper and pilotage skills to distinguish explain vs summarize vs continue in the first screen, so fresh agents do not confuse help output with real lifecycle continuation."
confidence: high
risk_level: medium
security_impact: no
docs_impact: yes
...
supersedes:
  - "shipflow_data/workflow/specs/agent-clarity-hardening-phase-2.md"
evidence:
...
next_step: "/009-sf-skill-build continue agent-clarity hardening after the phase-3 pilot"
---
...
As the ShipFlow operator, I want helper and pilotage skills to distinguish explain vs summarize vs continue in the first screen, so fresh agents do not confuse help output with real lifecycle continuation.
...
- `302-sf-help` makes it obvious that it explains and routes, but does not continue a chantier or mutate durable state.
- `303-sf-resume` makes it obvious that it summarizes only the visible conversation, not repo state or hidden execution truth.
...
- `shipflow_data/technical/skill-runtime-and-lifecycle.md` captures the helper/pilotage distinction so future skill edits do not reintroduce ambiguity.
...
- `RESUME-VS-TRUTH`: an agent is tempted to use `303-sf-resume` as durable state truth instead of a conversation-only recap.
...
- `python3 tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/agent-clarity-hardening-phase-3.md shipflow_data/technical/skill-runtime-and-lifecycle.md`
- `tools/shipflow_sync_skills.sh --check --skill 302-sf-help`
- `tools/shipflow_sync_skills.sh --check --skill 303-sf-resume`
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-27 | 009-sf-skill-build | GPT-5 Codex | Create phase-3 spec and start helper/pilotage clarity hardening | implemented | Patch targeted skills, validate, and ship if green |

## Current Chantier Flow

- `100-sf-spec` ✅ phase-3 spec created
- `101-sf-ready` ✅ ready via bounded continuation of the active clarity hardening campaign
- `102-sf-start` 🔄 in progress on helper/pilotage boundary clarifications
