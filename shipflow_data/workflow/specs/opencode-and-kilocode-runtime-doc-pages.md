...
owner: "Diane"
user_story: "As the ShipFlow operator, I want one OpenCode page and one KiloCode page that explain discovery, invocation, and configuration boundaries, so fresh agents and humans know what the repo proves for each runtime and what remains runtime-specific."
confidence: high
risk_level: medium
...
depends_on:
  - artifact: "shipflow_data/workflow/specs/agent-clarity-public-docs-handoffs-phase-2.md"
...
next_step: "/005-sf-ship opencode-and-kilocode-runtime-doc-pages"
---
...
As the ShipFlow operator, I want one OpenCode page and one KiloCode page that explain discovery, invocation, and configuration boundaries, so fresh agents and humans know what the repo proves for each runtime and what remains runtime-specific.

## Minimal Behavior Contract

ShipFlow must publish one repo-visible page for OpenCode and one for KiloCode. Each page must explain what the operator types, what the runtime may call internally, how ShipFlow is discovered from this repository, which shim or compatibility path is actually present in the repo, and which runtime-specific setup details are not claimed. The pages must stay aligned with README and the launch cheatsheet.
...
- The backlog item in `shipflow_data/workflow/TASKS.md` can be closed once the pages exist and are linked.
...
The repo now states the handoff vocabulary correctly, but it still lacks dedicated runtime pages that a fresh operator can open directly when asking "how do I use ShipFlow in OpenCode?" or "how do I use ShipFlow in KiloCode?".
...
- `python3 tools/shipflow_metadata_lint.py docs/opencode-shipflow.md docs/kilocode-shipflow.md README.md docs/skill-launch-cheatsheet.md shipflow_data/workflow/specs/opencode-and-kilocode-runtime-doc-pages.md`
...
- `302-sf-help` explains/routes; owner skills execute after handoff.
...
- Keep `shipflow_data/technical/skill-runtime-and-lifecycle.md` unchanged unless link drift appears during implementation.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-28 | 100-sf-spec | GPT-5 Codex | Create a dedicated follow-up spec for the OpenCode and KiloCode runtime documentation pages left out of the shipped public/docs handoff-clarity slice | implemented | Run `/300-sf-docs shipflow runtime pages for OpenCode and KiloCode` |
| 2026-06-28 | 300-sf-docs | GPT-5 Codex | Create dedicated OpenCode and KiloCode runtime pages, then link them from README and the launch cheatsheet without overstating repo-native runtime support | implemented | Run `/103-sf-verify opencode-and-kilocode-runtime-doc-pages` |
| 2026-06-28 | 103-sf-verify | GPT-5 Codex | Verify that the new OpenCode and KiloCode pages match repo-proven shim paths, preserve invocation boundaries, and stay linked from the repo discovery surfaces | verified | Run `/104-sf-end opencode-and-kilocode-runtime-doc-pages` |
| 2026-06-28 | 104-sf-end | GPT-5 Codex | Close the runtime-pages slice, mark the runtime-docs tracker item done, and prepare the dedicated spec for git shipping | closed | Run `/005-sf-ship opencode-and-kilocode-runtime-doc-pages` |
| 2026-06-28 | 103-sf-verify | GPT-5 Codex | Verify whether the runtime-pages slice was actually shipped after closure bookkeeping; implementation remains verified but git ship proof is still missing | partial | Run `/005-sf-ship opencode-and-kilocode-runtime-doc-pages` |

## Current Chantier Flow

- `100-sf-spec` ✅ follow-up runtime-pages spec created
- `101-sf-ready` ✅ bounded runtime-pages scope is ready from the spec contract
- `102-sf-start` ✅ runtime pages and repo discovery links implemented
- `103-sf-verify` ✅ metadata, shim-path, and invocation-boundary checks passed
- `104-sf-end` ✅ tracker and changelog closure completed
- `005-sf-ship` ⏳ pending
