...
- Given a content file lives in `shipflow-site/src/content/**`, when an agent edits it, then it preserves the Astro content collection schema and never adds incompatible ShipFlow frontmatter.
...
- [ ] AC 12: Given implementation finishes, when verification runs, then metadata lint passes for ShipFlow editorial artifacts and `cd shipflow-site && pnpm build` passes or the blocker is documented.
