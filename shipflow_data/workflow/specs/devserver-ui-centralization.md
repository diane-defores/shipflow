...
next_step: "/102-sf-start Centraliser le design system du shell DevServer et réduire la latence des sélecteurs"
---
...
En tant qu’opérateur du menu ShipFlow DevServer, je veux des sélecteurs d’environnement plus rapides et un shell UI dont la logique d’affichage est centralisée, pour réduire la latence perçue et éviter la dérive entre `gum` et fallback bash.
...
- `shipflow.sh`, `menu_gum.sh`, `menu_bash.sh` (sauf consommation directe des helpers `ui_*`).
...
- Le header `ShipFlow DevServer` reste centré dans la largeur 50.
...
- `lib.sh` est sourcé par `shipflow.sh`, `menu_gum.sh`, `menu_bash.sh`, et tous les `local/*.sh` qui utilisent `ui_*`. Tous les fichiers qui importent `ui_choose`/`ui_filter_choose` doivent continuer de fonctionner sans changement d’appel.
...
- `shipflow_data/workflow/TASKS.md` doit être mis à jour avec les tâches de refactor.
- `shipflow_data/technical/context-function-tree.md` doit être mis à jour si les signatures `ui_*` changent.

## Documentation Coherence

- Mettre à jour `shipflow_data/technical/context.md` si la couche UI est réorganisée.
- Mettre à jour `shipflow_data/technical/context-function-tree.md` si les signatures `ui_*` changent.
...
- `readers.ts` appelé sur un répertoire `shipflow_data` partiellement corrompu : diagnostics江东, pas de throw non capturé.
...
- checklist_path: `shipflow_data/workflow/test-checklists/devserver-ui-centralization.md` (à créer si nécessaire)
...
- [ ] Tâche 9 : Ajouter des gardes de cache pour les listes d’environnement (déjà partiellement implémenté)
  - Fichier : `lib.sh`
  - Action : Vérifier que `ENV_LIST_CACHE` et `HOME_FOLDERS_CACHE` sont bien invalidés dans `env_start`, `env_remove`, `env_rename`, et que le TTL est configurable via `SHIPFLOW_LIST_CACHE_TTL`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-22 | 100-sf-spec | unknown | create | draft | /101-sf-ready Centraliser le design system du shell DevServer et réduire la latence des sélecteurs |
| 2026-06-22 | 101-sf-ready | unknown | gate | ready | /102-sf-start Centraliser le design system du shell DevServer et réduire la latence des sélecteurs |
