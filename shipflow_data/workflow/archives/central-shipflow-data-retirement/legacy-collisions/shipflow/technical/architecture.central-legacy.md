...
linked_systems:
  ...
  - "/home/claude/shipflow_data/AGENT.md"
...
invariants:
  - "Global trackers remain in shipflow_data; project source artifacts stay in their respective repos."
...
evidence:
  ...
  - "shipflow_data/CLAUDE.md"
depends_on:
  - "/home/claude/shipflow_data/CONTEXT.md"
  - "/home/claude/shipflow_data/CONTEXT-FUNCTION-TREE.md"
supersedes: []
next_step: "/sf-docs audit ARCHITECTURE.md"
---

# Architecture Context — shipflow_data

## System Shape

`shipflow_data` is a control-plane repository layer for multi-project coordination.
It combines:
...
- `shipflow_data/PROJECTS.md`: canonical project registry map.
- `shipflow_data/TASKS.md`: global task orchestration for all projects.
- `shipflow_data/AUDIT_LOG.md`: audit evidence and trend history.
- `shipflow_data/autre/*` and `shipflow_data/migrations/*`: shared standards and migration context.
...
- `migrations/shipflow_data_metadata_inventory.md`: determines metadata migration scope and risk.
...
- new canonical artifact categories are introduced in `shipflow_data`,
