...
| notefinderz | 🔄 in progress | Run `/sf-prod notefinderz` against the live site, then resume English editorial recovery |
...
| 🟡 | Review major-version updates for FastAPI, Uvicorn, OpenAI Python SDK, and FunASR under `/sf-migrate` before changing worker dependencies | 📋 todo |
...
**Top priority**: Run `/sf-prod notefinderz` against the live site, then resume English editorial recovery.
...
| 🔴 | Plan and implement Astro/Vercel migration path for advisories that require major upgrades: `astro` 6.x, `@astrojs/vercel` 10.x, and `@astrojs/vue` 6.x; use `/sf-migrate` before changing majors | ✅ done |
...
- [x] sf-deps audit (2026-05-24): 0 critical / 0 high / 0 moderate / 0 low after non-major fixes; remaining major-lane follow-up: `eslint` 10.x, `satori` 0.26.x, `lucide-astro` deprecation.
...
| 🟡 | Plan `sentry_flutter` 9.x as `/sf-migrate` work; it remains the only direct major dependency migration after the cleanup pass | 📋 todo |
...
| ✅ | Retirer `npm build` des pre-checks de sf-ship et sf-verify (build reste dans sf-check uniquement) | ✅ done |
| ✅ | Refresh 2025-2026 state of the art : sf-audit-seo (AEO/GEO + llms.txt + INP + schemas AI) | ✅ done |
| ✅ | Refresh 2025-2026 state of the art : sf-audit-design (container queries, :has, view transitions, OKLCH, AI-gen smells) | ✅ done |
| ✅ | Refresh 2025-2026 state of the art : sf-audit-copy (AI-slop blacklist EN+FR, trust signals, AEO/GEO copy, CRO 2026) | ✅ done |
| ✅ | Refresh 2025-2026 state of the art : sf-enrich (AI Visibility Layer, E-E-A-T checklist, schema matrix) | ✅ done |
| ✅ | Créer sf-skills-refresh — skill méta pour refresh mensuel des skills + REFRESH_LOG.md | ✅ done |
| ✅ | Refonte du workflow spec-driven V3 (`sf-spec`, `sf-ready`, `sf-start`, `sf-verify`) + archivage de la doc obsolète racine + README aligné | ✅ done |
| ✅ | Ajouter `sf-model` + enrichir `sf-start` avec routing de modèle et orchestration multi-agent | ✅ done |
...
| 🟡 | Tester un flow complet réel `sf-spec -> sf-ready -> sf-start -> sf-verify -> sf-end -> sf-ship` sur une petite feature pilote | 📋 todo |
...
| 🟡 | Aligner `skills/sf-help/SKILL.md` et les docs restantes sur le workflow spec-driven V3 et la boucle de remédiation de `sf-verify` | 📋 todo |
...
| 🟠 | Plan `/sf-migrate` for deprecated and major-line tracks before changing them: `@primevue/themes` -> `@primeuix/themes`, PrimeVue 3 -> 4, `unplugin-vue-router`/Vue Router 4 -> Vue Router 5, Vite 8, Tailwind 4, Pinia 3, TypeScript 6, and ESLint 10 | ✅ done |
...
| 🟡 | Planifier les mises à jour Flutter compatibles: `go_router` patch, `supabase_flutter` minor, et `share_plus` major via `/sf-migrate` si nécessaire | 📋 todo |
...
| ✅ | Reorganize root migration/metadata docs into `shipflow_data` canonical structure and migrate path references | ✅ done |
...
| 🟡 | Revisit README/docs wording after verification so they reflect shipped behavior rather than migration intent | ✅ done — documentation now references canonical paths in `shipflow_data` |
...
| 🟡 | Route breaking upgrades through `/sf-migrate`: PrimeVue 3→4, Vite 6→8, Vitest 0.34→4, Pinia 2→3, Vue Router 4→5, TypeScript 5→6, Tailwind 3→4, FormKit 1→2, web-ext 8→10 | 📋 todo |
...
**Top priority**: Run `/sf-ready` on `shipflow-github-managed-clone-indexer.md`, then implement the managed clone/indexer boundary only after the ready gate passes.
...
| 🔴 | Run `/sf-ready` for `shipflow-github-managed-clone-indexer.md` so the managed clone/indexer boundary can move from draft to implementation-ready | 📋 todo |
...
- [x] sf-deps migration (2026-05-25): gocharbon major deps lane executed (`eslint@10.4.0`, `satori@0.26.0`, `vue@3.5.34`); audit remains clean (0 critical/0 high/0 moderate/0 low); remaining incompatibility to resolve: replace/update `eslint-plugin-jsx-a11y` for ESLint 10.
