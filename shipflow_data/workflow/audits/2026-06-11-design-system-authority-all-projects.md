...
issue_counts:
  ...
  medium: 2
linked_systems:
  - "shipflow_data/workflow/specs/audit-design-system-authority-all-projects.md"
  - "skills/references/design-system-token-contract.md"
  - "tools/design_system_drift_check.py"
depends_on:
  - artifact: "shipflow_data/workflow/specs/audit-design-system-authority-all-projects.md"
    artifact_version: "1.0.0"
    required_status: ready
supersedes: []
evidence:
  - "Project discovery under /home/claude using .git and shipflow_data markers."
  - "Authority detection checked shipflow_data/technical/design-system-authority.md and nested app authority markers."
  - "Warn-only drift scans ran with /home/claude/shipflow/tools/design_system_drift_check.py."
next_step: "/103-sf-verify audit-design-system-authority-all-projects"
---
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `contentglowz` | Astro site, Flutter app, Remotion worker | missing | `contentglowz_app/lib/presentation/theme/app_theme_tokens.dart`, `app_theme.dart`, `contentglowz_theme.json`, `tools/check_design_tokens.mjs`, existing spec `centraliser-design-tokens-contentflow-app-site.md` | 286 files, 2010 findings | dirty app/auth/runtime files | P1 | `300-sf-docs` create authority, then `503-sf-audit-design-tokens`; avoid edits until dirty scope is owned |
| `winflowz` | Astro site, Flutter app, Tailwind config | missing | `winflowz_app/lib/core/theme/winflowz_theme_tokens.dart`, `app_theme.dart`, specs for settings/design system and keyboard themes | 591 files, 1506 findings | clean at scan time | P1 | `300-sf-docs` authority with site/app scoped entries, then `503`/`504` |
| `shipflow_app` | Astro site, Flutter app | missing | `app/lib/presentation/theme/app_theme.dart`, `app_theme_preference.dart`, theme tests | 163 files, 1190 findings | dirty app diagnostics/build files | P1 | `300-sf-docs` authority, confirm monorepo/root governance, then token audit |
| `socialglowz` | Vite app, Astro site, Tailwind config | missing | `src/stores/theme.ts`, `themeAuto.ts`, `useTheme.ts`, `ThemeSwitch.vue` | 197 files, 1474 findings | dirty setup/native/webview files plus untracked cache/diagnostics | P1 | `300-sf-docs` authority, then classify brand/social-network color exceptions |
| `replayglowz` | Astro site, Flutter app | missing | `replayglowz_app/lib/app/theme.dart`, theme tests | 184 files, 621 findings | dirty workflow artifacts | P2 | `300-sf-docs` authority, then migrate site/app scope |
| `gocharbon` | Astro site | missing | mostly theme-toggle/content-topic files; no obvious central token authority | 103 files, 1064 findings | clean at scan time | P2 | `500-sf-design-from-scratch` or `300-sf-docs` after confirming actual token source |
| `temu` | Vite app | present locally, untracked | `shipflow_data/technical/design-system-authority.md`, canonical page alignment spec | 67 files, 427 findings | dirty UI and workflow files; authority untracked | P2 | finish/verify/ship existing authority and active redesign scope before calling compliant |
| `shipflow` | Astro site, TUI package | present and shipped | `shipflow_data/technical/design-system-authority.md`, token contract/tooling | 49 files, 183 findings | dirty unrelated spec plus this chantier | P3/control | use as doctrine control; later clean site drift separately |

## Findings

### P1 - Missing authority despite existing tokens

`contentglowz`, `winflowz`, `shipflow_app`, and `socialglowz` already have theme/token-like files or theme infrastructure, but no shipped `shipflow_data/technical/design-system-authority.md`. This is the highest-risk pattern because future agents may see local tokens but still choose the wrong canonical source or add local one-off values.
...
- `contentglowz`: `/300-sf-docs technical design-system-authority contentglowz`, then `/503-sf-audit-design-tokens contentglowz`.
- `winflowz`: `/300-sf-docs technical design-system-authority winflowz`, then `/503-sf-audit-design-tokens winflowz`.
- `shipflow_app`: `/300-sf-docs technical design-system-authority shipflow_app`, then `/503-sf-audit-design-tokens shipflow_app`.
- `socialglowz`: `/300-sf-docs technical design-system-authority socialglowz`, then `/503-sf-audit-design-tokens socialglowz`.
- `replayglowz`: `/300-sf-docs technical design-system-authority replayglowz`, then `/503-sf-audit-design-tokens replayglowz`.
- `gocharbon`: `/500-sf-design-from-scratch gocharbon tokens-only` if no central token source is confirmed; otherwise `/300-sf-docs technical design-system-authority gocharbon`.
- `temu`: `/103-sf-verify temu-canonical-page-design-system-alignment`, then `/005-sf-ship` inside `temu` if scoped and safe.
...
find /home/claude -maxdepth 4 -type d -name shipflow_data -print
```
...
Run `103-sf-verify` on `shipflow_data/workflow/specs/audit-design-system-authority-all-projects.md`, then choose the first remediation project. The strongest first target is `contentglowz`, but `winflowz` is cleaner if the operator wants a lower-friction first remediation.
