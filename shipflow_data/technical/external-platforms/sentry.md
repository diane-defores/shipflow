...
This note is the global ShipFlow/Chiclou cheat sheet for Sentry-related freshness checks. Use it before relying on assumptions about SDK setup, environments, releases, source maps, Debug IDs, issue/event evidence, alerts, monitors, Session Replay, PII scrubbing, or production incident proof.

It does not replace Sentry documentation. It records the source map and ShipFlow rules agents should use before changing observability code, deploy proof, release correlation, or project-local Sentry usage docs.
...
- A project uses Sentry but lacks `shipflow_data/technical/platforms/sentry.md`.
...
- issue/event evidence route for `sf-prod`, `sf-fix`, and `sf-verify`

Use `templates/artifacts/project_platform_usage.md` as the starter structure.

## Security Notes

- Never store Sentry auth tokens, raw event payloads, breadcrumbs, replay contents, headers, cookies, auth codes, private URLs, or PII in ShipFlow docs.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/sentry.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/sentry.md
```
...
Update this note when Sentry SDK, environment/release, source map/Debug ID, data collection, Replay, Monitors & Alerts, or ShipFlow incident-proof rules change.
