...
This note is the global ShipFlow/Chiclou cheat sheet for Python-related freshness checks. Use it before relying on assumptions about Python runtime versions, virtual environments, packaging, CLI parsing, subprocess behavior, filesystem/path handling, JSON/YAML tooling, local scripts, or Python-based helper services.

It does not replace Python documentation. It records the source map and ShipFlow rules agents should use before changing Python tooling, dependency setup, project-local Python usage docs, or Python scripts that affect ShipFlow operations.
...
- Python docs list `argparse`, `pathlib`, `subprocess`, `logging`, `json`, `tempfile`, and `venv` as standard library modules relevant to ShipFlow tooling.
...
- A project uses non-trivial Python but lacks `shipflow_data/technical/platforms/python.md`.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/python.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/python.md
```
...
- Python dependency changes -> route to `sf-deps` or `sf-migrate` as appropriate.

## Maintenance Rule

Update this note when Python runtime support, packaging guidance, virtualenv behavior, stdlib APIs used by ShipFlow, or Python dependency/security posture changes.
