...
This note is the global ShipFlow/Chiclou cheat sheet for Supabase-related freshness checks. Use it before relying on assumptions about Supabase Auth, Postgres/RLS, Storage policies, Edge Functions, SSR sessions, API keys, CLI behavior, migrations, generated types, or database advisors.

It does not replace Supabase documentation. It records the source map and ShipFlow rules agents should use before changing Supabase code, policies, migrations, storage, auth, or project-local Supabase usage docs.
...
- A project uses Supabase but lacks `shipflow_data/technical/platforms/supabase.md`.
...
- Never store service-role keys, secret keys, DB passwords, connection strings, JWT signing secrets, refresh tokens, raw table data, or raw storage objects in ShipFlow docs.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/supabase.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/supabase.md
```
