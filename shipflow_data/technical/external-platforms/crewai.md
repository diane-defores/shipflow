...
This note is the global ShipFlow/Chiclou cheat sheet for CrewAI freshness checks. Use it before relying on assumptions about CrewAI agents, crews, tasks, flows, tools, memory, knowledge, MCP integration, LiteLLM/OpenRouter behavior, structured outputs, tracing, CLI behavior, or release-driven migrations.

It does not replace CrewAI documentation. It records the source map and ShipFlow rules agents should use before changing CrewAI code, dependency posture, technical docs, or governance-root CrewAI usage notes.
...
A project using CrewAI should maintain `<governance-root>/shipflow_data/technical/platforms/crewai.md` when CrewAI drives production behavior, with:
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/crewai.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/crewai.md
```

For governance-root CrewAI usage notes:

```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/platforms/crewai.md
rg -n "CrewAI role|LLM/provider|tools|memory|Validation|Maintenance Rule" shipflow_data/technical/platforms/crewai.md
```
...
- CrewAI dependency range, lockfiles, Python version, LiteLLM/OpenAI/Pydantic constraints, or extras changed -> route to `sf-deps` or `sf-migrate`.
