...
This note is the global ShipFlow/Chiclou cheat sheet for Convex-related freshness checks. Use it before relying on assumptions about Convex deployments, generated APIs, auth propagation, Clerk integration, schema/index changes, scheduled functions, preview/staging environments, local deployments, or production deploy keys.

It does not replace Convex documentation. It records the source map and ShipFlow rules agents should use before changing Convex code, deployment configuration, auth logic, or project-local Convex usage docs.
...
- A project uses Convex but lacks `shipflow_data/technical/platforms/convex.md`.
...
- Never store `CONVEX_DEPLOY_KEY`, dashboard tokens, auth issuer secrets, webhook secrets, or raw production documents in ShipFlow docs.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/convex.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/convex.md
```
