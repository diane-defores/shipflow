...
linked_systems:
  ...
  - shipflow_data/technical/external-platforms/README.md
...
This note is the global ShipFlow/Chiclou cheat sheet for Firecrawl-related freshness checks. Use it before relying on assumptions about Firecrawl API endpoints, SDKs, MCP tooling, scrape/crawl/search behavior, document parsing, browser interaction, pricing/credits, retention, or release-driven migration work.

It does not replace Firecrawl documentation. It records the source map and ShipFlow decision rules agents should use before updating code, dependency posture, technical docs, or project-local Firecrawl usage notes.
...
- A project uses Firecrawl but lacks `shipflow_data/technical/platforms/firecrawl.md`.
...
- Never store Firecrawl API keys, raw cookies, authenticated browser profiles, private scraped data, customer documents, raw provider logs, or sensitive extracted content in ShipFlow docs.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/firecrawl.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/firecrawl.md
```

For project-local Firecrawl notes:

```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/platforms/firecrawl.md
rg -n "integration path|endpoint|retention|Validation|Maintenance Rule" shipflow_data/technical/platforms/firecrawl.md
```
...
- Major Firecrawl release affects integration shape -> route upgrades to `sf-deps` or `sf-migrate`; route code impact to `sf-audit-code`; route docs impact to `sf-docs`.

## Maintenance Rule

Update this note when Firecrawl endpoint semantics, SDK coverage, MCP behavior, retention/privacy options, pricing/credit assumptions, self-hosted deployment requirements, or ShipFlow Firecrawl proof rules change. Review it at least monthly while Firecrawl remains a candidate for technical freshness and web-data workflows.
