...
This note is the global ShipFlow/Chiclou cheat sheet for Vercel-related freshness checks. Use it before relying on assumptions about preview deployments, production deployment triggers, environment variables, CLI behavior, deployment URLs, runtime logs, or hosted validation.

It does not replace Vercel documentation. It points agents to the official sources and records ShipFlow decision rules that are repeatedly relevant across projects.
...
- The project depends on preview/production behavior but lacks a local `shipflow_data/technical/platforms/vercel.md` usage note.
...
- `sf-prod` owns deployment truth: matching deployment URL, status, provider logs, runtime logs, and ready/failed/pending state.
- `sf-deploy` owns orchestration around release confidence and should route Vercel provider truth to `sf-prod`.
- `sf-auth-debug` must use hosted proof for auth issues involving OAuth callbacks, cookies, provider dashboards, deployment env, domain, Edge/serverless runtime, or preview/prod differences.
...
- Never store Vercel tokens, project secrets, raw runtime logs containing sensitive data, cookies, private deployment inspection output, or customer data in ShipFlow docs.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/vercel.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/vercel.md
```

For project-local Vercel notes:

```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/platforms/vercel.md
rg -n "validation surface|production branch|Environment|Verification|Maintenance Rule" shipflow_data/technical/platforms/vercel.md
```
...
- `sf-prod` or `sf-auth-debug` reports provider uncertainty -> update or create the local Vercel usage note after evidence is confirmed.

## Maintenance Rule

Update this note when Vercel deployment semantics, CLI behavior, environment model, logging model, MCP routing, or ShipFlow Vercel proof rules change. Review it at least monthly while Vercel remains a common deployment target.
