...
This note is the global ShipFlow/Chiclou cheat sheet for Firebase-related freshness checks. Use it before relying on assumptions about Firebase Authentication, ID tokens, Admin SDK, Firestore Security Rules, Firestore server/client libraries, Firebase Hosting, Firebase CLI deploys, or Firebase/GCP security boundaries.

It does not replace Firebase documentation. It records the source map and ShipFlow rules agents should use before changing Firebase auth, Firestore access, rules, hosting, CI deploys, or project-local Firebase usage docs.
...
- A project uses Firebase but lacks `shipflow_data/technical/platforms/firebase.md`.
...
- Never store service account JSON, refresh tokens, Firebase Admin credentials, ID tokens, cookies, raw user records, or raw Firestore documents in ShipFlow docs.
...
```bash
python3 tools/shipflow_metadata_lint.py shipflow_data/technical/external-platforms/firebase.md
rg -n "Freshness Gate|Source Map|ShipFlow Decision Rules|Maintenance Rule" shipflow_data/technical/external-platforms/firebase.md
```
