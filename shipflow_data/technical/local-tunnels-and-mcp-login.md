...
This doc covers the local tools that connect a workstation to a remote ShipFlow server: app tunnels, saved SSH connection state, remote PM2 and Flutter Web `tmux` port discovery, `shipflow-mcp-login` for remote Codex MCP OAuth callbacks, `shipflow-clerk-login` for remote Clerk CLI OAuth callbacks, `shipflow-blacksmith-login` for remote Blacksmith CLI OAuth callbacks, `shipflow-turso-login` for remote Turso CLI login, and `shipflow-turso-ssh` for Turso CLI auth transfer and schema proof.
...
- Clerk auth tokens remain owned by Clerk CLI on the remote server; ShipFlow
