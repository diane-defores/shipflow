...
linked_systems:
  ...
  - "OpenTUI"
depends_on:
  - "shipflow_data/workflow/specs/shipflow-terminal-tui-v1.md@1.0.0"
supersedes: []
evidence:
...
next_step: "/sf-verify ShipFlow Terminal TUI V1"
---
...
`/home/claude/shipflow/tui` provides an optional local terminal cockpit for ShipFlow operations next to the skills used from the terminal.
...
source of truth stays in ShipFlow Markdown artifacts and skill-run ledgers.

## Documentation surfaces

- Internal operator guide: `/home/claude/shipflow/tui/README.md`.
- Internal technical contract: `/home/claude/shipflow/shipflow_data/technical/terminal-tui.md`.
- Public overview surface: `/home/claude/shipflow/site/src/pages/docs.astro`.
- Repo onboarding pointer: `/home/claude/shipflow/README.md`.
...
- Subproject: `/home/claude/shipflow/tui` (Bun + TypeScript).
...
- `install.sh` calls `tui/scripts/install-shipflow-tui.sh` during per-user setup so the main ShipFlow install also provisions the TUI.
- `tui/scripts/install-shipflow-tui.sh` bootstraps Bun when missing, installs TUI dependencies, and creates `tui`, `sftui`, `sf-tui`, and `shipflow-tui` commands in `~/.local/bin`.
...
`tui/scripts/install-shipflow-tui.sh` during per-user setup.

Set `SHIPFLOW_SKIP_TUI_INSTALL=1` to skip TUI provisioning during a full
ShipFlow install. Set `SHIPFLOW_INSTALL_TUI_FOR_ROOT=1` only when the root
...
Specs keep the selection marker after the traffic light, for example `🟢 > [shipflow_app] [ready] ...`. The source Markdown tables may keep their existing columns; `sources/readers.ts` and `viewModels/dashboard.ts` own normalization into this display contract.

Canonical records are parsed using
`/home/claude/shipflow/skills/references/operational-record-format.md` before any legacy table parsing.
