...
depends_on:
  - artifact: "shipflow_data/technical/blacksmith.md"
...
This note preserves ShipFlow's default GitHub Actions rules for CI cost, cache use, and monorepo workflows. Read it before editing `.github/workflows/*`, debugging quota spikes, adding expensive jobs, or moving a project into a monorepo.
...
- `sf-check` should run allowed local checks before recommending CI retries.
- `sf-deploy` may trigger/push CI only when its ship workflow requires it or Diane explicitly asks.
- `sf-prod` owns live CI/deploy proof, job logs, artifacts, and provider-specific runner evidence.
- `sf-docs` owns this reference and should update it after repeated CI cost incidents.
- `sf-start` and code-changing skills should load this reference before editing `.github/workflows/*`.
...
Update this note when GitHub Actions workflow policy changes, a new stack gets a standard CI pattern, a quota incident reveals a missing guardrail, or ShipFlow skill routing around CI/deploy proof changes.
