---
name: shipflow
description: Route ShipFlow workflows, packs, and packaging audits in OpenCode-compatible agents.
argument-hint: "<instruction | help | packs | audit packaging>"
---

# ShipFlow for OpenCode-compatible agents

Use this skill when working in the ShipFlow repository with an OpenCode-compatible agent runtime.

## Mission

ShipFlow is the public workflow entrypoint for ShipFlow tasks.

Use it when the operator wants to:

- understand what ShipFlow can do from the repository
...
- bootstrap the complete ShipFlow corpus when the lightweight surface is not enough
...
- `audit packaging`, `audit packs`, `portability`, or `local ShipFlow packaging`: run the packaging audit when local ShipFlow source exists.
- `refresh pack`, `update pack`, or `pack maintenance`: explain pack refresh flow and point to the refresh script.
- `spec`, `ready`, `start`, `verify`, `check`, `fix`, or French equivalents: route conceptually to the matching ShipFlow workflow.
- `references`, `docs`, `site`, or `hosted docs`: explain the local-vs-hosted reference policy.
- `full ShipFlow`, `clone repo`, or `installation complète`: offer the sparse bootstrap route and ask before any networked change.

## Reference Strategy

ShipFlow uses local repository references for execution-critical behavior and hosted docs for support material.
