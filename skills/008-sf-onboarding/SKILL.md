...
Because this skill can produce product-activation findings, evaluate whether non-trivial follow-up needs `/100-sf-spec` before implementation.
...
`008-sf-onboarding` is the ShipFlow entrypoint for user activation.
...
This skill complements `006-sf-design`. Design owns broad UI/UX quality. Onboarding owns activation: helping users understand what to do, why it matters, what state they are in, how to recover, and how to get value regardless of technical level. Onboarding may specify visual semantics when they directly support comprehension, but detailed layout, component systems, and visual polish route to `006-sf-design`.
...
| Feature target, audience, or activation promise is too fuzzy | Route to `700-sf-explore` before spec or edits |
