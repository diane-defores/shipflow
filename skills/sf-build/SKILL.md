...
This skill owns end-to-end lifecycle orchestration through `sf-end` and `sf-ship`, with `main-only`, `delegated sequential`, and `spec-gated parallel` as reportable execution modes.
...
Load `$SHIPFLOW_ROOT/skills/sf-build/references/build-lifecycle-workflow.md` for the detailed execution-mode playbook, question framing, governance/documentation gates, browser evidence routing, onboarding gate, and final report templates.
...
`sf-build` is the user-facing lifecycle orchestrator. It keeps user interaction high level while executing:
...
- `delegated sequential` (default): `/sf-build <story>` or `$sf-build <story>` is bounded delegation consent for the current chantier; run one bounded implementation/validation owner at a time.
...
For non-trivial work, run or route through `sf-spec`, then `sf-ready`, and do not run `sf-start` until the spec is `ready`. If readiness fails, apply one correction pass and rerun readiness; stop after the bounded loop with `blocked` or a user decision.
...
- `sf-browser`: non-auth browser evidence.
- `sf-auth-debug`: auth/session/callback/cookie/provider/tenant/protected-route issues.
- `sf-prod`: hosted deployment/runtime truth, logs, serverless/edge behavior, or live deployment health.
- `sf-test`: durable manual QA scripts, retests, and structured test logs.

In `vercel-preview-push` or preview-required `hybrid` mode, ship first, then route to `sf-prod`, then to the downstream proof owner.
...
- `rg -n "Trace category|Process role|Master Delegation|Master Workflow Lifecycle|Existing Chantier Check|Question Gate|Stop Conditions|Final Report|build-lifecycle-workflow" skills/sf-build/SKILL.md`
- `python3 tools/skill_budget_audit.py --skills-root skills --format markdown`
- `python3 tools/shipflow_metadata_lint.py skills/sf-build/references/build-lifecycle-workflow.md`
