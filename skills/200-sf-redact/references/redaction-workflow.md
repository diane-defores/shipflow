...
scope: 200-sf-redact-redaction-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/200-sf-redact/references/redaction-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/200-sf-redact/SKILL.md during Compact ShipFlow Skill Instructions Phase 3."
...
This reference preserves the detailed pre-compaction instructions for `200-sf-redact`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, provider routing, examples, or report details below.
...
→ Lancer /305-sf-init pour générer les fichiers de base, ou /300-sf-docs update pour les mettre à jour.
```
