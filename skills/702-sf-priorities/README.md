...
`702-sf-priorities` reviews open tasks and reorders them using impact, effort, blockers, dependencies, and delivery risk. It defaults to the selected project's local `shipflow_data/workflow/TASKS.md` and uses the external portfolio tracker only when the run is workspace-scoped.
...
```bash
/702-sf-priorities
/702-sf-priorities high-roi
/702-sf-priorities blockers
```
...
- `309-sf-tasks` to clean up the backlog structure
- `102-sf-start` once the top task is chosen
- `303-sf-resume` for a fast thread-level status snapshot
