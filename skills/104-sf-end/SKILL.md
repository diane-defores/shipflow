...
Before closing a spec-first chantier, load `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md`, then read the spec's `Skill Run History` and `Current Chantier Flow` when a unique spec exists. Append a current `104-sf-end` row with result `closed`, `deferred`, `blocked`, or `not applicable`, update `Current Chantier Flow`, and end the report with the compact `Chantier` block from `$SHIPFLOW_ROOT/skills/references/reporting-contract.md`. If no unique spec is available, do not write to a spec; report `Chantier: non applicable` or `Chantier: non trace` with the reason.
...
[📝 Not committed — run /005-sf-ship when ready to push]

## Chantier

Skill courante: 104-sf-end
...
- 104-sf-end: [closed | deferred | blocked | not applicable]
- 005-sf-ship: [status]
...
Prochaine etape:
- [/005-sf-ship puis /405-sf-prod si la validation preview-push est requise | /005-sf-ship | explicit action | None]
...
- Do NOT commit or push — that's 005-sf-ship's job
