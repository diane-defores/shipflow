...
Quand un projet devient gros, un audit monolithique dilue l'attention sur trop de checklists à la fois, ce qui baisse la rigueur de chaque check. `502-sf-audit-design` résout ça avec **deux modes** adaptés à la taille du projet.

## Les 4 modes

| Mode | Déclenchement | Durée estimée | Usage |
|------|---------------|---------------|-------|
| **Page** | `/502-sf-audit-design src/pages/home.astro` | ~5 min | Revue d'une page précise |
| **Project (light)** | `/502-sf-audit-design` (sans argument) | ~15-20 min | Audit de surface sur tout le projet, self-sufficient |
| **Deep** | `/502-sf-audit-design deep` | ~30-45 min | Audit pro-grade sur projet large : 3 specialists en parallèle |
| **Global** | `/502-sf-audit-design global` | variable | Tous les projets UI du workspace d'un coup |
...
```
502-sf-audit-design (deep)
    ├─ 503-sf-audit-design-tokens   ← design tokens (theme + typo + spacing + motion)
    ├─ 504-sf-audit-components      ← architecture composants
    └─ 409-sf-audit-a11y            ← WCAG 2.2 complet + patterns ARIA
```
...
- **En amont** : `400-sf-audit` (top-level) peut appeler `502-sf-audit-design` en mode standard sur plusieurs projets en parallèle.
- **Orchestrée par** : `006-sf-design` quand la demande utilisateur est un objectif design plutôt qu'un audit explicitement choisi.
- **En aval (deep mode)** : spawn 3 agents via Agent tool sur `503-sf-audit-design-tokens`, `504-sf-audit-components`, `409-sf-audit-a11y`.
- **Associée (non appelée)** : `501-sf-design-playground` — générateur de page de preview des design tokens. L'audit recommande de la lancer si aucune playground n'est détectée.
