...
supersedes: []
evidence:
  - "Extracted from 107-sf-test/SKILL.md during residual body-risk cleanup."
next_step: "none"
---

# Manual QA Workflow

Use this reference after the top-level `107-sf-test` activation contract has loaded project development mode and selected a scope.
...
For auth, OAuth, callbacks, protected routes, cookies, or session persistence, include browser-level steps, reload checks, and direct protected-route checks. If the test fails, route to `109-sf-auth-debug` unless the bug is obviously unrelated to auth configuration or browser state.
...
3. Route Web proof to `108-sf-browser` for non-auth UI and `109-sf-auth-debug` for auth/session/callback/protected-route behavior.
...
Right before editing `TEST_LOG.md`, re-read it if it exists. If absent, create it. Append only this compact format:
...
- Follow-up: [none | /106-sf-fix BUG-ID | /107-sf-test --retest BUG-ID | next command]
```

Rules:

- `TEST_LOG.md` is a tracker, not a full bug file.
...
- `bugs/BUG-ID.md`: detailed Markdown source of truth
- `BUGS.md`: optional compact/generated triage index when present or generated
...
2. Re-read optional `BUGS.md` and collect same-day suffixes.
3. Pick the next suffix greater than every suffix found.
4. Check whether `bugs/BUG-ID.md` exists immediately before writing.
...
1. Read `bugs/BUG-ID.md` and optional `BUGS.md`.
...
6. Write only compact pointers in `TEST_LOG.md` and optional `BUGS.md`.
...
- all required scenarios pass: recommend `/103-sf-verify [scope]` if not already done, otherwise `/005-sf-ship`
- preview-push deployment required first: `/005-sf-ship [scope]`, then `/405-sf-prod [project or URL]`, then rerun `/107-sf-test --preview [scope]`
- bug opened: `/106-sf-fix [bug title]`
- auth/browser evidence needed: `/109-sf-auth-debug [bug title]`
- non-auth one-off browser evidence without durable manual log: `/108-sf-browser [URL or scope] [objective]`
- Sentry identifies likely runtime fault but no fix exists: `/106-sf-fix [BUG-ID or Sentry issue summary]`
- expected behavior unclear: `/100-sf-spec [scope]` or `/101-sf-ready [scope]`
- environment/deploy blocked: `/405-sf-prod` or project-specific deployment check
