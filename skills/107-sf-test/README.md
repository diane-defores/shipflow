...
`107-sf-test` is the missing layer between "the agent thinks the work is ready" and "a human has tried the real user flow."
...
100-sf-spec
  ...
  -> 103-sf-verify
  -> 107-sf-test
  -> 003-sf-bug or 106-sf-fix when a bug is found
  -> 107-sf-test --retest
  -> 005-sf-ship
```
...
- after `103-sf-verify`, before `005-sf-ship`
...
- a clean route into `106-sf-fix` when the bug is actionable

## Checklist-first mode

When a ready spec declares a manual checklist under `shipflow_data/workflow/test-checklists/<scope>.md`, `107-sf-test` uses that checklist as the scenario source:
...
Instead of asking "does login work?", `107-sf-test` should guide the actual flow:
...
With `107-sf-test`, the same signal becomes project memory:
...
- Follow-up: /106-sf-fix BUG-2026-04-26-001
```
...
- `003-sf-bug` can read the bug state and continue the next safe lifecycle action through owner skills.
- `107-sf-test` opens or updates the bug file when a scenario fails.
- `106-sf-fix` reads the bug file and appends diagnosis and fix attempts.
- `107-sf-test --retest BUG-ID` appends retest history and moves the bug toward verification or closure.
- `103-sf-verify` checks whether the bug state still blocks the wider release decision.
- `005-sf-ship` uses the bug state to decide whether shipping is clean or partial-risk.
...
```bash
/107-sf-test
/107-sf-test Google Auth
/107-sf-test --retest BUG-2026-04-26-001
/107-sf-test --prod onboarding flow
```

## Limits

`107-sf-test` depends on human observation for flows that cannot be safely or fully automated. It should not invent generic QA scripts detached from the spec, diff, acceptance criteria, or recent verification findings. It also must not log a pass or fail result before the user answers, unless the agent directly collected browser/tool evidence in the same run.

It records evidence; it does not prove the absence of every bug.

## Related Skills

- `103-sf-verify` before manual test planning
- `108-sf-browser` when a one-off non-auth browser proof is enough and no durable QA log is needed
- `109-sf-auth-debug` when auth failure needs browser-level diagnosis
- `003-sf-bug` when a `BUG-ID` needs lifecycle execution
- `106-sf-fix` when a failed test creates an actionable bug
- `005-sf-ship` once checks, verification, and testing are good enough
