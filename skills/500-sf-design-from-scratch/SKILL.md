---
name: 500-sf-design-from-scratch
...
existing UI evidence
  ...
  -> validation
  -> 501-sf-design-playground and 503-sf-audit-design-tokens follow-up
```
...
- preparing or routing to `501-sf-design-playground` after tokens exist
- running or routing to `503-sf-audit-design-tokens` after creation

Rejected scope:

- acting as the future generic `006-sf-design` router
...
- run or route to `501-sf-design-playground` when the user wants visual editing/preview
- run or route to `503-sf-audit-design-tokens` for deep token coherence
- route to `006-sf-design` when the token source exists but the remaining work is a site-wide page/component migration or visual non-regression proof
- route to `504-sf-audit-components` if component variants drift after tokenization
- route to `409-sf-audit-a11y` if contrast, focus, motion, or target-size risks appear
...
- If the project needs a durable migration plan, route to `/100-sf-spec <design-system title>`.
...
"${SHIPFLOW_ROOT:-$HOME/shipflow}/tools/shipflow_sync_skills.sh" --check --skill 500-sf-design-from-scratch
```
...
Follow-up: [501-sf-design-playground / 503-sf-audit-design-tokens / 006-sf-design token migration / none]
