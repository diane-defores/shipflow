...
This reference preserves the detailed lifecycle workflow for `100-sf-spec`. Load it after the compact `SKILL.md` activation contract when this skill is invoked.
...
`100-sf-spec` n'édite jamais `TASKS.md`, `AUDIT_LOG.md` ou legacy `PROJECTS.md` ; la skill produit une spec, pas du tracking.
...
Utiliser ces titres de sections exacts pour que `/101-sf-ready` et `/103-sf-verify` puissent relire la spec mécaniquement :
- `Title`
- `Status`
- `User Story`
...
- **Skill Run History** : table persistante `Date UTC | Skill | Model | Action | Result | Next step`, avec une première ligne `100-sf-spec` pour la création ou mise à jour de la spec.
- **Current Chantier Flow** : résumé lisible des statuts `100-sf-spec`, `101-sf-ready`, `102-sf-start`, `103-sf-verify`, `104-sf-end`, `005-sf-ship` et de la prochaine commande.
...
- Ou continuer à explorer avec /700-sf-explore

## Chantier

Skill courante: 100-sf-spec
