...
Before creating or updating a spec-first chantier, load `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md`. `100-sf-spec` must initialize the chantier registry entry inside the spec itself: frontmatter includes `created_at`, `updated_at`, and `source_model`; the body includes `Skill Run History` and `Current Chantier Flow`; and the first history row records the current `100-sf-spec` run. End the report with the compact `Chantier` block from `$SHIPFLOW_ROOT/skills/references/reporting-contract.md`. If no chantier spec is created or updated, report `Chantier: non applicable` or `Chantier: non trace` with the reason.
...
- `rg -n "Trace category|Process role|Chantier potentiel|Report Modes|Required References|Mode Detection|ready spec|TASKS.md|references/spec-creation-workflow" skills/100-sf-spec/SKILL.md`
