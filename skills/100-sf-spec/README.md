...
- a next step that usually routes to `101-sf-ready`

## Typical Examples

```bash
/100-sf-spec
/100-sf-spec add invite-only onboarding flow
/100-sf-spec fix webhook retry behavior
```

## Limits

`100-sf-spec` prepares implementation; it does not implement the change. It also depends on enough real context to define the contract honestly. If core product decisions are still unresolved, the output should surface those open questions instead of hiding them in vague tasks.

## Related Skills

- `101-sf-ready` to validate the spec before coding
- `102-sf-start` to implement from a ready contract
- `103-sf-verify` to check the finished work against the spec
