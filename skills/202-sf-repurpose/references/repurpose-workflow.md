...
scope: 202-sf-repurpose-repurpose-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/202-sf-repurpose/references/repurpose-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/202-sf-repurpose/SKILL.md during Compact ShipFlow Skill Instructions Phase 2."
...
This reference preserves the detailed pre-compaction instructions for `202-sf-repurpose`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, or examples below.
...
`202-sf-repurpose` is an owner skill, not a master lifecycle skill, but its existing-content placement analysis is read-heavy and can safely use read-only parallel fan-out when subagents are available.
...
- the main `202-sf-repurpose` context integrates the findings into `Existing Content Opportunities`, `Evidence Ledger`, and the final user report

Before any content mutation, docs update, validation, closure, or ship work, stop the read-only fan-out and produce an owner-skill handoff. `202-sf-repurpose` does not edit content directly.
...
If the user clearly wants long-form content or external research, hand off to `200-sf-redact` or `201-sf-enrich` after producing the factual source pack.
...
- route the article/page to `200-sf-redact` because the user asked to apply the recommendation
...
When the user asks to apply, route every `must write` and `should write` placement to the correct owner skill unless blocked by a missing surface, unsafe claim, runtime schema constraint, or user review gate. When the user asks to stop before review, produce the placement analysis and handoff context, then stop before `104-sf-end` or ship.
...
- absence is acceptable; presence means the registry must stay metadata-compliant and should be included in any `300-sf-docs` handoff if stale or incomplete
...
If `shipflow_data/editorial/content-map.md` exists (fallback legacy `CONTENT_MAP.md`), recommend a `300-sf-docs` handoff when a new recurring topic, article, pillar page, or cross-surface rule should be recorded.

If competitor/inspiration or affiliate/referral details become durable project context, recommend a `300-sf-docs` handoff to create or update the optional business registry rather than burying the decision in generated copy.
...
`202-sf-repurpose` must route writing, improvement, audit, and validation work to owner skills. It should not perform those actions itself.

Use this routing map:

| Destination | Owner skill |
| --- | --- |
| Internal documentation, README, workflow docs, technical docs, editorial governance docs | `300-sf-docs` |
| Existing public page, docs page, skill page, FAQ, or article that should be improved | `201-sf-enrich` |
...
If a user asks for a missing `sf-copy` lane, explain that no `sf-copy` skill exists yet. Use the existing owner skills above and recommend a future `009-sf-skill-build sf-copy` chantier only if short-form copy writing remains a recurring gap after audits.
