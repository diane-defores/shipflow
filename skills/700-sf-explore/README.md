...
`700-sf-explore` is ShipFlow’s deliberate thinking mode. It is for clarifying a problem, mapping options, challenging assumptions, and understanding the shape of a solution before anyone writes code.
...
```bash
/700-sf-explore should we add realtime collaboration?
/700-sf-explore why has auth become so hard to maintain here?
/700-sf-explore postgres or sqlite for this local-first tool?
```

## Limits

`700-sf-explore` does not implement features or fixes. If you are already sure what to build, use an execution skill instead.

## Related Skills

- `301-sf-context` to load focused files first
- `100-sf-spec` when exploration needs to become an execution contract
- `102-sf-start` when the problem is clear enough to build
