...
Because this skill has process role `source-de-chantier`, evaluate the standard threshold from `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md` before the final report. If the findings reveal non-trivial future work and no unique chantier owns it, add a `Chantier potentiel` block with `oui`, `non`, or `incertain`, proposed title, reason, severity, scope, evidence, recommended `/sf-spec ...` command, and next step.
...
- `$SHIPFLOW_ROOT/skills/sf-fix/references/bug-fix-workflow.md`
...
Direct bug fixes still require durable bug memory. `sf-fix` must finish with a bug reference and one Markdown bug file under `bugs/*.md` unless the issue is a narrow minor exception.
...
If `spec-first`, do not code; route to `/sf-spec`, `/sf-ready`, then `/sf-start`.

If `diagnostic only`, do not code; route to `sf-auth-debug`, `sf-browser`, or a concrete next step as appropriate.
...
- `rg -n "Trace category|Process role|Chantier Potential|spec-driven-development-discipline|decision-quality-contract|Direct fix|Spec-first|BUG-ID|Stop Conditions|bug-fix-workflow" skills/sf-fix/SKILL.md`
- `python3 tools/skill_budget_audit.py --skills-root skills --format markdown`
- `python3 tools/shipflow_metadata_lint.py skills/sf-fix/references/bug-fix-workflow.md`
