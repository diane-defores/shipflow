...
Preset:
  --preset shipflow|docs  Output routing profile. `000-shipflow` routes to shipflow_data/workflow/conversations/.
...
infer_title_from_raw() {
  ...
  if grep -qi '001-sf-build' "$raw_file" && grep -Eqi 'architecture (de|des|of).{0,40}skills?|skills?.{0,40}architecture' "$raw_file"; then
    printf '%s\n' "Conversation 001-sf-build - architecture des skills"
    return 0
  fi
