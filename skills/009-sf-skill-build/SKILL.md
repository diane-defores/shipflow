...
Before executing from a spec-first chantier, load `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md`, read the spec's `Skill Run History` and `Current Chantier Flow`, append a current `009-sf-skill-build` row with result `implemented`, `partial`, `blocked`, or `rerouted`, update `Current Chantier Flow`, and end with the compact `Chantier` block from `$SHIPFLOW_ROOT/skills/references/reporting-contract.md`.

If no unique spec is identified, do not write to any spec. Route to `/700-sf-explore <idea>` when the skill idea is too fuzzy to frame a ready spec; otherwise route to `/100-sf-spec <title>`.
...
`700-sf-explore when needed -> 100-sf-spec -> edit/create SKILL.md -> 307-sf-skills-refresh -> skill budget audit -> 103-sf-verify -> 300-sf-docs/help update -> 005-sf-ship`
...
2. Validate name policy before any file creation:
   ...
   - no `--`
   - max 64 chars
3. Search existing skills for overlap (`001-sf-build`, `307-sf-skills-refresh`, `skill-creator`, and close neighbors).
4. If overlap is material, ask one targeted question before proceeding.

## Exploration Gate

Before routing to `100-sf-spec`, decide whether the input is clear enough to produce a durable skill-maintenance contract.

Route to `/700-sf-explore <idea>` before `/100-sf-spec` when any of these remain true after a quick overlap scan and, if useful, one targeted question:
...
`700-sf-explore` is read-only for chantier specs. If it writes an `exploration_report`, treat that report as primary intake context for the later `100-sf-spec`. Do not edit `SKILL.md` until exploration has either resolved the ambiguity or explicitly recommended stopping.
...
1. If the request is too fuzzy for a spec, route to `/700-sf-explore <idea>` first.
2. If no matching chantier spec exists and exploration is not needed, route to `/100-sf-spec <title>`.
3. Run `/101-sf-ready <spec>` and block until status is `ready`.
...
- Do not duplicate full internals of `100-sf-spec`, `307-sf-skills-refresh`, `103-sf-verify`, `300-sf-docs`, `302-sf-help`, or `005-sf-ship`; orchestrate them.

### Step 2 — Enforce lifecycle gates in the skill body

The skill body must enforce:

- `700-sf-explore` before `100-sf-spec` when skill intent, placement, or public promise is too ambiguous for a durable spec
- `100-sf-spec` and `101-sf-ready` before non-trivial edits
- `307-sf-skills-refresh <name>` after material skill changes
- `tools/skill_budget_audit.py --skills-root skills --format markdown` after add/rename/expansion
- `103-sf-verify` before closure
- `005-sf-ship` only after verification and bounded ship scope
...
- If `agents/openai.yaml` exists, set `interface.display_name` to the exact skill invocation key, for example `002-sf-maintain`, not a title-cased label such as `SF Maintain`. The `$` skill picker should expose the same name the operator can type.
...
Run `/307-sf-skills-refresh <name>` and apply only additive findings. Do not rewrite the skill from scratch.
...
- Run `/103-sf-verify <spec>`.
- If verify fails, stop and return corrective next step.
- If verify passes, route to `/005-sf-ship "<message>"`.
...
Lifecycle gates:
- 700-sf-explore -> [not needed/rerouted/completed]
- 100-sf-spec -> [status]
- 101-sf-ready -> [status]
- SKILL.md edit/create -> [status]
- 307-sf-skills-refresh -> [status]
- runtime skill links -> [status]
- skill budget audit -> [pass/fail]
- 103-sf-verify -> [status]
- 300-sf-docs/help update -> [status]
- 005-sf-ship route -> [status]
...
Next step:
- [/103-sf-verify <spec> | /005-sf-ship <message> | corrective command]

## Chantier

Skill courante: 009-sf-skill-build
...
- Route to `700-sf-explore` before `100-sf-spec` when skill intent or placement is too fuzzy for one targeted question to settle.
