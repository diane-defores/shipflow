...
scope: 401-sf-audit-code-audit-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
supersedes: []
evidence:
  - "Extracted from skills/401-sf-audit-code/SKILL.md during Compact ShipFlow Skill Instructions Phase 2."
...
This reference preserves the detailed pre-compaction instructions for `401-sf-audit-code`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, or examples below.
...
> Deep dependency analysis has moved to `/402-sf-deps`.
...
> Deep performance analysis has moved to `/403-sf-perf`.
...
6. **PERFORMANCE** — critical issues only (N+1, blocking ops); run `/403-sf-perf` for deep analysis
...
ARCHITECTURE                           [A/B/C/D]
  ...
  Dependency Health (quick)            [A/B/C/D]  → /402-sf-deps for full audit

PERFORMANCE (quick scan)               [A/B/C/D]  → /403-sf-perf for full audit
