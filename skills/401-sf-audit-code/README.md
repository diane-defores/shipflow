...
`401-sf-audit-code` performs a professional code review on either a single file or a full project. It focuses on product behavior, workflow integrity, architecture quality, reliability, and security, not just style issues.
...
```bash
/401-sf-audit-code
/401-sf-audit-code src/lib/auth.ts
/401-sf-audit-code global
```
...
- `106-sf-fix` to move from bug triage to implementation
- `400-sf-audit` for a cross-domain master review
- `105-sf-check` to verify typecheck, lint, and build after changes
