...
scope: 405-sf-prod-production-verification-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/405-sf-prod/references/production-verification-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/405-sf-prod/SKILL.md during Compact ShipFlow Skill Instructions Phase 3."
...
This reference preserves the detailed pre-compaction instructions for `405-sf-prod`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, provider routing, examples, or report details below.
...
`405-sf-prod` ne doit jamais modifier `TASKS.md`, `AUDIT_LOG.md` ou `PROJECTS.md`.

Lire `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills/references/project-development-mode.md` avant de choisir la cible. Si le projet est en mode `vercel-preview-push`, `405-sf-prod` vérifie le déploiement Vercel correspondant au dernier commit poussé et renvoie l'URL de preview prête pour les tests. Dans ce mode, le mot "prod" désigne le gate post-push, pas forcément le domaine de production custom.

After `405-sf-prod` confirms the deployment URL and deploy/runtime state, route page-level browser assertions, visual checks, console summaries, and non-auth network checks to `/108-sf-browser [URL] [objective]`. Keep deployment discovery, Vercel status, build logs, runtime logs, and live health ownership in `405-sf-prod`.
...
- recommander un passage `109-sf-auth-debug` ou une vérification Playwright ciblée si le doute principal porte sur login, session ou redirect
- recommander `/108-sf-browser [URL] [objective]` for a non-auth browser assertion after the deployment URL is confirmed
...
6. **Si erreur détectée** — proposer des actions via **AskUserQuestion** :
   ...
   - Options :
     ...
     - **Lancer /105-sf-check** — "Diagnostic complet avant de corriger"
...
**Options :** Corriger automatiquement | /105-sf-check | Rollback | Ignorer
```
...
- Si la release semble sensible (auth, paiement, données privées, permissions, multi-tenant, webhooks, admin), être explicite sur l'absence éventuelle de preuve côté sécurité ou cohérence produit et recommander `/103-sf-verify` ou une vérification manuelle ciblée.
- Si le risque principal est un flow d'auth navigateur, recommander explicitement `/109-sf-auth-debug [URL ou bug]`.
- If the main remaining risk is a non-auth browser assertion after deployment confirmation, recommend `/108-sf-browser [URL] [objective]`.
