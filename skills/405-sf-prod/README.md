...
`405-sf-prod` verifies the latest deployment by combining deploy status, live URL checks, and build-log access when something failed. On projects that use Vercel preview-push validation, it is also the required post-`005-sf-ship` gate that waits for the matching Vercel deployment before manual/browser tests. It is intentionally skeptical: a green homepage alone is not treated as proof that the shipped feature works.
...
- a route to `108-sf-browser` when the next proof is a non-auth page-level browser assertion
- a clear reminder when auth or protected flows still need `109-sf-auth-debug`

## Typical Examples

```bash
/405-sf-prod
/405-sf-prod my-project
/405-sf-prod https://app.example.com
```
...
- `105-sf-check` before pushing
- `005-sf-ship` to send the change
- `108-sf-browser` when the confirmed deployment URL needs non-auth page, visual, console, or network evidence
- `109-sf-auth-debug` when production doubts are really about login, callbacks, redirects, or private app flows
- `203-sf-research` or `106-sf-fix` if the failure needs investigation
