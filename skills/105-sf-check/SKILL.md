...
- Never edit `TASKS.md`, `AUDIT_LOG.md`, or `PROJECTS.md` from `105-sf-check`.
...
- Options:
  ...
  - **Dependencies** — "Quick vulnerability + outdated check (run /402-sf-deps for full audit)"
...
> For comprehensive dependency auditing (unused deps, license compliance, type coverage, supply chain), run `/402-sf-deps`.
...
Report a quick summary. Do NOT auto-update dependencies. Recommend `/402-sf-deps` for full analysis (unused, duplicates, licenses, configuration).
