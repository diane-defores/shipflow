...
It respects the project development mode: local projects can prove local auth flows locally, while Vercel preview-push projects must use `005-sf-ship` -> `405-sf-prod` before a preview auth result is treated as authoritative.

For public UI, visual, console, network, or non-auth navigation checks, use `108-sf-browser` instead. `109-sf-auth-debug` stays focused on auth, sessions, callbacks, cookies, tenants, providers, and protected routes.
...
```bash
/109-sf-auth-debug login with Google returns to sign-in page
/109-sf-auth-debug Clerk callback fails on staging after Google auth
/109-sf-auth-debug users authenticate but land on a blank dashboard
```
...
- It is not the generic browser evidence path; non-auth page, visual, console, or network checks belong in `108-sf-browser`.
- It is a diagnostic skill, not a replacement for `106-sf-fix`, `102-sf-start`, or `103-sf-verify`.

## Related Skills

- `106-sf-fix` for quick bug triage and direct fixes
- `108-sf-browser` for non-auth page, visual, console, or network evidence
- `100-sf-spec` when the auth bug still lacks a clear contract
- `102-sf-start` to implement the chosen fix path
- `103-sf-verify` to confirm the repaired auth flow is ready to ship
- `405-sf-prod` if the issue only appears after deployment
- `005-sf-ship` then `405-sf-prod` before preview auth retests when the project uses Vercel preview-push validation
