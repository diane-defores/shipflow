...
1. `005-sf-ship` pushes the change.
2. `405-sf-prod` waits for the matching Vercel deployment and returns the deployment URL.
3. `109-sf-auth-debug` uses Playwright against that URL for OAuth, callbacks, cookies, sessions, middleware, and protected-route proof.
