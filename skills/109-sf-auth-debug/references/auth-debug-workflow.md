...
scope: 109-sf-auth-debug-auth-debug-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/109-sf-auth-debug/references/auth-debug-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/109-sf-auth-debug/SKILL.md during Compact ShipFlow Skill Instructions Phase 3."
...
This reference preserves the detailed pre-compaction instructions for `109-sf-auth-debug`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, provider routing, examples, or report details below.
...
`109-sf-auth-debug` est une skill de diagnostic spécialisée. Elle ne remplace pas `100-sf-spec`, `106-sf-fix`, `102-sf-start` ou `103-sf-verify`.
...
Boundary: use `/108-sf-browser` for public UI, visual, console, network, and non-auth navigation checks. Keep `109-sf-auth-debug` for Clerk, Supabase Auth, OAuth, cookies, sessions, callbacks, tenants, protected routes, and auth provider behavior.
...
- `$SHIPFLOW_ROOT/shipflow_data/workflow/specs/master-auth-playbook.md` ou `$SHIPFLOW_ROOT/shipflow_data/technical/master-auth-playbook.md` comme playbook transverse si présent avant tout diagnostic auth multi-app ou tout bug auth non trivial. L'utiliser pour classifier la famille auth, les invariants, la preuve minimale, les stop conditions, et les checklists sécurité/env/redirect.
...
`109-sf-auth-debug` ne doit jamais modifier `TASKS.md`, `AUDIT_LOG.md` ou `PROJECTS.md`.
...
Charger d'abord le playbook auth ShipFlow-local si le fichier existe, sauf si le bug est explicitement un micro-ajustement mono-fichier sans impact auth réel.
...
- Si le mode est `vercel-preview-push`, utiliser une URL de déploiement confirmée par `405-sf-prod` comme surface de preuve pour tout bug auth/callback/session qui dépend du navigateur ou de l'environnement hébergé.
- Si le mode est `hybrid`, utiliser local seulement pour les signaux purement UI/static. Pour OAuth, callbacks, cookies secure/sameSite, domaines autorisés, variables d'env déployées, middleware serverless/edge, ou bug visible seulement en preview/prod, exiger la séquence `005-sf-ship` -> `405-sf-prod` avant de déclarer le flow réparé.
...
- `vercel-preview-push`: si des changements non poussés sont nécessaires au diagnostic, router d'abord vers `/005-sf-ship [scope]`, puis `/405-sf-prod [project or URL]`; reprendre `109-sf-auth-debug` sur l'URL confirmée par `405-sf-prod`.
- `hybrid`: diagnostiquer localement seulement jusqu'au point utile; pour OAuth/callback/session/domaine/cookies hébergés, basculer vers preview après `405-sf-prod`.
...
- Sentry détecté, diagnostics/log-copy UI visible, event ID visible, 5xx, crash, error boundary, ou exception runtime pendant le flow auth -> lire `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills/references/sentry-observability.md`
...
- Mode `vercel-preview-push`, `hybrid` avec flow hébergé, ou Vercel détecté -> lire aussi `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills/references/project-development-mode.md` et utiliser `405-sf-prod` pour obtenir l'URL de déploiement fiable avant Playwright
...
- si la config Playwright MCP pointe vers Google Chrome stable, un channel `chrome`, ou seulement `["-y", "@playwright/mcp@latest"]` sur Linux ARM64, ne pas lancer Playwright comme preuve; router vers `/106-sf-fix BUG-2026-05-02-001`
...
- utiliser l'URL de preview/déploiement confirmée par `405-sf-prod`
- si `405-sf-prod` n'a pas encore confirmé le déploiement du dernier push, arrêter le diagnostic avec next step `/405-sf-prod [project or URL]`
...
Le résultat doit être actionnable par `106-sf-fix`, `102-sf-start` ou `103-sf-verify`.
...
- Validation authority: [local authoritative / preview required via 005-sf-ship -> 405-sf-prod / partial only]
...
Evidence:
- [déploiement vérifié par 405-sf-prod / non requis / manquant]
- [URL finale]
- [message visible]
...
Utiliser `109-sf-auth-debug` comme capability intégrée au workflow existant:
- après `100-sf-spec` si la spec décrit un bug d'auth à confirmer
- depuis `106-sf-fix` quand un bug auth doit être trié rapidement
- pendant `102-sf-start` si l'implémentation dépend d'un diagnostic navigateur réel
- avant `103-sf-verify` pour prouver que le flux cassé a été reproduit
- après un fix pour confirmer que la rupture a disparu
- après `005-sf-ship` et `405-sf-prod` quand le projet utilise `vercel-preview-push` ou un mode `hybrid` concerné par auth hébergée
