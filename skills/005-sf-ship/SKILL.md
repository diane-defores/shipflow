...
Before shipping a spec-first chantier, load `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md`, then read the spec's `Skill Run History` and `Current Chantier Flow` when a unique spec exists. Append a current `005-sf-ship` row with result `shipped`, `not shipped`, `blocked`, or `skipped checks`, update `Current Chantier Flow`, and end the report with a compact `Chantier` block. If quick ship is not attached to one unique chantier spec, do not write to a spec; report `Chantier: non applicable` or `Chantier: non trace` with the reason.
...
- Project-local TASKS.md: !`cat shipflow_data/workflow/TASKS.md 2>/dev/null || cat TASKS.md 2>/dev/null || echo "No TASKS.md"`
- Existing CHANGELOG: !`head -20 CHANGELOG.md 2>/dev/null || echo "no CHANGELOG.md"`

## Your task

`005-sf-ship` has two modes.
...
```
Use `--check --all` for broad skill visibility drift. Do not duplicate symlink repair logic in `005-sf-ship`; route repair to the shared helper or back to the owning lifecycle skill.
...
```bash
/405-sf-prod [project name or deployment URL if known]
```

If the project development mode is `hybrid`, require the same `/405-sf-prod` handoff when the shipped change needs hosted validation: auth/OAuth, callbacks, webhooks, Vercel routing, edge/serverless runtime, deployment env vars, or preview/prod-only data behavior.

Do not tell the user to run manual/browser tests before `405-sf-prod` has confirmed the matching deployment is ready. In the report, state that the push is complete but preview validation is still pending behind `405-sf-prod`.
...
- Use one compact `Flux:` line with status markers, for example `Flux: 100-sf-spec ✅ -> 101-sf-ready ✅ -> 102-sf-start ✅ -> 103-sf-verify ✅ -> 104-sf-end ✅ -> 005-sf-ship ✅🎯`.
- Omit `Trace spec`, `Verdict 005-sf-ship`, `Reste a faire`, and `Prochaine etape` when they are redundant or empty.
- Include `Reste a faire:` only when a concrete remaining item exists.
- Include `Prochaine etape:` only when a real next command/action exists, especially required `/405-sf-prod` preview validation.
...
Flux: 100-sf-spec [status marker] -> 101-sf-ready [status marker] -> 102-sf-start [status marker] -> 103-sf-verify [status marker] -> 104-sf-end [status marker] -> 005-sf-ship [status marker]
[Reste a faire: item, only if non-empty]
[Prochaine etape: /405-sf-prod [project or URL] if preview-push validation is required, only if non-empty]
```
...
- If project mode is `vercel-preview-push`, never make `/107-sf-test` the next step before `/405-sf-prod`.
