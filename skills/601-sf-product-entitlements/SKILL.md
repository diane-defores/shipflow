...
`601-sf-product-entitlements` is the ShipFlow domain skill for product access lifecycle and authorization design.
...
| Sync-oriented questions touching hydration/promote/reinstall recovery | Route entitlement preconditions to `600-sf-local-cloud-sync` |
| Auth/callback/session/cookie/provider identity issues | Route to `109-sf-auth-debug` |
| New implementation path requiring project-level spec | Route `100-sf-spec -> 101-sf-ready -> 102-sf-start/001-sf-build` |
...
- `109-sf-auth-debug` for OAuth/session/callback/token/session anomalies and provider identity failures.
- `600-sf-local-cloud-sync` after entitlement preconditions are explicit, for data promotion, hydration, merge, conflict, tombstones, offline queue, and reinstall-recovery.
- `100-sf-spec` when behavior changes need durable implementation scope.
- `101-sf-ready` for non-trivial ready-state transition.
- `102-sf-start` or `001-sf-build` for execution planning and implementation.
- `103-sf-verify` after contract or implementation work.
- `405-sf-prod` and `107-sf-test` for deployment and manual proof after implementation.
- `300-sf-docs` when claims, support docs, workflow guides, or technical claims drift.
...
rg -n "product-entitlements|product_entitlements|suite ledger|provider event|activation code|snapshot|mirror|backend authorization|600-sf-local-cloud-sync|109-sf-auth-debug|scenario-first" "$SHIPFLOW_ROOT/skills/601-sf-product-entitlements/SKILL.md"
python3 tools/skill_budget_audit.py --skills-root skills --format markdown
tools/shipflow_sync_skills.sh --check --skill 601-sf-product-entitlements
tools/shipflow_metadata_lint.py shipflow_data/workflow/specs/601-sf-product-entitlements-skill.md "${SHIPFLOW_DATA_DIR:-$HOME/shipflow_data}/technical"
git diff --check
```
