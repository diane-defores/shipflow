...
Examples:
- `/407-sf-audit-translate sync`
- `/407-sf-audit-translate apply`
- `/407-sf-audit-translate sync src/i18n`
- `/407-sf-audit-translate apply src/content/blog`
...
1. Read discovered project-local corpora (`shipflow_data/` markers) — check the **Domain Applicability** table. Identify projects with ✓ in the Translate column.
...
5. Update project-local `shipflow_data/workflow/AUDIT_LOG.md` (one traffic-first audit record per project, Translate column) and project-local `shipflow_data/workflow/TASKS.md` (each project's `### Audit: Translate` subsection).
...
- Options:
  ...
  - One option per multilingual project from discovered project-local corpora (`shipflow_data/` markers): label = project name, description = stack
...
1. **Project-local `shipflow_data/workflow/AUDIT_LOG.md`**: create or update a traffic-first `audit:` record for the Translate audit.
