...
```text
intake -> 107-sf-test -> bug file -> 106-sf-fix -> 107-sf-test --retest -> 103-sf-verify -> 005-sf-ship
```
...
- `107-sf-test` owns guided manual QA, failed-test capture, `TEST_LOG.md`, bug files under `bugs/*.md`, optional `BUGS.md` triage updates, and retests.
- `106-sf-fix` owns bug diagnosis, direct/spec-first repair routing, and fix attempts.
- `109-sf-auth-debug` owns auth, OAuth, sessions, callbacks, cookies, tenants, and protected-route browser diagnosis.
- `108-sf-browser` owns narrow non-auth browser evidence.
- `103-sf-verify` owns closure, user-story coherence, and remaining bug-risk verification.
- `005-sf-ship` owns commit/push and pre-ship bug-risk reporting.
- `003-sf-bug` owns status interpretation, safety gates, execution topology, lifecycle continuation, owner-skill routing, integration of downstream evidence, and final bug-loop reporting.
...
- free text -> decide whether this is an observed failure needing `107-sf-test`, a narrow actionable bug needing `106-sf-fix`, or an ambiguous defect needing `100-sf-spec`; continue through that owner when safe.
- `--fix BUG-ID` -> delegate to `106-sf-fix BUG-ID` after confirming the bug file exists.
- `--retest BUG-ID` -> delegate to `107-sf-test --retest BUG-ID`.
- `--verify BUG-ID` -> delegate to `103-sf-verify BUG-ID`.
- `--ship BUG-ID` -> verify bug state first; delegate to `005-sf-ship BUG-ID` only when the bug state does not block clean shipping.
...
- continue through `107-sf-test --retest BUG-ID` or `106-sf-fix BUG-ID` only if enough context remains to make that safe
- otherwise ask for recovery context

If a bug file exists without an index row:

- report the optional index gap
- continue through `107-sf-test --retest BUG-ID` or `106-sf-fix BUG-ID` after confirming the bug file frontmatter and status are usable

## Step 2 — Interpret Status

Use canonical professional bug states:

- `open`: continue through `106-sf-fix BUG-ID`, or through evidence gathering when reproduction is weak.
- `needs-info`: ask for the missing environment, observed behavior, expected behavior, or evidence.
- `needs-repro`: continue through `107-sf-test --retest BUG-ID`, `108-sf-browser`, or `109-sf-auth-debug` based on the missing proof.
- `in-diagnosis`: continue through `106-sf-fix BUG-ID` unless another skill is actively running.
- `fix-attempted`: continue through `107-sf-test --retest BUG-ID`; do not verify or ship as clean yet.
- `fixed-pending-verify`: continue through `103-sf-verify BUG-ID`.
- `closed`: report no action unless the user is investigating a regression or release notes.
- `closed-without-retest`: report residual risk and continue through `107-sf-test --retest BUG-ID` if closure confidence matters.
...
3. If status is `fixed-pending-verify`, continue through `103-sf-verify BUG-ID` first.
4. If status is `closed`, `duplicate`, or `wontfix`, continue through `005-sf-ship BUG-ID` only if the code scope is otherwise bounded.
5. If user explicitly accepts partial-risk shipping, continue through `005-sf-ship BUG-ID` with a risk note; do not claim bug closure.
...
- If neither condition is met, continue through `107-sf-test --retest BUG-ID` or `103-sf-verify BUG-ID` when safe.
...
Spec recommandee: [/100-sf-spec ... | None]
Prochaine etape: [command or None]

## Chantier

Skill courante: 003-sf-bug
