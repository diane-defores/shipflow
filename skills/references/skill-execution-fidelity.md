...
Define how ShipFlow skill activation bodies should expose the instructions Codex must obey.
...
If a required reference is missing, stop and report a ShipFlow installation or contract gap. Do not continue from memory unless the skill explicitly marks that reference as optional for the current objective.
...
The official ShipFlow skill budget audit remains the authority for discovery-budget and body-size compliance.
