...
depends_on:
  ...
  - artifact: "shipflow_data/README.md"
    artifact_version: "1.0.0"
    required_status: reviewed
supersedes: []
evidence:
  - "Operator request 2026-06-30: provide one reusable `#rules` recentering tag so agents can reload the full governance rule set for a ShipFlow-governed project."
...
next_step: "/300-sf-docs audit shared project-governance-rules reference"
---

# Project Governance Rules

Use this reference when the operator says `#rules` or asks what must be true for a project to be governed by ShipFlow.

This is the compact synthesis layer. It does not replace the more detailed contracts it links to.

## Core Rule

A ShipFlow-governed project must keep one canonical governance corpus, one clear ownership path for each decision, and one explicit proof/update path when code, docs, or public claims change.
...
- `shipflow_data/` is the canonical governance and workflow corpus.
- Keep durable decisions in versioned artifacts with frontmatter.
- Keep fast-moving operational trackers under `shipflow_data/workflow/` without forcing frontmatter onto tracker files.
- Keep technical governance under `shipflow_data/technical/`.
- Keep business and product truth under `shipflow_data/business/`.
- Keep public-content and claim governance under `shipflow_data/editorial/`.
- Keep specs, audits, reviews, playbooks, checklists, and evidence under `shipflow_data/workflow/`.
...
- In a monorepo, keep exactly one canonical `shipflow_data/` at the monorepo root.
- Do not create parallel `shipflow_data/` directories under `apps/*`, `packages/*`, or sibling surfaces unless the subproject is intentionally shipped as its own standalone repository.
...
- For any code-changing task, `shipflow_data/technical/code-docs-map.md` is the first technical governance file to load when it exists.
...
- External provider or framework behavior that materially affects decisions should be documented through the ShipFlow technical corpus rather than left as implicit tribal knowledge.

## Editorial And Public-Claim Rules

- Any project with public pages, README promises, docs, FAQ, pricing, support copy, blog/article surfaces, or runtime content must use the editorial corpus under `shipflow_data/editorial/`.
- `shipflow_data/editorial/content-map.md` is the canonical routing map for public content surfaces.
- Public claims should stay aligned with business, product, branding, GTM, and verified implementation truth.
- Runtime content schemas must be preserved; ShipFlow governance metadata does not override app/runtime schema contracts.
...
- Trackers belong under `shipflow_data/workflow/`; do not scatter operational truth across ad hoc root files.
- Reusable playbooks and reusable checklists belong under `shipflow_data/workflow/playbooks/` and `shipflow_data/workflow/checklists/`.
...
- Runtime app content that does not accept ShipFlow governance metadata
...
- missing `shipflow_data/technical/code-docs-map.md` on a code project
- missing `shipflow_data/editorial/README.md` or related editorial corpus files on a public-content project
...
- nested duplicate `shipflow_data/` directories inside a monorepo without a documented standalone exception
...
- Use `#rules #shipflow` when the target system is ShipFlow itself rather than the current project repo

## Maintenance Rule

Update this reference when ShipFlow changes the minimum compliant governance shape, the monorepo topology rule, or the bootstrap/non-compliance boundary for governed projects.
