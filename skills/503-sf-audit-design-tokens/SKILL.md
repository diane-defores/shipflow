---
name: 503-sf-audit-design-tokens
...
- Brand voice: !`if [ -f shipflow_data/branding/branding.md ]; then head -40 shipflow_data/branding/branding.md; else head -40 BRANDING.md 2>/dev/null || echo "no shipflow_data/branding/branding.md (and no legacy BRANDING.md)"; fi`
...
Next steps:
  1. Run /502-sf-audit-design (standard mode) if the visual direction is unclear
  2. Run /500-sf-design-from-scratch to create the professional token system
  3. Run /501-sf-design-playground once the token system exists
  4. Re-run /503-sf-audit-design-tokens for the deep audit
```
