...
supersedes: []
evidence:
  - "Extracted from 106-sf-fix/SKILL.md during residual body-risk cleanup."
next_step: "none"
---

# Bug Fix Workflow

Use this reference after the top-level `106-sf-fix` activation contract has selected a proof path and classified the bug.
...
If `BUGS.md` has an entry but `bugs/BUG-ID.md` is missing, keep the index row, report `needs-info`, and ask for recovery context or route to `/107-sf-test --retest BUG-ID`.
...
For auth, protected routes, OAuth redirects, Clerk/Supabase session state, callback handling, or code that works statically but fails in browser, use `109-sf-auth-debug` as diagnostic evidence while keeping `106-sf-fix` as execution owner.
...
- route preview-push retests through `005-sf-ship -> 405-sf-prod -> 107-sf-test --preview --retest BUG-ID`
...
- run `103-sf-verify` logic after retest before closure

## Spec-First Or Diagnostic-Only

If `spec-first`, do not code. Route to `/100-sf-spec [bug title]`, `/101-sf-ready [bug title or spec path]`, then `/102-sf-start [bug title]`.

If `diagnostic only`, do not code. Route to `109-sf-auth-debug`, `108-sf-browser`, or report suspected root cause and a concrete next command.
