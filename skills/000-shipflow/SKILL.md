...
`000-shipflow` does not write to chantier specs, bug files, release scopes, content surfaces, commits, or deployment state. The selected owner skill owns durable state and chantier tracing after handoff. If invoked inside a spec-first flow, do not modify `Skill Run History`; include `Chantier: non applicable` or `Chantier: non trace` only when useful, with the selected route and reason.
...
`000-shipflow` is a primary router, not a master lifecycle executor. Its default topology is `main-thread routing`:
...
Do not launch a selected master skill inside a subagent. In particular, do not run `001-sf-build`, `002-sf-maintain`, `003-sf-bug`, `004-sf-deploy`, `007-sf-content`, `600-sf-local-cloud-sync`, or `009-sf-skill-build` as a nested subagent from `000-shipflow`. After direct handoff, the selected master or domain skill owns its own delegated sequential execution through the shared delegation reference.
...
When routing a user-facing feature to `001-sf-build` and the instruction mentions
...
Options:
1. Product improvement - hand off to `001-sf-build`.
2. Existing bug - hand off to `003-sf-bug`.
