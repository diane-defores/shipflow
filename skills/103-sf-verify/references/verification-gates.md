...
supersedes: []
evidence:
  - "Extracted from 103-sf-verify SKILL.md during compact-skill pilot."
...
- For preview-required proof, route through `005-sf-ship -> 405-sf-prod -> 107-sf-test --preview` or `109-sf-auth-debug` / `108-sf-browser`.
...
- scope transition moved to a more concrete proof artifact (`107-sf-test` produced a confirmed `Bug Link` or documented manual exception)
...
2. Agent-run Flutter Web smoke for shared UI surfaces that can run without native-only behavior. Use `108-sf-browser` for non-auth UI objectives and `109-sf-auth-debug` for auth/session/callback/protected-route objectives.
