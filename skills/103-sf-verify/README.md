...
It also respects the project development mode: when Vercel preview-push validation is required, `103-sf-verify` must not call work ready to ship until the needed `005-sf-ship` -> `405-sf-prod` preview evidence exists.
...
- a push toward `108-sf-browser` when non-auth browser-observable behavior was not proven
- a push toward `109-sf-auth-debug` when auth behavior was not proven in a real browser
...
```bash
/103-sf-verify
/103-sf-verify invite flow
/103-sf-verify billing webhook retry handling
```

## Limits

`103-sf-verify` is not a full audit and it is not purely technical. It can correct stable code in some cases, but its main job is to judge evidence honestly. If the contract is weak or validation is missing, the result should stay uncertain rather than pretending the work is safe.

## Related Skills

- `105-sf-check` for broader technical validation
- `108-sf-browser` when ship-readiness depends on non-auth page, visual, console, or network evidence
- `109-sf-auth-debug` when ship-readiness depends on a real Clerk, OAuth, or session-flow confirmation
- `100-sf-spec` and `101-sf-ready` to strengthen the contract before implementation
- `005-sf-ship` once verification is good enough to push
- `405-sf-prod` after `005-sf-ship` when the project requires preview-push validation
