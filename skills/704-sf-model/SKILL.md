...
- à partir de quand il faut arrêter d'optimiser et juste lancer `/102-sf-start` ?

Lire `references/model-routing.md` avant de décider.

## Runtime application boundary

`704-sf-model` chooses a model policy; it does not guarantee that the already-running main conversation can mutate its own runtime model mid-thread.
...
- dire explicitement de lancer `/102-sf-start`
