...
`704-sf-model` routes a task to the model that best fits its scope, risk, speed requirements, and expected session length. Instead of overthinking model choice, it gives a practical recommendation: primary model, reasoning level, faster fallback, cheaper fallback, and the point where you should stop optimizing and just start the work.
...
- a fast fallback
- a cheap fallback
- a clear next step, usually `102-sf-start`

## Typical Examples

```bash
/704-sf-model
/704-sf-model "fix payment webhook race condition"
/704-sf-model specs/onboarding-rewrite.md
```
...
- `101-sf-ready` before execution on spec-driven work
- `102-sf-start` to actually run the task
- `303-sf-resume` if you need a quick thread snapshot before deciding
