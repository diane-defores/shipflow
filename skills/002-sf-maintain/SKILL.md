...
Before executing, load `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md`. Search for a matching active `specs/*.md` chantier. If exactly one chantier owns the maintenance scope, append the current `002-sf-maintain` run to `Skill Run History`, update `Current Chantier Flow`, and include the compact `Chantier` block from `$SHIPFLOW_ROOT/skills/references/reporting-contract.md`.

If no matching chantier exists and the maintenance work is non-trivial, run or route through `100-sf-spec` and `101-sf-ready` before implementation. If the work is a narrow local fix safe without a full spec, write a short maintenance mini-contract in the final report and continue in delegated sequential mode. If multiple specs plausibly match, ask the user to select one.
...
`002-sf-maintain` is the project maintenance master skill.
...
- `003-sf-bug` owns bug lifecycle execution, bug files, retests, verification, and ship risk.
- `402-sf-deps` owns dependency health, vulnerabilities, supply chain, licenses, drift, and config.
- `300-sf-docs` owns documentation update/audit, metadata, technical corpus, editorial corpus, and stale-doc repair.
- `105-sf-check` owns local typecheck, lint, build, tests, and quick dependency checks.
- `400-sf-audit` owns broad multi-domain audits.
- `401-sf-audit-code` owns code, architecture, reliability, and security review.
- `404-sf-migrate` owns framework/package upgrade migrations and breaking-change work.
- `309-sf-tasks` owns tracker reconciliation and durable task updates.
- `106-sf-fix`, `001-sf-build`, and `004-sf-deploy` own repair, feature work, and release execution.
...
`/002-sf-maintain` or `$002-sf-maintain` is explicit bounded maintenance delegation consent for the current project. Apply the shared master delegation semantics for short approvals, mini-contracts, degradation, and subagent boundaries.
...
- `deps` -> run the dependency maintenance lane through `402-sf-deps`, remediation/migration when needed, verification, and ship routing.
- `docs` -> run the docs/governance maintenance lane through `300-sf-docs`, validation, verification, and ship routing.
- `audits` -> run the audit maintenance lane through `400-sf-audit` or narrower audit skills, then remediation lifecycle for findings that cross the implementation threshold.
...
- `no-ship` -> run through verification, then stop before `005-sf-ship` or `004-sf-deploy` with a ship-ready report.
...
intake
  ...
  -> 105-sf-check / targeted validations
  -> Documentation Update Plan
  -> Editorial Update Plan when public surfaces or claims changed
  -> 103-sf-verify
  -> 104-sf-end when a chantier needs closure bookkeeping
  -> 004-sf-deploy for release confidence or 005-sf-ship for bounded repo/docs/tooling changes
```

Recommended owner-skill order for broad maintenance:

```text
003-sf-bug -> 402-sf-deps -> 300-sf-docs update/audit -> 105-sf-check nofix -> 401-sf-audit-code or 400-sf-audit -> 404-sf-migrate candidates -> 106-sf-fix/001-sf-build -> 309-sf-tasks -> 103-sf-verify -> 004-sf-deploy/005-sf-ship
```
...
- Lane Executor: write-capable only for one assigned owner lane and write set; use `402-sf-deps`, `300-sf-docs`, `106-sf-fix`, `001-sf-build`, `404-sf-migrate`, or `105-sf-check fix` as appropriate.
...
- Ship Executor: run or route through `004-sf-deploy` when deployment proof is needed, or `005-sf-ship` for bounded repo/docs/tooling changes. Do not ship unrelated dirty files.
...
ShipFlow does not need a separate `400-sf-audit-security` yet. Security maintenance is covered by two existing owners:

- `402-sf-deps`: dependency vulnerabilities, supply chain, package drift, licenses, registry/config posture.
- `401-sf-audit-code`: authn/authz, tenant boundaries, trust boundaries, secrets, webhooks, destructive actions, input validation, secure failure modes, abuse resistance.

`002-sf-maintain security` should:
...
3. Run or route to `/402-sf-deps` for dependency/security posture and remediation proposals.
4. Run or route to `/401-sf-audit-code report=agent` when code-level security review is needed.
...
If repeated security-only work becomes common, recommend a future `/400-sf-audit-security` spec. Do not create it from `002-sf-maintain`.
...
If a spec is created or continued, run `101-sf-ready` and do not start implementation until readiness is `ready`.

## Verification And Ship Gate

After execution:

1. Run focused owner validations and `105-sf-check` when applicable.
...
4. Run or route through `103-sf-verify`.
5. If `no-ship` is absent and verification passes, run or route through:
   - `004-sf-deploy` when deployment truth or browser/manual proof is required.
   - `005-sf-ship` when the change is bounded to repo/docs/tooling and deployment proof is not required.
...
- no matching spec exists for non-trivial maintenance and `100-sf-spec` / `101-sf-ready` cannot produce a ready contract
...
Ship: <004-sf-deploy | 005-sf-ship | no-ship | blocked with reason>
...
- Do not conflate `404-sf-migrate` and `402-sf-deps`: deps finds risk and drift; migrate executes breaking-change upgrade work.
...
- Do not commit, push, deploy, or mark complete without the relevant `005-sf-ship` or `004-sf-deploy` gate.
