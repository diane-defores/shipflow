...
scope: 206-sf-audit-copy-copy-audit-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/206-sf-audit-copy/references/copy-audit-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/206-sf-audit-copy/SKILL.md during Compact ShipFlow Skill Instructions Phase 3."
...
This reference preserves the detailed pre-compaction instructions for `206-sf-audit-copy`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, provider routing, examples, or report details below.
...
→ Lancer /305-sf-init pour générer ces fichiers, ou /300-sf-docs update pour les mettre à jour.
```
...
1. Read discovered project-local corpora (`shipflow_data/` markers) — check the **Domain Applicability** table. Identify projects with ✓ in the Copy column.
...
5. Update project-local `shipflow_data/workflow/AUDIT_LOG.md` (one traffic-first audit record per project, Copy column) and project-local `shipflow_data/workflow/TASKS.md` (each project's `### Audit: Copy` subsection).
...
- Options:
  ...
  - One option per content project from discovered project-local corpora (`shipflow_data/` markers): label = project name, description = stack
...
1. **Project-local `shipflow_data/workflow/AUDIT_LOG.md`**: create or update a traffic-first `audit:` record for the Copy audit.
