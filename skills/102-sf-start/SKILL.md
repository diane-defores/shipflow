...
Before executing from a ready spec, load `$SHIPFLOW_ROOT/skills/references/chantier-tracking.md`, read the spec's `Skill Run History` and `Current Chantier Flow`, and preserve that flow in the execution contract. When a unique spec is used, append a current `102-sf-start` row with result `implemented`, `partial`, `blocked`, or `rerouted`, update `Current Chantier Flow`, and end the report with the compact `Chantier` block from `$SHIPFLOW_ROOT/skills/references/reporting-contract.md`. If the task is direct or no unique chantier spec is identified, do not write to a spec; report `Chantier: non applicable` or `Chantier: non trace` with the reason.
...
- When a unique ready spec is in scope and the only remaining lifecycle proof is local, tool-backed, non-destructive verification, `102-sf-start` may run that local verification itself and report `auto-verify: run`.
...
- Missing or unready spec: stop and route to `/100-sf-spec`, `/101-sf-ready`, then `/102-sf-start`.
