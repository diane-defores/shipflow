...
evidence:
  ...
  - "Added bounded local auto-verify follow-through for 102-sf-start while preserving proof-owner routing."
next_step: "none"
---

# Execution Workflow

This reference preserves the detailed lifecycle workflow for `102-sf-start`. Load it after the compact `SKILL.md` activation contract when this skill is invoked.
...
`102-sf-start` is an execution skill. It should implement, not only plan.
...
- keep `102-sf-start` as the execution owner
- pull in `109-sf-auth-debug` logic before patching when browser evidence is needed to avoid coding blind
- do not assume static code reading is enough for Clerk/OAuth/session issues

If the task needs browser evidence but does not touch auth, sessions, callbacks, tenants, protected routes, or provider behavior, route non-auth browser proof through `108-sf-browser` instead of `109-sf-auth-debug`.
...
If user picks **Clarifier d'abord**, route to `/700-sf-explore [task]` and stop execution.

If `spec-first` and no matching `Status: ready` spec exists:
- stop execution
- route to:
  1. `/100-sf-spec [task]`
  2. `/101-sf-ready [task/spec]`
  3. `/102-sf-start [task]`
...
- Read `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills/704-sf-model/references/model-routing.md` before choosing execution model(s)
- If the spec is missing any of the above, stop and route back to `/101-sf-ready` or `/100-sf-spec`
- If a non-trivial spec lacks `Minimal Behavior Contract`, `Success Behavior`, `Error Behavior`, implementation approach, adversarial gaps, or explicit constraints, stop and route back to `/101-sf-ready` or `/100-sf-spec`
- If the spec is missing required metadata/version context, treat it as a contract gap. Continue only for trivial/local work where the missing metadata cannot change product or security semantics; otherwise route back to `/101-sf-ready`.
...
- If the task touches auth, redirects, protected pages, callback flows, or browser session state, include the relevant login/callback entrypoints and the minimum routes needed for `109-sf-auth-debug`
- If the task touches non-auth browser behavior, visual state, console errors, network failures, or page-level assertions, include the minimum routes and validation objective needed for `108-sf-browser`
- If the task touches Flutter UI that can run on Web, include agent-run Flutter Web smoke scenarios before APK testing; use `108-sf-browser` for non-auth UI objectives and `109-sf-auth-debug` for auth/session/callback/protected-route objectives. Reserve APK/device proof for IME, permissions, overlays, native plugins, platform channels, notifications, storage, install/update, or real-device performance
...
Use `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills/704-sf-model/references/model-routing.md` as the shared provider-aware source of truth, bounded by `${SHIPFLOW_ROOT:-$HOME/shipflow}/skills/references/decision-quality-contract.md`.
...
- the `codex` implementation profile from `704-sf-model` for long agentic implementation, multi-file coding, refactors, hard debugging, and terminal-heavy execution; do not pin this profile to a deprecated slug
...
Treat `spark`, `codex`, `sous-agent`/`subagent`/`agents`, and `mini` arguments as delegated subagent requests using the model-topology alias rules from `704-sf-model/references/model-routing.md`.
...
- when the user story depends on a browser auth flow or protected app path, run or emulate `109-sf-auth-debug` logic to confirm the observable flow in a real browser
- when the user story depends on non-auth browser proof, route to or emulate `108-sf-browser` logic after the Playwright MCP runtime preflight
- targeted tests if available
- If the task produces or updates a manual checklist, create or update it before validation so `107-sf-test` can run against concrete rows.
...
- If `development_mode` is `vercel-preview-push`, local static checks are allowed, but browser/manual/integration/user-flow validation must be deferred until after `005-sf-ship` pushes the change and `405-sf-prod` confirms the matching Vercel deployment. Do not ask the user to test a local URL as proof for this mode.
- If `development_mode` is `hybrid`, use local validation only for purely local unit/static/UI checks. For auth, OAuth, webhooks, hosted env vars, serverless/edge runtime, Vercel routing, or anything that differs by deployment environment, require the `005-sf-ship` -> `405-sf-prod` sequence before the manual or browser test.
- When preview-push validation is required, stop after implementation and quick local checks with next step `/005-sf-ship [task]`. The next action after that successful push must be `/405-sf-prod [project or URL]`; testing follows only after `405-sf-prod` returns the ready preview/deployment URL.

If checks fail, report clearly and include next repair action.

### Step 7.5 — Local auto-verify follow-through

After implementation checks pass, decide whether `102-sf-start` may continue into local verification before the final report.

Use `auto-verify: run` only when all criteria are true:

- one unique ready spec owns the work
- implementation and required local checks in `102-sf-start` scope passed
...
- preview or hosted proof: `005-sf-ship -> 405-sf-prod`, then `108-sf-browser`, `109-sf-auth-debug`, or `107-sf-test` as needed
- production truth or deployment health: `405-sf-prod`
- non-auth browser proof: `108-sf-browser`
- auth/session/provider proof: `109-sf-auth-debug`
- durable manual QA or retest: `107-sf-test`
- broader lifecycle/adversarial verification: `103-sf-verify`

Local auto-verify is not closure, ship, or full lifecycle orchestration. It must not run `104-sf-end`, `005-sf-ship`, commits, pushes, deployments, provider actions, destructive tests, manual prompts, or secret-bearing proof. `001-sf-build` remains the full lifecycle owner when the operator invokes a master build flow.
...
Validation surface: [local / preview after 005-sf-ship -> 405-sf-prod / mixed]
...
Next step:
- [/103-sf-verify [task] | /005-sf-ship [task] when preview-push validation is required]

## Chantier

Skill courante: 102-sf-start
Chantier: [spec path | non applicable | non trace]
Trace spec: [ecrite | non ecrite | non applicable]
Flux:
- 100-sf-spec: [status]
- 101-sf-ready: [status]
- 102-sf-start: [implemented | partial | blocked | rerouted]
...
Prochaine etape:
- [/103-sf-verify [task] | /005-sf-ship [task] puis /405-sf-prod si le mode impose une preview Vercel]

Verdict 102-sf-start:
...
- Exception: when the documented project mode requires Vercel preview-push validation, `102-sf-start` still must not run raw git commands itself, but it must route the immediate next action to `005-sf-ship`, followed by `405-sf-prod`, before any preview/browser/manual test is treated as proof.
...
- Do not silently drop or reinterpret `depends_on` metadata from the spec; version context must survive from read -> implementation -> report -> 103-sf-verify
...
- Reuse the `704-sf-model` routing reference rather than inventing ad hoc model choices
