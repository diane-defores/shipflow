...
docs_impact: yes
linked_systems:
  - skills/409-sf-audit-a11y/SKILL.md
  - skills/409-sf-audit-a11y/references/accessibility-audit-workflow.md
depends_on:
...
This reference preserves the detailed pre-compaction instructions for `409-sf-audit-a11y`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, provider routing, examples, or report details below.
...
- Called by `502-sf-audit-design` in deep mode, or standalone via `/409-sf-audit-a11y`
