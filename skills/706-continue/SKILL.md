...
As a `pilotage` skill, `706-continue` can route toward `/100-sf-spec` when the next useful step clearly deserves a durable chantier, but it should not declare every continuation or backlog note as a chantier source.
...
- scan only the focused context needed to identify candidate work items: active `shipflow_data/workflow/specs/*.md`, bug files, current TASKS entry, release scope, recent validation failure, or the current dirty write scope;
...
- if no durable work item exists and the next work is non-trivial, route to the correct owner (`100-sf-spec`, bug intake, backlog, or a mini-contract) instead of inventing a hidden continuation target.
...
- Existing skill instructions only when directly useful, especially `102-sf-start`, `106-sf-fix`, `105-sf-check`, `103-sf-verify`, `704-sf-model`, or `104-sf-end`.
...
- `spec-route`: the work is non-trivial or ambiguous and needs `100-sf-spec`, `101-sf-ready`, or `102-sf-start` rather than ad hoc execution.
...
- `102-sf-start`: execute a defined task end-to-end.
- `106-sf-fix`: triage and fix a bug.
- `105-sf-check`: run typecheck, lint, build, and repair failures.
- `103-sf-verify`: verify that work is ready to ship.
- `400-sf-audit-*`: run focused audits for code, design, SEO, GTM, copy, a11y, components, dependencies, or performance.
- `100-sf-spec` / `101-sf-ready`: clarify or harden non-trivial work before implementation.
- `704-sf-model`: choose the model when routing is uncertain.
...
- the `codex` implementation profile from `704-sf-model` with `medium` or `high`: long coding agents, multi-file implementation, debugging, refactors, test repair; do not pin this profile to a deprecated slug.
...
If the choice is not obvious, or if the task has high ambiguity, high cost of error, long execution, security/data implications, or unclear provider/runtime constraints, use the `704-sf-model` skill before spawning: open `$SHIPFLOW_ROOT/skills/704-sf-model/SKILL.md`, then read `$SHIPFLOW_ROOT/skills/704-sf-model/references/model-routing.md` if instructed or needed. Otherwise inherit the current model only when that is clearly adequate.
