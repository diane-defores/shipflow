...
- Deployment URL discovery, Vercel status, build logs, runtime logs, and live deploy readiness route to `/405-sf-prod`.
- Actionable code bugs route to `/106-sf-fix` or `/102-sf-start`.
...
In those cases, do not diagnose the app. Report the runtime or ShipFlow installation blocker. For Playwright runtime blockers, route to `/106-sf-fix BUG-2026-05-02-001` or request a Codex/MCP reload as the runtime reference requires. For missing ShipFlow references, report the missing canonical path and route to `/009-sf-skill-build` or `/106-sf-fix` instead of continuing with partial local memory.
...
- Auth wall or auth objective -> `/109-sf-auth-debug [target or bug]`
- Unconfirmed deploy or missing preview URL -> `/405-sf-prod [project or URL]`
- Preview-push project with unshipped changes -> `/005-sf-ship [scope]`, then `/405-sf-prod [project or URL]`
- Full manual flow or durable QA evidence -> `/107-sf-test [scope]`
- Narrow actionable bug -> `/106-sf-fix [summary]`
- Non-trivial or cross-system follow-up -> `/100-sf-spec [title and compact context]`
- Implementation verification gap -> `/103-sf-verify [scope]`
...
If a finding crosses the chantier threshold, report `Chantier potentiel` and route to `/100-sf-spec`. Do not write `BUGS.md`, `bugs/`, `TEST_LOG.md`, `TASKS.md`, `AUDIT_LOG.md`, or `PROJECTS.md` from this skill.
...
Spec recommandee: [/100-sf-spec ... or None]
Prochaine etape: [next command or none]

## Chantier

Skill courante: 108-sf-browser
