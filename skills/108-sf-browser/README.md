...
`108-sf-browser` is ShipFlow's generic browser evidence skill. It opens a local, preview, or production surface in a real browser and checks one concrete objective: visible state, layout, screenshot need, accessibility snapshot, console signal, or network signal.
...
- when a browser finding should route to `106-sf-fix`, `107-sf-test`, `405-sf-prod`, `109-sf-auth-debug`, or `103-sf-verify`
- when `105-sf-check` or `103-sf-verify` says browser-observable non-auth behavior is still unproven
...
```bash
/108-sf-browser https://example.com verify Example Domain is visible
/108-sf-browser local homepage check that the pricing CTA renders
/108-sf-browser preview URL inspect console errors on /dashboard
/108-sf-browser production page verify the docs link is visible
```

## Limits

- It is read-only by default.
- It does not diagnose login, OAuth, cookies, sessions, callbacks, tenants, providers, or protected routes; use `109-sf-auth-debug`.
- It does not replace guided manual QA, retests, `TEST_LOG.md`, bug files, or optional `BUGS.md` triage views; use `107-sf-test`.
- It does not discover deployment URLs, inspect Vercel status, or read runtime logs; use `405-sf-prod`.
- It does not click destructive or production-mutating actions without explicit approval.

## Related Skills

- `109-sf-auth-debug` for auth, session, callback, cookie, tenant, provider, and protected-route evidence
- `107-sf-test` for durable manual QA, retests, and bug records
- `405-sf-prod` for deployment URL, runtime log, and live deploy confidence
- `106-sf-fix` when the browser finding is a narrow actionable bug
- `103-sf-verify` when browser evidence is needed before readiness can be claimed
