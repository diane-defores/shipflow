...
`303-sf-resume` gives a compact summary of the current conversation only. It lists the main completed, in-progress, or planned items, notes any commits made in the conversation when they are visible, then tells you whether the thread can be closed or whether something important would be lost.
...
- `102-sf-start` to continue work in a fresh thread
- `702-sf-priorities` if the summary shows too many competing next steps
- `101-sf-ready` or `103-sf-verify` when the missing item is execution-critical
