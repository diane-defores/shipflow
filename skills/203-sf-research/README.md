...
`203-sf-research` runs a structured multi-source research pass on a topic and turns it into a markdown report with sources, options, best practices, and project-relevant recommendations. It is designed for questions that need more than one blog post or one doc page to answer well.
...
```bash
/203-sf-research
/203-sf-research "best auth stack for Astro SaaS"
/203-sf-research "React Server Components caching patterns"
```
...
- `204-sf-market-study` for market-level validation
- `404-sf-migrate` for upgrade-specific research plus execution
- `300-sf-docs` if the findings should update permanent project docs
