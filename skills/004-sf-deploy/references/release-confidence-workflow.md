...
scope: 004-sf-deploy-release-confidence-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/304-sf-changelog/SKILL.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/004-sf-deploy/SKILL.md to keep the activation body compact."
next_review: "2026-07-13"
next_step: "/103-sf-verify 004-sf-deploy release confidence compaction"
---
...
Define the detailed phase workflow for `004-sf-deploy`. The top-level `SKILL.md` owns activation, routing, stop conditions, and validation visibility; this reference owns the long release-confidence playbook.
...
Do not stage or commit files directly inside `004-sf-deploy`.

If `005-sf-ship` blocks on checks, secrets, bug risk, unrelated dirty files, or push failure, stop at that gate.
...
When Sentry is configured, deployment truth should include only Sentry evidence that `405-sf-prod` can see from a supplied or visible issue/event pointer, plus PM2/Doppler fallback evidence when no pointer exists. Skills must not assume direct Sentry dashboard access. Do not treat missing Sentry evidence as full product proof.
...
`103-sf-verify` must check the user story, success and error behavior, bug gate, documentation coherence, and project development mode implications.
