...
`205-sf-veille` analyzes URLs or pasted content for business relevance across the ShipFlow project portfolio. It fetches and summarizes each item, scores which projects it matters to using project-local governance context, then walks through interactive triage: ignore it, route a content opportunity through `007-sf-content`, turn it into an architecture/product backlog item, or research it further right now.
...
- `007-sf-content` / `202-sf-repurpose` handoffs or `surface missing: blog` findings when a content idea needs an undeclared editorial surface

## Typical Examples

```bash
/205-sf-veille https://example.com/tool
/205-sf-veille https://site-a.com https://site-b.com
/205-sf-veille [paste article notes here]
```
...
- `203-sf-research` when a topic needs deeper investigation after triage
- `007-sf-content` / `202-sf-repurpose` when a source should become public content, repurposing, or a governed content brief
- `309-sf-tasks` when the resulting backlog needs further cleanup
- `703-sf-review` to fold research-driven work into a broader session review
