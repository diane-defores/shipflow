...
`301-sf-context` primes an agent session for a specific task. It checks what is already known, reuses existing memory when possible, and pulls only the most relevant files into context.
...
```bash
/301-sf-context fix the broken billing webhook
/301-sf-context add pagination to the admin users page
/301-sf-context explain src/lib/auth.ts
```

## Limits

`301-sf-context` is a prep step, not an implementation step. It does not search the whole repo indiscriminately, and it is intentionally conservative about how many files it reads up front.

## Related Skills

- `106-sf-fix` for bug-first execution after priming
- `102-sf-start` for full implementation
- `700-sf-explore` when the task is still too fuzzy to execute
