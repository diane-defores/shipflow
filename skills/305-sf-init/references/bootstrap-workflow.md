...
scope: 305-sf-init-bootstrap-workflow
owner: unknown
confidence: high
risk_level: high
security_impact: yes
...
linked_systems:
  ...
  - skills/305-sf-init/references/bootstrap-workflow.md
depends_on:
...
supersedes: []
evidence:
  - "Extracted from skills/305-sf-init/SKILL.md during Compact ShipFlow Skill Instructions Phase 2."
...
This reference preserves the detailed pre-compaction instructions for `305-sf-init`. The top-level `SKILL.md` is now the activation contract; load this file only when the selected mode needs the detailed workflow, checklist, templates, or examples below.
...
- **Vercel preview push** — "Every browser/manual preview test requires 005-sf-ship, then 405-sf-prod waits for Vercel"
...
- post_ship_verification: 405-sf-prod
...
- notes: After each code change that needs browser/manual validation, run 005-sf-ship first, then 405-sf-prod must wait for the matching Vercel deployment before testing.
- last_reviewed: [YYYY-MM-DD]
```
...
- logs_copy_action: [available or missing]
- diagnostic_log_header: [commit/build + Paris/UTC build time or missing]
- notes: Local checks are valid for unit/static work; hosted flows require 005-sf-ship -> 405-sf-prod before manual/browser testing.
- last_reviewed: [YYYY-MM-DD]
```
...
- if root `TASKS.md` exists as a real file, treat it as legacy local content: do not overwrite it, report layout migration debt, and route to `/300-sf-docs migrate-layout` unless an external project tool explicitly requires the root file
...
## Audit Findings
<!-- Populated by /400-sf-audit — dated sections added automatically:
...
## Concurrents connus
[À compléter plus tard — laisser vide avec un placeholder "À renseigner via /204-sf-market-study"]
```
...
evidence:
  - "Detected stack and project files during 305-sf-init"
depends_on:
  - artifact: CLAUDE.md
...
After `shipflow_data/editorial/content-map.md` generation or skip decision, detect and report the project-local governance corpus state. This step is a bootstrap and status gate; long-term maintenance stays owned by `300-sf-docs`.
...
- when no major code area can be mapped precisely, still create `shipflow_data/technical/code-docs-map.md` with an explicit `non-coverage` reason and next step `/300-sf-docs technical`
- do not create a mega-doc during init; route subsystem detail to `/300-sf-docs technical`
...
- name `/300-sf-docs technical` as the recovery command when the map is missing, stale, or incomplete
...
- name `/300-sf-docs editorial` as the adoption command if public surfaces appear later

If public surfaces are detected but `shipflow_data/editorial/` cannot be created safely:
- report `editorial governance: blocked`
- name the blocked files and the next safe command `/300-sf-docs editorial`
...
- If `AGENT.md` is missing, create a baseline project-specific entrypoint that points to `CLAUDE.md`, `shipflow_data/technical/context.md`, `shipflow_data/technical/code-docs-map.md`, `shipflow_data/editorial/content-map.md`, and `README.md`. If only legacy sources such as root `CONTEXT.md`, root `CONTENT_MAP.md`, or `docs/technical/code-docs-map.md` exist, mention them as layout migration debt and route to `/300-sf-docs migrate-layout`.
...
═══════════════════════════════════
Next steps:
  /400-sf-audit        — Run initial audit
  /105-sf-check        — Verify build passes
  /309-sf-tasks        — Start tracking work
  /300-sf-docs technical — Bootstrap or audit technical governance
  /300-sf-docs editorial — Bootstrap or audit editorial governance when public surfaces exist
```
...
- Never overwrite an existing root `TASKS.md` or root `AUDIT_LOG.md`; migrate with `/300-sf-docs migrate-layout` after collision checks.
