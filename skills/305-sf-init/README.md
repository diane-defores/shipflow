...
`305-sf-init` bootstraps a project for the ShipFlow workflow. It detects the stack, generates a project-specific `CLAUDE.md`, creates a real `TASKS.md` tracker, registers the project in the shared workspace registry, and can scaffold initial business and brand context files.
...
```bash
/305-sf-init
/305-sf-init /path/to/project
```

## Limits

`305-sf-init` creates the operating scaffolding, not the product itself. The first pass may still contain draft assumptions that need human confirmation, especially in business and brand documents.

## Related Skills

- `300-sf-docs` to refine generated project documentation
- `302-sf-help` to understand the workflow after setup
- `102-sf-start` once the project is ready for implementation work
