...
1. **Verify token system exists**: scan for `--fs-*`, `--space-*`, `--color-*`, `--duration-*` (or framework equivalent: Tailwind config, theme files, Flutter `Theme.of(context)`). If **no tokens found**, abort with:
   ...
   The playground reads from your token files. Run /500-sf-design-from-scratch
   to create a professional token system, then re-run /501-sf-design-playground.
   ```

   If the user explicitly asks to scaffold "from scratch" with seed tokens,
   route to `/500-sf-design-from-scratch` instead of creating ad hoc seed tokens
...
NEXT STEPS
  ...
  5. If tokens were centralized but pages/components still use hardcoded values:
     run /006-sf-design "migrer le site pour consommer les tokens design centralises sans changement visuel volontaire"
═══════════════════════════════════════
```
...
Next design route: /006-sf-design "migrer le site pour consommer les tokens design centralises sans changement visuel volontaire"
Expected proof: 503-sf-audit-design-tokens -> 105-sf-check -> 108-sf-browser -> 103-sf-verify
```

If hardcoded design values remain across pages, layouts, or components, do not present the playground as the end of the design-system chantier. Route to `006-sf-design` or `001-sf-build` for the site-wide implementation pass.
