...
supersedes: []
evidence:
  - "Extracted from sf-test/SKILL.md during residual body-risk cleanup."
next_step: "none"
---

# Manual QA Workflow

Use this reference after the top-level `sf-test` activation contract has loaded project development mode and selected a scope.
...
For auth, OAuth, callbacks, protected routes, cookies, or session persistence, include browser-level steps, reload checks, and direct protected-route checks. If the test fails, route to `sf-auth-debug` unless the bug is obviously unrelated to auth configuration or browser state.
...
3. Route Web proof to `sf-browser` for non-auth UI and `sf-auth-debug` for auth/session/callback/protected-route behavior.
...
- all required scenarios pass: recommend `/sf-verify [scope]` if not already done, otherwise `/sf-ship`
- preview-push deployment required first: `/sf-ship [scope]`, then `/sf-prod [project or URL]`, then rerun `/sf-test --preview [scope]`
- bug opened: `/sf-fix [bug title]`
- auth/browser evidence needed: `/sf-auth-debug [bug title]`
- non-auth one-off browser evidence without durable manual log: `/sf-browser [URL or scope] [objective]`
- Sentry identifies likely runtime fault but no fix exists: `/sf-fix [BUG-ID or Sentry issue summary]`
- expected behavior unclear: `/sf-spec [scope]` or `/sf-ready [scope]`
- environment/deploy blocked: `/sf-prod` or project-specific deployment check
