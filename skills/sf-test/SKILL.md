...
- `$SHIPFLOW_ROOT/skills/sf-test/references/manual-qa-workflow.md`
...
`sf-test` must identify the feature or bug flow, generate concrete manual steps, prompt the user with result choices, wait for the user's answer before claiming a result, write a durable `TEST_LOG.md` entry, write or update a durable bug file under `bugs/*.md` when the result fails, refresh optional `BUGS.md` triage only when present or generated by the project workflow, and route the next step.

For one-off browser proof that does not need a guided manual QA campaign, `TEST_LOG.md`, bug files, or optional `BUGS.md`, route to `/sf-browser [URL or objective]`.
...
If project mode is `vercel-preview-push` and the requested test targets changed app behavior, do not generate a preview/manual test while dirty code has not been shipped. Route first to `/sf-ship [scope]`, then `/sf-prod [project or URL]`, then rerun `/sf-test --preview [scope]`.
...
Allowed `sf-test` bug transitions:
...
`sf-test` must never mark `closed` directly. It may use `closed-without-retest` only with explicit operator-visible reason and residual risk.
...
- `rg -n "Trace category|Process role|Never invent test results|TEST_LOG|BUG-|project-development-mode|manual-qa-workflow|Final Report" skills/sf-test/SKILL.md`
- `python3 tools/skill_budget_audit.py --skills-root skills --format markdown`
- `python3 tools/shipflow_metadata_lint.py skills/sf-test/references/manual-qa-workflow.md`
