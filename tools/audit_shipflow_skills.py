#!/usr/bin/env python3
"""Execution-fidelity audit for local ShipFlow skills."""
...
def audit_skill(path: Path) -> FindingSet:
    ...
    if path.parent.name == "008-sf-end-user" and not has_any(text, ACTIVATION_ROUTE_SIGNALS):
...
def main() -> int:
    root = Path(os.environ.get("SHIPFLOW_ROOT", str(Path.home() / "shipflow")))
    skills_dir = root / "skills"
    if not skills_dir.exists():
        print(f"ShipFlow skills directory not found: {skills_dir}")
        return 2
