#!/usr/bin/env python3
"""Inventory central ~/shipflow_data repository for retirement-only workflows."""
...
ROOT = Path("/home/claude/shipflow_data")
SHARE_ROOT = Path("/home/claude/shipflow/shipflow_data")
ARCHIVE_ROOT = Path("/home/claude/shipflow/shipflow_data/workflow/archives/central-shipflow-data-retirement")
...
ROOT_TO_SHIPFLOW = {
    ...
    "migrations/shipflow_data_metadata_inventory.md": "workflow/archives/central-shipflow-data-retirement/shipflow_data_metadata_inventory.md",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--json-output",
        type=Path,
        default=Path("/home/claude/shipflow/shipflow_data/workflow/archives/central-shipflow-data-retirement/inventory.json"),
        help="Output JSON file.",
    )
    parser.add_argument(
        "--md-output",
        type=Path,
        default=Path("/home/claude/shipflow/shipflow_data/workflow/archives/central-shipflow-data-retirement/inventory.md"),
...
def canonical_owners_in_home() -> list[Path]:
    ...
    for p in sorted(home.iterdir()):
        if p.is_dir() and p.name != "shipflow_data":
            paths.append(p)
    return paths
...
def classify_entry(path: Path, rel: Path, git_state: str, size: int) -> tuple[str, str | None, str, str]:
    ...
    if len(rel.parts) >= 2 and rel.parts[0] == "projects":
        ...
        if not project_root.joinpath("shipflow_data").exists():
            destination = str(project_root / "shipflow_data" / "workflow" / "/".join(rel.parts[2:]))
            return (
                "project-local",
                destination,
                "migrate-project",
                "project folder exists but has no shipflow_data corpus yet",
            )
...
def write_md(output: Path, entries: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# central shipflow_data retirement inventory",
        "",
