...
mkdir -p "$tmp_project/.git" "$tmp_project/shipflow_data/workflow/conversations" "$tmp_project/docs/conversations"
...
assert_contains \
  ...
  "$shipflow_output" \
  "Destination: $SHIPFLOW_ROOT_UNDER_TEST/shipflow_data/workflow/conversations/"

assert_fails_with \
  "shipflow preset blocks relative project-local conversation destination" \
  "shipflow preset output must stay under $SHIPFLOW_ROOT_UNDER_TEST/shipflow_data/workflow/conversations" \
  bash -c "cd '$tmp_project' && SHIPFLOW_ROOT='$SHIPFLOW_ROOT_UNDER_TEST' '$SCRIPT' --preset shipflow --title 'Bad Relative' --destination shipflow_data/workflow/conversations/bad-relative.md --dry-run"

assert_fails_with \
  "shipflow preset blocks absolute project-local conversation destination" \
  "shipflow preset output must stay under $SHIPFLOW_ROOT_UNDER_TEST/shipflow_data/workflow/conversations" \
  env SHIPFLOW_ROOT="$SHIPFLOW_ROOT_UNDER_TEST" "$SCRIPT" --preset shipflow --title "Bad Absolute" --destination "$tmp_project/shipflow_data/workflow/conversations/bad-absolute.md" --dry-run
