...
describe("buildDashboardViewModel", () => {
  ...
  it("adds a left margin and hanging continuation indent to wrapped lines", () => {
    ...
    try {
        ...
        specs: [
          {
            ...
            title: "Very Long ShipFlow Terminal Specification Name That Must Wrap Cleanly"
          }
        ]
      };
      ...
      const specLineIndex = lines.findIndex((line) => line.includes("Very Long ShipFlow"));

      expect(lines[0]?.startsWith("    ShipFlow TUI")).toBe(true);
  ...
  it("does not match unrelated specs through generic path segments", () => {
    const data: DashboardData = {
      ...
      specs: [
        {
          ...dashboardData.specs[0]!,
          path: "/home/claude/shipflow_app/app/shipflow_data/workflow/specs/shipflow-terminal-tui-v1.md",
          project: "shipflow_app",
          title: "ShipFlow Terminal TUI V1"
        },
        {
          ...dashboardData.specs[1]!,
          path: "/home/claude/shipflow_app/app/shipflow_data/workflow/specs/gocharbon-quiz.md",
    ...
    expect(vm.specLines.join("\n")).not.toContain("ShipFlow Terminal TUI V1");
  });
