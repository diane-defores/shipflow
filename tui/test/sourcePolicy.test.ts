...
describe("SourcePolicy", () => {
  it("allows reading file in allowlisted root", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "sf-tui-"));
  ...
  it("denies file outside allowlisted roots", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "sf-tui-"));
    const other = await mkdtemp(path.join(tmpdir(), "sf-tui-other-"));
  ...
  it("denies symlink escapes", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "sf-tui-"));
    const outside = await mkdtemp(path.join(tmpdir(), "sf-tui-out-"));
