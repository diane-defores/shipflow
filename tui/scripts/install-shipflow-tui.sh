...
COMMAND_PATH="$BIN_DIR/shipflow-tui"

if [ ! -f "$TUI_DIR/package.json" ]; then
  echo "ShipFlow TUI package not found at: $TUI_DIR" >&2
  exit 1
fi
...
touch node_modules/.shipflow-tui-install-stamp
...
if [ -n "\${BUN_BIN:-}" ] && [ -x "\$BUN_BIN" ]; then
  ...
  echo "Bun is required. Re-run: ${TUI_DIR}/scripts/install-shipflow-tui.sh" >&2
  exit 127
fi

cd "\$TUI_DIR"
stamp="node_modules/.shipflow-tui-install-stamp"
if [ ! -d node_modules ] || [ ! -f "\$stamp" ] || [ package.json -nt "\$stamp" ] || { [ -f bun.lock ] && [ bun.lock -nt "\$stamp" ]; }; then
  echo "Installing ShipFlow TUI dependencies..." >&2
  "\$bun_bin" install --frozen-lockfile
  mkdir -p node_modules
  touch "\$stamp"
fi
...
echo "ShipFlow TUI command installed:"
echo "  tui"
echo "  sftui"
echo "  sf-tui"
echo "  shipflow-tui"
