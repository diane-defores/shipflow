...
example_prompts:
  - "/sf-end-user onboard users after the new keyboard permissions flow"
  - "/sf-end-user audit the setup checklist before we ship"
  - "/sf-end-user create the activation plan for the new cloud sync feature"
argument_modes:
...
Use `sf-end-user` when the question is not just "does the feature exist?"
...
For first-run setup overlays, `sf-end-user` can also use the shared
