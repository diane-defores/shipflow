---
title: "Why ShipFlow should not rush global primitives for profiles and tags"
...
summary: "Global primitives look elegant, but they hard-freeze semantics, debugging posture, and runtime obligations earlier than ShipFlow can safely support."
...
That is exactly why ShipFlow should not rush them.

## The distinction that matters

A local convention is implemented inside the ShipFlow layer. It can be changed, renamed, clarified, or even removed without pretending that the host runtime understands it natively.
...
ShipFlow already knows the value of named profiles such as `%Victoire`, `%Prudence`, and `%Ariane`: they change posture and arbitration. They do not replace skill ownership, and they do not remove the need for project context.
...
ShipFlow can document conventions today because documentation is under our control.
...
ShipFlow should first prove the model locally:
...
The issue is whether ShipFlow is ready to promise exact runtime semantics for profiles and tags across threads, tools, and debugging surfaces.
