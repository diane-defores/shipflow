#!/usr/bin/env sh
# Bootstrap ShipFlow without a manual git clone.
...
SHIPFLOW_DIR="${SHIPFLOW_DIR:-$INSTALL_HOME/shipflow}"
BOOTSTRAP_LOG="${SHIPFLOW_BOOTSTRAP_LOG:-$INSTALL_HOME/shipflow-bootstrap.log}"
...
stash_shipflow_changes() {
    ...
    run_or_explain "sauvegarde des modifications locales ShipFlow" as_install_user env \
        GIT_AUTHOR_NAME="ShipFlow Bootstrap" \
        GIT_AUTHOR_EMAIL="shipflow-bootstrap@example.invalid" \
        GIT_COMMITTER_NAME="ShipFlow Bootstrap" \
        GIT_COMMITTER_EMAIL="shipflow-bootstrap@example.invalid" \
        git -C "$SHIPFLOW_DIR" stash push -u -m "shipflow-bootstrap backup $(date -u +%Y%m%dT%H%M%SZ)"
}
...
prepare_log
log "Préparation de l'installation ShipFlow..."
install_bootstrap_deps

if [ -d "$SHIPFLOW_DIR/.git" ]; then
    log "Mise à jour du dépôt ShipFlow..."
    stash_shipflow_changes
    run_or_explain "récupération de la dernière version ShipFlow" as_install_user git -C "$SHIPFLOW_DIR" fetch origin "$BRANCH"
    run_or_explain "sélection de la branche $BRANCH" as_install_user git -C "$SHIPFLOW_DIR" checkout "$BRANCH"
    run_or_explain "mise à jour du dépôt ShipFlow" as_install_user git -C "$SHIPFLOW_DIR" pull --ff-only origin "$BRANCH"
    ...
    log "Téléchargement de ShipFlow..."
