...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("pack", help="Pack id from assets/pack-catalog.json, for example shipflow-main.")
    parser.add_argument(
        "--shipflow-root",
        default=str(Path.home() / "shipflow"),
...
def read_report(staged_root: Path) -> dict:
    report_path = staged_root / "shipflow-pack-report.json"
...
def main() -> int:
    ...
    stage_script = PLUGIN_ROOT / "scripts" / "stage_shipflow_pack.py"
    ...
    print(f"- Report: {staged_root / 'shipflow-pack-report.json'}")
