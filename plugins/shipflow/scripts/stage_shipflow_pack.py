...
DEFAULT_SHIPFLOW_ROOT = Path(os.environ.get("SHIPFLOW_ROOT", Path.home() / "shipflow"))
...
def stage_pack(args: argparse.Namespace) -> int:
    ...
    write_json(
        staged_root / "shipflow-pack-report.json",
        {
    ...
    (staged_root / "README.md").write_text(
        ...
        "Review `shipflow-pack-report.json` before installing or publishing it.\n",
    )
